...
| **8** | **Metin sadeleştirmesi:** `VerticalBrickPlacementCard`'ın "🎉 Sekme Tuğlası Yerleştirildi! / ...Sağlamca Örüldü! / Tuğlan bina kulesine dikey olarak eklendi..." anlatımı kullanıcı tarafından "çocukça" bulundu — rozet "✅ Tamamlandı"ya, başlık `"{tabTitle}" hakkında bilgi sahibi oldun!`'e sadeleşti, açıklama paragrafı kaldırıldı. | ✅ Tamamlandı |
| **9** | **Geri alındı — Kart tamamen kaldırıldı:** Kullanıcı Aşama 8'deki sadeleştirilmiş metni de görüp "bu yazıları tamamen çıkart" dedi. `AskUserQuestion` ile netleştirildi: kart TAMAMEN kaldırılsın, sadece konfeti kalsın (mevcut "✅ Bu bölümü bitirdin → Sıradaki" butonu zaten tamamlanma bilgisini veriyor). `VerticalBrickPlacementCard.jsx` SİLİNDİ, `TopicPage.jsx` artık `ConfettiExplosion`'ı DOĞRUDAN render ediyor — kart/rozet/başlık YOK. Konfeti tekrarını önleme mantığı `celebratingTab` STATE'ine taşındı (ref-check yerine — parent başka sebeple re-render olursa animasyonun ortada kesilmemesi için, bkz. `TopicPage.jsx` yorumu). | ✅ Tamamlandı |
