...
const kafkaDockerDesktopSetupStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'İşletim sistemine göre paket yöneticisi…', en: 'The OS-specific package manager…' }, detail: { tr: 'İşletim sistemine göre paket yöneticisi (winget/brew/apt) Docker Desktop\'ı veya docker.io paketini indirir — bu, Kafka değil, Kafka\'yı içinde çalıştıracak container motorudur.', en: 'The OS-specific package manager (winget/brew/apt) downloads Docker Desktop or the docker.io package — this is not Kafka, it is the container engine that will run Kafka inside it.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'Docker Desktop başlatılır ve taskbar\'da…', en: 'Docker Desktop starts and the whale…' }, detail: { tr: 'Docker Desktop başlatılır ve taskbar\'da balina ikonu görünür — bu, Docker daemon\'unun (arka plan servisi) hazır olduğunun tek görsel kanıtıdır.', en: 'Docker Desktop starts and the whale icon appears in the taskbar — this is the only visual proof that the Docker daemon (background service) is ready.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'docker --version ve docker compose version…', en: 'The docker --version and docker compose…' }, detail: { tr: '`docker --version` ve `docker compose version` komutları, CLI\'ın daemon\'a bağlanabildiğini doğrular — daemon çalışmıyorsa bu komutlar "Cannot connect" hatası verir.', en: 'The `docker --version` and `docker compose version` commands confirm the CLI can reach the daemon — if the daemon isn\'t running, these commands fail with "Cannot connect".' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Bu doğrulama atlanırsa…', en: 'If this verification is skipped…' }, detail: { tr: 'Bu doğrulama atlanırsa, bir sonraki adımdaki `docker compose up` komutu "daemon not running" hatasıyla anında başarısız olur — kurulumun geri kalanı bu ön koşula bağımlıdır.', en: 'If this verification is skipped, the next step\'s `docker compose up` command fails immediately with "daemon not running" — the rest of the setup depends on this prerequisite.' } },
  ],
}
...
const kafkaComposeUpStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'kafka-topics --create --partitions 3…', en: 'kafka-topics --create --partitions 3 runs…' }, detail: { tr: '`kafka-topics --create --partitions 3` çalıştırılır — Kafka, "orders" adında 3 partition\'lı boş bir log dosyası ailesi ayırır, henüz hiçbir veri yoktur.', en: '`kafka-topics --create --partitions 3` runs — Kafka allocates an empty family of 3 partitioned log files named "orders", no data exists yet.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'kafka-topics --list topic\'in gerçekten…', en: 'kafka-topics --list confirms the topic…' }, detail: { tr: '`kafka-topics --list` topic\'in gerçekten var olduğunu doğrular — bu adım atlanırsa, bir sonraki komutun "topic bulunamadı" hatası verip vermeyeceği bilinmez.', en: '`kafka-topics --list` confirms the topic truly exists — skipping this step means you don\'t know if the next command will fail with "topic not found".' } },
    { id: 3, icon: '3️⃣', label: { tr: 'console-producer\'a yazılan her satır…', en: 'Each line typed into the…' }, detail: { tr: 'console-producer\'a yazılan her satır ayrı bir mesaj olarak "orders" topic\'ine EKLENİR, üzerine yazılmaz — Enter tuşu her satırı anında gönderir.', en: 'Each line typed into the console-producer is APPENDED as a separate message to the "orders" topic, never overwritten — pressing Enter sends each line instantly.' } },
    { id: 4, icon: '4️⃣', label: { tr: '--from-beginning ile başlatılan…', en: 'The console-consumer started with…' }, detail: { tr: '`--from-beginning` ile başlatılan console-consumer, offset 0\'dan başlayarak TÜM mesajları okur — mesajlar zaten log\'da kalıcı olarak durduğu için consumer ne zaman başlarsa başlasın hiçbir şey kaybolmaz.', en: 'The console-consumer started with `--from-beginning` reads ALL messages starting from offset 0 — because messages already sit permanently in the log, nothing is lost no matter when the consumer starts.' } },
  ],
}
...
const kafkaBinaryInstallStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'Önce java -version ile Java 11+ kontrol edilir…', en: 'First java -version checks for Java 11+…' }, detail: { tr: 'Önce `java -version` ile Java 11+ kontrol edilir — Kafka JVM üzerinde çalışır, bu ön koşul Docker\'da otomatik gelir ama binary kurulumda SEN sağlamak zorundasın.', en: 'First `java -version` checks for Java 11+ — Kafka runs on the JVM, this prerequisite comes automatically in Docker but in a binary install YOU must provide it.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'ZooKeeper & ile arka planda başlatılır…', en: 'ZooKeeper is started in the background with &…' }, detail: { tr: 'ZooKeeper `&` ile arka planda başlatılır — bu, cluster koordinasyon kaydını (broker listesi, leader seçimi metadata\'sı) tutan ayrı bir süreçtir.', en: 'ZooKeeper is started in the background with `&` — this is a separate process that holds the cluster coordination registry (broker list, leader-election metadata).' } },
    { id: 3, icon: '3️⃣', label: { tr: 'Kafka broker\'ı da ayrı bir arka plan…', en: 'The Kafka broker is also started as a…' }, detail: { tr: 'Kafka broker\'ı da ayrı bir arka plan süreci olarak başlatılır ve ZooKeeper\'a bağlanır — Docker Compose\'daki tek `up` komutu burada İKİ elle başlatılan komuta dönüşür.', en: 'The Kafka broker is also started as a separate background process and connects to ZooKeeper — the single `up` command in Docker Compose becomes TWO manually started commands here.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Bu iki süreçten biri çökerse (örn…', en: 'If either of these two processes dies (e.g…' }, detail: { tr: 'Bu iki süreçten biri çökerse (örn. terminal kapatılırsa) hiçbir otomatik yeniden başlatma yoktur — Docker Compose\'un `restart` politikasının aksine, binary kurulumda süreç sağlığını SEN izlersin.', en: 'If either of these two processes dies (e.g. the terminal closes), there is no automatic restart — unlike Docker Compose\'s `restart` policy, in a binary install YOU monitor process health.' } },
  ],
}
...
const kafkaProducerSendFlowStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'ProducerRecord oluşturulur…', en: 'A ProducerRecord is built…' }, detail: { tr: '`ProducerRecord` oluşturulur: topic="orders", key="user-123", value=JSON — key, mesajın hangi partition\'a gideceğini belirleyecek olan tek alandır.', en: 'A `ProducerRecord` is built: topic="orders", key="user-123", value=JSON — the key is the single field that will decide which partition the message goes to.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'producer.send() ASENKRON çalışır…', en: 'producer.send() runs ASYNCHRONOUSLY…' }, detail: { tr: '`producer.send()` ASENKRON çalışır — ana thread hemen devam eder, gönderim arka planda bir buffer\'da kuyruğa alınır.', en: '`producer.send()` runs ASYNCHRONOUSLY — the main thread continues immediately, the send is queued in a background buffer.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'acks=all ayarı sayesinde broker…', en: 'Because of acks=all…' }, detail: { tr: '`acks=all` ayarı sayesinde broker, mesajı SADECE leader değil TÜM in-sync replica\'lar yazana kadar onaylamaz — bu, en yavaş ama en güvenli seçenektir.', en: 'Because of `acks=all`, the broker does NOT confirm the message until ALL in-sync replicas — not just the leader — have written it — this is the slowest but safest option.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Onay geldiğinde callback tetiklenir…', en: 'When the acknowledgment arrives…' }, detail: { tr: 'Onay geldiğinde callback tetiklenir: `exception == null` ise partition/offset loglanır, değilse (örn. `retries=3` tükendiyse) hata yakalanır — sonuç asla sessizce kaybolmaz.', en: 'When the acknowledgment arrives, the callback fires: if `exception == null`, partition/offset are logged; otherwise (e.g. `retries=3` exhausted), the error is caught — the outcome is never silently lost.' } },
  ],
}
...
const kafkaSpringPomSetupStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'spring-kafka dependency\'si eklendiğinde…', en: 'When the spring-kafka dependency is added…' }, detail: { tr: '`spring-kafka` dependency\'si eklendiğinde, versiyonu Spring Boot parent POM\'u yönetir — Kafka client kütüphanesiyle uyumsuz bir versiyon seçme riski ortadan kalkar.', en: 'When the `spring-kafka` dependency is added, its version is managed by the Spring Boot parent POM — the risk of picking a version incompatible with the Kafka client library disappears.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'Bu tek dependency…', en: 'This single dependency transitively…' }, detail: { tr: 'Bu tek dependency, arka planda `kafka-clients` kütüphanesini de transitive olarak getirir — `KafkaTemplate` ve `@KafkaListener` bu ham client\'ı sarmalar.', en: 'This single dependency transitively brings in the `kafka-clients` library too — `KafkaTemplate` and `@KafkaListener` wrap that raw client underneath.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'jackson-databind AYRI eklenmezse…', en: 'If jackson-databind is NOT added separately…' }, detail: { tr: '`jackson-databind` AYRI eklenmezse, `JsonSerializer`/`JsonDeserializer` mesajı JSON\'a çeviremez ve uygulama başlangıçta `ClassNotFoundException` ile çöker.', en: 'If `jackson-databind` is NOT added separately, `JsonSerializer`/`JsonDeserializer` cannot convert the message to JSON and the app crashes at startup with `ClassNotFoundException`.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Java analojisi…', en: 'Java analogy…' }, detail: { tr: 'Java analojisi: Spring Data JPA\'nın Hibernate\'i sarmalaması gibi, spring-kafka da raw `KafkaProducer`/`KafkaConsumer`\'ı sarmalar — az kod, aynı alttaki mekanizma.', en: 'Java analogy: just as Spring Data JPA wraps Hibernate, spring-kafka wraps the raw `KafkaProducer`/`KafkaConsumer` — less code, same underlying mechanism.' } },
  ],
}
...
const kafkaSpringProducerStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'OrderEvent nesnesi…', en: 'The OrderEvent object is built from the…' }, detail: { tr: '`OrderEvent` nesnesi, sipariş verisinden inşa edilir — bu, raw Kafka API\'deki `ProducerRecord`\'un Spring soyutlamasıdır.', en: 'The `OrderEvent` object is built from the order data — this is the Spring abstraction over the raw Kafka API\'s `ProducerRecord`.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'kafkaTemplate.send("orders"…', en: 'In kafkaTemplate.send("orders"…' }, detail: { tr: '`kafkaTemplate.send("orders", order.getUserId(), event)` çağrısında key=userId seçilir — böylece aynı kullanıcının TÜM event\'leri aynı partition\'a, dolayısıyla sırayla işlenir.', en: 'In `kafkaTemplate.send("orders", order.getUserId(), event)`, key=userId is chosen — so ALL events for the same user go to the same partition and are therefore processed in order.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'send() bir CompletableFuture döner…', en: 'send() returns a CompletableFuture…' }, detail: { tr: '`send()` bir `CompletableFuture` döner — `.whenComplete()` bloğu, gönderim tamamlandığında (başarılı veya başarısız) ÇAĞRILAN thread\'i bloklamadan çalışır.', en: '`send()` returns a `CompletableFuture` — the `.whenComplete()` block runs when the send finishes (success or failure), without blocking the CALLING thread.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'ex == null ise partition/offset loglanır…', en: 'If ex == null…' }, detail: { tr: '`ex == null` ise partition/offset loglanır — bu bilgi, mesajın TAM OLARAK nereye yazıldığını kanıtlayan tek log satırıdır ve production debugging\'de kritiktir.', en: 'If `ex == null`, partition/offset are logged — this is the one log line that proves EXACTLY where the message was written, and it is critical in production debugging.' } },
  ],
}
...
const kafkaSpringConsumerStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'Raw API\'deki while(true) {…', en: 'The raw API\'s while(true) {…' }, detail: { tr: 'Raw API\'deki `while(true) { consumer.poll(...) }` döngüsü tamamen ortadan kalkar — `@KafkaListener` annotation\'ı bu döngüyü Spring\'in kendi thread havuzunda arka planda çalıştırır.', en: 'The raw API\'s `while(true) { consumer.poll(...) }` loop disappears entirely — the `@KafkaListener` annotation runs that loop in Spring\'s own thread pool behind the scenes.' } },
    { id: 2, icon: '2️⃣', label: { tr: '@Payload mesajın JSON body\'sini…', en: '@Payload automatically deserializes the…' }, detail: { tr: '`@Payload` mesajın JSON body\'sini otomatik `OrderEvent` nesnesine deserialize eder — `JsonDeserializer` bunu senin elle `ObjectMapper.readValue()` çağırmana gerek kalmadan yapar.', en: '`@Payload` automatically deserializes the message JSON body into an `OrderEvent` object — `JsonDeserializer` does this without you manually calling `ObjectMapper.readValue()`.' } },
    { id: 3, icon: '3️⃣', label: { tr: '@Header(RECEIVED_PARTITION) ve…', en: '@Header(RECEIVED_PARTITION) and…' }, detail: { tr: '`@Header(RECEIVED_PARTITION)` ve `@Header(OFFSET)` metadata\'yı ayrı parametreler olarak enjekte eder — raw API\'de bu bilgi `ConsumerRecord.partition()`/`.offset()` ile elle çekilirdi.', en: '`@Header(RECEIVED_PARTITION)` and `@Header(OFFSET)` inject metadata as separate parameters — in the raw API this data would be pulled manually with `ConsumerRecord.partition()`/`.offset()`.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'İkinci bir @KafkaListener (audit-group)…', en: 'A second @KafkaListener (audit-group)…' }, detail: { tr: 'İkinci bir `@KafkaListener` (audit-group) AYNI "orders" topic\'ini BAĞIMSIZ bir consumer group olarak dinler — iki listener birbirinden habersiz, kendi offset\'ini kendi ilerletir.', en: 'A second `@KafkaListener` (audit-group) listens to the SAME "orders" topic as an INDEPENDENT consumer group — the two listeners are unaware of each other, each advancing its own offset.' } },
  ],
}
...
const kafkaEcosystemPomStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'spring-kafka normal dependency olarak eklenir…', en: 'spring-kafka is added as a normal dependency…' }, detail: { tr: '`spring-kafka` normal dependency olarak eklenir — production jar\'ına dahil olur, çünkü uygulama gerçek Kafka\'ya bağlanmak için buna ihtiyaç duyar.', en: '`spring-kafka` is added as a normal dependency — it gets bundled into the production jar because the app needs it to connect to a real Kafka.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'spring-kafka-test ise…', en: 'spring-kafka-test is marked…' }, detail: { tr: '`spring-kafka-test` ise `<scope>test</scope>` ile işaretlenir — bu, `@EmbeddedKafka` gibi test-only sınıfların SADECE test derlemesinde bulunacağı anlamına gelir.', en: '`spring-kafka-test` is marked `<scope>test</scope>` — meaning test-only classes like `@EmbeddedKafka` will ONLY exist in the test compilation.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'Bu ayrım olmasaydı…', en: 'Without this separation…' }, detail: { tr: 'Bu ayrım olmasaydı, production jar\'ı gereksiz yere büyür VE test altyapısı (embedded broker kodu) yanlışlıkla canlı ortama sızabilirdi.', en: 'Without this separation, the production jar would grow unnecessarily AND test infrastructure (embedded broker code) could accidentally leak into the live environment.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Java analojisi…', en: 'Java analogy…' }, detail: { tr: 'Java analojisi: JUnit\'in kendisi de her zaman `test` scope\'undadır — `mvn package` production artifact\'ı üretirken test kütüphanelerini otomatik dışarıda bırakır.', en: 'Java analogy: JUnit itself is always in `test` scope too — `mvn package` automatically excludes test libraries when producing the production artifact.' } },
  ],
}
...
const kafkaEcosystemProducerConsumerStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'OrderProducer bir OrderEvent gönderir…', en: 'OrderProducer sends an OrderEvent…' }, detail: { tr: '`OrderProducer` bir `OrderEvent` gönderir, key=orderId — bu, aynı siparişin tüm event\'lerinin sırayla işlenmesini garanti eder.', en: '`OrderProducer` sends an `OrderEvent`, key=orderId — this guarantees all events for the same order are processed in order.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'InventoryConsumer…', en: 'InventoryConsumer listens for this…' }, detail: { tr: '`InventoryConsumer`, "inventory-service" grubunda bu event\'i dinler ve `reserveStock()` çağırır — bu, senkron bir HTTP çağrısı DEĞİL, bağımsız bir tüketimdir.', en: '`InventoryConsumer` listens for this event in the "inventory-service" group and calls `reserveStock()` — this is an independent consumption, NOT a synchronous HTTP call.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'InsufficientStockException fırlatılırsa…', en: 'If InsufficientStockException is thrown…' }, detail: { tr: '`InsufficientStockException` fırlatılırsa, consumer BAŞARISIZLIĞI "orders-failed" topic\'ine bir telafi (compensating) event\'i yazarak ele alır — bu Saga pattern\'idir.', en: 'If `InsufficientStockException` is thrown, the consumer handles the FAILURE by writing a compensating event to the "orders-failed" topic — this is the Saga pattern.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Entegrasyon testi @EmbeddedKafka ile…', en: 'The integration test uses…' }, detail: { tr: 'Entegrasyon testi `@EmbeddedKafka` ile GERÇEK bir mini Kafka broker\'ı bellekte başlatır — mock yerine gerçek produce/consume akışını uçtan uca doğrular.', en: 'The integration test uses `@EmbeddedKafka` to start a REAL miniature Kafka broker in memory — instead of a mock, it verifies the actual produce/consume flow end to end.' } },
  ],
}
...
const kafkaSchemaRegistryComposeStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'schema-registry container\'ı depends_on…', en: 'The schema-registry container starts with…' }, detail: { tr: '`schema-registry` container\'ı `depends_on: kafka` ile başlar — kendi şema geçmişini SAKLAMAK için bile Kafka\'nın kendi topic\'lerinden birini kullanır.', en: 'The `schema-registry` container starts with `depends_on: kafka` — it even uses one of Kafka\'s own topics to STORE its own schema history.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'Registry ayağa kalktıktan sonra…', en: 'Once the registry is up…' }, detail: { tr: 'Registry ayağa kalktıktan sonra, `curl -X POST .../subjects/orders-value/versions` ile bir Avro/JSON şema kayıt altına alınır — bu andan itibaren "orders" topic\'inin sözleşmesi RESMİLEŞİR.', en: 'Once the registry is up, `curl -X POST .../subjects/orders-value/versions` registers an Avro/JSON schema — from this moment on, the "orders" topic\'s contract becomes OFFICIAL.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'Bir producer artık bu kayıtlı şemaya…', en: 'If a producer now tries to send a…' }, detail: { tr: 'Bir producer artık bu kayıtlı şemaya UYMAYAN bir mesaj göndermeye çalışırsa, Registry gönderimi REDDEDER — hatalı veri broker\'a asla ulaşmaz.', en: 'If a producer now tries to send a message that does NOT conform to the registered schema, the Registry REJECTS the send — bad data never reaches the broker.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Her consumer…', en: 'Every consumer queries the same…' }, detail: { tr: 'Her consumer, gelen byte\'ları işlemeden ÖNCE aynı Registry\'ye şemayı sorar — bu, bir alan adı yanlışlıkla değiştirildiğinde 12 downstream consumer\'ın sessizce bozulmasını ÖNLER.', en: 'Every consumer queries the same Registry for the schema BEFORE processing incoming bytes — this PREVENTS 12 downstream consumers from silently breaking when a field name is accidentally changed.' } },
  ],
}
...
const kafkaRealWorldTopicSetupStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: '"orders" ve "payments" topic\'leri 3…', en: '"orders" and "payments" topics are…' }, detail: { tr: '"orders" ve "payments" topic\'leri 3 partition\'la oluşturulur — bu, yüksek hacimli akışların paralel işlenmesine izin verir (3 consumer aynı anda çalışabilir).', en: '"orders" and "payments" topics are created with 3 partitions — this allows high-volume streams to be processed in parallel (3 consumers can run at once).' } },
    { id: 2, icon: '2️⃣', label: { tr: '"orders-failed" ise SADECE 1 partition\'la…', en: '"orders-failed" is created with just 1…' }, detail: { tr: '"orders-failed" ise SADECE 1 partition\'la oluşturulur — hata akışı düşük hacimlidir, paralellik yerine SIRALI işleme (tüm hatalar tek bir yerde, kronolojik) daha değerlidir.', en: '"orders-failed" is created with just 1 partition — the error stream is low-volume, and ORDERED processing (all failures in one place, chronological) matters more than parallelism.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'Her --create komutu --replication-factor 1…', en: 'Every --create command uses…' }, detail: { tr: 'Her `--create` komutu `--replication-factor 1` kullanır — bu sadece LOKAL geliştirme içindir; production\'da bu değer en az 3 olmalıdır (tek broker kaybında veri kaybı olmasın diye).', en: 'Every `--create` command uses `--replication-factor 1` — this is for LOCAL development only; in production this must be at least 3 (so a single broker loss doesn\'t lose data).' } },
    { id: 4, icon: '4️⃣', label: { tr: '--list komutu üç topic\'in de gerçekten…', en: 'The --list command confirms all three…' }, detail: { tr: '`--list` komutu üç topic\'in de gerçekten oluştuğunu doğrular — bir sonraki adımdaki produce/consume simülasyonu bu doğrulamaya bağımlıdır.', en: 'The `--list` command confirms all three topics actually exist — the produce/consume simulation in the next step depends on this verification.' } },
  ],
}
...
const kafkaRealWorldPipelineSimStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'TERMINAL 1\'de "inventory-service" grubu…', en: 'TERMINAL 1 subscribes as the…' }, detail: { tr: 'TERMINAL 1\'de "inventory-service" grubu, TERMINAL 2\'de "payment-service" grubu AYNI "orders" topic\'ine abone olur — iki farklı group.id, iki tamamen ayrı okuma konumu demektir.', en: 'TERMINAL 1 subscribes as the "inventory-service" group, TERMINAL 2 as the "payment-service" group — both to the SAME "orders" topic — two different group.ids mean two completely separate read positions.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'TERMINAL 3\'ten üretilen HER mesaj…', en: 'EVERY message produced from TERMINAL 3…' }, detail: { tr: 'TERMINAL 3\'ten üretilen HER mesaj, her iki terminale de ANINDA görünür — çünkü mesaj bir kuyruktan silinmiyor, log\'a ekleniyor ve her grup kendi offset\'ini bağımsız ilerletiyor.', en: 'EVERY message produced from TERMINAL 3 appears in BOTH terminals INSTANTLY — because the message isn\'t removed from a queue, it\'s appended to the log, and each group advances its own offset independently.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'Bu, geleneksel bir kuyruktan (RabbitMQ)…', en: 'This is the FUNDAMENTAL difference from…' }, detail: { tr: 'Bu, geleneksel bir kuyruktan (RabbitMQ) TEMEL farktır: orada bir mesaj bir worker tarafından alınırsa diğeri onu ASLA göremez.', en: 'This is the FUNDAMENTAL difference from a traditional queue (RabbitMQ): there, once one worker takes a message, the other NEVER sees it.' } },
    { id: 4, icon: '4️⃣', label: { tr: '--describe --group inventory-service…', en: 'The "LAG 0" line in the --describe…' }, detail: { tr: '`--describe --group inventory-service` çıktısındaki "LAG 0" satırı, o grubun ürettiği HER mesajı işlediğinin kanıtıdır — LAG\'in ne anlama geldiği bir sonraki adımda derinleşir.', en: 'The "LAG 0" line in the `--describe --group inventory-service` output proves that group has processed EVERY message produced — what LAG means in depth is covered in the next step.' } },
  ],
}
        ...
        blocks: [
          {
            type: 'simple-box',
            emoji: '📰',
            content: 'Kafka acts as a permanent, replayable logbook for every event in your system — like a ship\'s voyage log that every department reads independently: navigation and cargo read the same entries, each at their own pace, and neither can overwrite the other\'s progress marker. But if a database already records everything and RabbitMQ already delivers messages, why add Kafka at all? Because a database overwrites the old value on update (history lost), and RabbitMQ deletes a message the moment one consumer reads it — no second team ever sees it. In Java terms, think of a traditional queue as a `LinkedList` where `poll()` removes the head permanently; Kafka is more like an `ArrayList` with a per-consumer index: every consumer group has its own offset and can re-read the same events independently, even hours later. For QA engineers, this changes the game: when a payment event disappears silently in production, you can replay the Kafka topic from the exact offset where the consumer crashed and reproduce the exact state — something impossible with a classic queue where the message is already gone.',
          },
          {
            type: 'text',
    ...
    void whenOrderSent_thenConsumerReceivesIt() throws Exception {
        ...
        assertThat(records.iterator().next().value().getOrderId()).isEqualTo(1L);
          ...
          { type: 'heading', text: 'Kafka + Schema Registry: Type Safety' },
          {
            type: 'simple-box',
            emoji: '📋',
            content: 'Schema Registry is to Kafka messages what a Java interface is to service contracts: the interface declares the exact method signatures both caller and implementor must honor, and the compiler rejects any code that violates the contract at build time. Schema Registry does the same at the data layer — every Avro or Protobuf schema is versioned and published before any producer can send a message, and every consumer validates incoming bytes against the registered schema before processing. But here is the question that reveals whether you truly understand the problem: if the teams agree verbally on the event format, why do you need a technical registry at all? Because verbal agreements erode instantly — one developer renames a field from `orderId` to `order_id` in a microservice refactor, the CI pipeline for that service goes green, and 12 downstream consumers silently start receiving null values for a field that no longer exists. In Java terms, think of removing a public method from a widely-used interface: the compiler catches direct callers immediately, but Kafka consumers are runtime callers, not compile-time callers — without Schema Registry enforcing backward/forward compatibility, the breakage is invisible until production traffic starts flowing. For QA, the operational discipline is this: any Avro or Protobuf schema change must go through Schema Registry compatibility checks before deployment; a test that sends raw JSON to a topic and skips schema validation is giving false confidence about production behavior.',
          },
          {
            type: 'code',
            language: 'yaml',
...
#   -d '{"schema": "{\"type\":\"record\",\"name\":\"Order\",\"fields\":[{\"name\":\"orderId\",\"type\":\"long\"},{\"name\":\"product\",\"type\":\"string\"}]}"}'`,
          ...
          { type: 'heading', text: 'Debezium: Change Data Capture (CDC)' },
          {
            type: 'simple-box',
            emoji: '🔄',
            content: 'Debezium works like a change auditor who sits directly inside the database\'s internal transaction log and publishes a Kafka event the instant any row is inserted, updated, or deleted — without touching the application code or adding a single trigger to the database. The crucial distinction to understand before writing integration tests: Debezium reads the database\'s binary log (binlog for MySQL, WAL for PostgreSQL), not the application\'s API. That means it captures changes even when a DBA runs a direct SQL update, a migration script backfills a column, or a batch job deletes records at midnight — changes that bypass the application entirely and therefore bypass any API-level Kafka producer. But if your application already publishes Kafka events when it saves data, why add Debezium on top? Because application-level events and database-level events are rarely identical: the application might publish an `OrderCreated` event but miss the database trigger that a fraud-detection rule automatically updated the `status` column to `BLOCKED` 200ms later. In Java terms, Debezium is the database equivalent of AOP around every `@Repository.save()` call, but at the binary log level where no application code can suppress it. For QA, this means that end-to-end tests that verify downstream Elasticsearch indexes or Kafka-driven read models must account for Debezium connector lag — if the connector is a few seconds behind, assertions immediately after a write will return stale data, producing flaky tests that pass on a fast machine and fail on a slow CI runner.',
          },
          {
            type: 'code',
            language: 'json',
...
# Lag = -:    Consumer group has no members (app not running)`,
          ...
          { type: 'heading', text: 'Dead Letter Topic (DLT): Handling Poison Messages' },
          {
            type: 'simple-box',
            emoji: '☠️',
            content: 'A Dead Letter Topic (DLT) is Kafka\'s equivalent of a `catch` block with a quarantine zone: instead of letting a malformed message crash the consumer in an infinite retry loop, you route it to a separate DLT topic after a configurable number of attempts, where it can be inspected, replayed, or manually remediated without blocking the main processing flow. The mechanism to understand before moving on: Kafka\'s offset commit model means a consumer that crashes on message N will re-read message N on every restart — unlike a traditional queue where the message is returned to the back of the line, Kafka keeps serving the same message from the same position until the consumer explicitly commits past it. Without a DLT, one malformed event (a JSON field with the wrong type, a null required field) can halt an entire consumer group forever, stopping all downstream processing for every subsequent event in that partition. In Java terms, the DLT pattern is structurally identical to a `try { process(message) } catch (Exception e) { dltTemplate.send(DLT_TOPIC, message, e.getMessage()) }` — except that Spring Kafka\'s `@RetryableTopic` annotation configures the entire retry-and-route lifecycle declaratively, including exponential backoff between retries. For QA, the DLT is both a reliability mechanism and a diagnostic tool: in an integration test, asserting that a malformed event lands on the DLT topic within a timeout is the correct way to verify that your error-handling contract is working — and any consumer that has been running for more than a week with zero DLT messages is either processing perfectly or not handling errors at all.',
          },
          {
            type: 'code',
            language: 'java',
        ...
        blocks: [
          {
            type: 'simple-box',
            emoji: '📰',
            content: 'Kafka, sisteminizdeki her event için kalıcı ve yeniden oynatılabilir bir seyir defteri gibi çalışır — tıpkı bir geminin sefer günlüğünün her departman tarafından bağımsız olarak okunabilmesi gibi: navigasyon ve kargo aynı girişleri okur, her biri kendi hızında ilerler, hiçbiri diğerinin ilerleme işaretini silemez. Peki veritabanı zaten her şeyi kaydediyorken ve RabbitMQ mesajları zaten iletiyorken neden Kafka\'ya ihtiyaç var? Çünkü veritabanı bir kaydı güncellediğinde eski değeri siler (tarih kaybolur), RabbitMQ ise mesajı tüketen ilk consumer\'a verir ve siler — ikinci ekip aynı mesajı hiç göremez. Java\'da bir `LinkedList`te `poll()` başı kalıcı olarak kaldırır; Kafka ise consumer başına ayrı offset tutan bir `ArrayList` gibidir: her consumer group aynı event\'i bağımsız olarak okuyabilir, saatler sonra bile. QA mühendisleri için bu kritiktir: production\'da bir ödeme eventi kaybolduğunda, consumer\'ın çöktüğü tam offset\'ten topic\'i yeniden oynatabilir ve tam durumu yeniden üretebilirsin — mesajın zaten silindiği klasik bir kuyrukta bu imkânsızdır.',
          },
          {
            type: 'text',
        ...
        blocks: [
          {
            type: 'simple-box',
            emoji: '🛠️',
            content: 'Geliştirme ortamı için Kafka kurmak, laboratuvar için özel bir güç trafosu çekmeye benzer: ulusal şebekeyi yeniden kablolamamak — tam olarak ihtiyaç duyduğunuz voltajı sağlayan, oturum bitince kapatılabilen kendi kendine yeterli bir trafo kutusu çalıştırmak. Tek bir `docker-compose up` bunu yapar: ZooKeeper (koordinasyon kaydı) ve Kafka broker\'ı birlikte başlatır, portlar ve ağ bağlantıları önceden ayarlanmış olarak, 30 saniyeden kısa sürede. Peki geliştirme ortamında zaten yerel bir veritabanı varken neden ayrıca çalışan bir Kafka cluster\'ına ihtiyaç duyarsınız? Çünkü uygulamanız sipariş oluşturma, envanter güncellemesi veya ödeme onayı için Kafka event\'leri yayınlıyorsa, hiçbir mock veri yalnızca bir partition\'ın yavaş takip ettiğinde ortaya çıkan race condition\'ı ortaya çıkaramaz. Java\'da yerel Kafka\'yı Docker Compose ile başlatmak, birim testleri için H2 in-memory veritabanı çalıştırmakla yapısal olarak aynıdır — hafif, izole, tek kullanımlık. QA ekipleri için operasyonel kural açıktır: Kafka cluster\'ına bağlı entegrasyon testleri ancak o cluster tek bir komutla başlatılabiliyorsa commit\'lenebilir; test ortamı manuel Kafka kurulumu gerektiriyorsa test CI\'da atlanır ve hatalar production\'a ulaşır.',
          },
        ...
        blocks: [
          {
            type: 'simple-box',
            emoji: '🏗️',
            content: 'Kafka mimarisi, birden fazla şubesi olan dağıtık bir kütüphane sistemi gibi çalışır: her şube (broker) belirli bölümlerin (partition) bir kopyasını tutar, her bölümün numaralı rafları (offset) vardır ve her okuma grubunun şu anda hangi rafta olduğunu takip eden bir kütüphaneci (consumer offset) bulunur. Devam etmeden önce üzerinde durulması gereken kritik soru: neden tek bir büyük depolama alanı yerine partition\'lara bölünmüş bir topic? Çünkü tek bir partition, tek bir sıralı yazma şerididir — yüksek hacimli bir topic\'i 12 partition\'a bölmek, 12 consumer\'ın birbirini hiç bloklamadan paralel okuyabilmesi demektir, bu da throughput\'u partition sayısıyla doğrusal olarak ölçekler. Java açısından, kilit çakışmasını önlemek için anahtarları bucket\'lara dağıtan bir ConcurrentHashMap\'i düşünün: Kafka da aynı şekilde event\'leri consumer bottleneck\'lerini önlemek için partition\'lara dağıtır. QA mühendisleri için bu mimari kararın doğrudan bir test sonucu vardır: test consumer\'ınız her zaman partition 0\'a kendini atıyorsa, partition 7\'ye inen event\'leri kaçırır — partition key hash\'lemesinin event\'leri eşit olmayan şekilde dağıttığı gerçek production yükü altında görünen sessiz, yeniden üretilmesi güç bir boşluk.',
          },
          {
            type: 'diagram-svg',
...
consumer.commitSync(); // Offset'i ilerlet: 42 → 43`,
          ...
          { type: 'heading', text: 'Producer — Mesaj Yazma' },
          {
            type: 'simple-box',
            emoji: '📤',
            content: 'Kafka Producer, bir kurye deposundaki paket yönlendirme sistemi gibi çalışır: gönderici (producer) bir hedef etiketi (mesaj key\'i) yapıştırır, paketi intake penceresine (broker) bırakır ve deponun yönlendirme algoritması hangi konveyör bandına (partition) gideceğine karar verir — aynı etiket her zaman aynı banda iner, böylece aynı müşteriye ait tüm paketlerin sırayla gelmesi garanti edilir. Peki producer\'ın key\'i neden var — bir topic\'e yayınlayıp geçemez misiniz? Çünkü Kafka\'nın sıralama garantisi topic bazında değil, partition bazındadır: key olmadan, event\'ler partition\'lar arasında round-robin dağılır ve sırasız gelebilir. Java açısından, mesaj key\'i bir HashMap\'teki hash key\'iyle yapısal olarak aynıdır: `hash(key) % partitionSayisi` bucket\'ı belirler ve aynı key her zaman aynı bucket\'a eşlenir. QA açısından, sessiz hataların doğduğu yer burasıdır: testiniz aynı order ID için bir order-created event\'i ve bir payment-captured event\'i aynı key olmadan yayınlarsa, farklı partition\'lara düşebilir ve ters sırayla consume edilebilir — böylece sipariş mevcut olmadan önce ödeme gelir; bu yalnızca partition key hash\'lemesinin event\'leri düzensiz dağıttığı yük altında görünen ve CI loglarında her zaman timing problemi gibi görünen bir race condition\'dır.',
          },
          {
            type: 'code',
            language: 'java',
        ...
        blocks: [
          {
            type: 'simple-box',
            emoji: '☕',
            content: 'Spring Kafka, ham Apache Kafka client\'ını Spring Data JPA\'nın JDBC\'yi sardığı gibi sarar: connection açmayı, transaction yönetimini ve sonuç setlerini elle eşleştirmeyi manuel yapmak yerine bir metoda anotasyon eklersiniz ve Spring tüm lifecycle\'ı kablolar. `@KafkaListener(topics = "orders")` ile otomatik deserializasyon, consumer group yönetimi, hata işleme ve offset commit\'i elde edersiniz — eskiden 40 satır olan `KafkaConsumer.poll()` boilerplate kodu 2 satıra iner. Peki Spring bunu bu kadar temiz sardığı hâlde ham Kafka client API\'sini neden öğrenmeye değer? Çünkü `@KafkaListener`\'ınız yük altında sessizce mesaj düşürmeye başladığında veya CI\'da her 30 saniyede bir rebalance yaptığında, teşhis etmenin tek yolu anotasyonun gerçekte ne yaptığını anlamaktır — `max.poll.records`, `session.timeout.ms`, `enable.auto.commit` Spring\'in kendi özelliklerine eşleştirdiği ham client config\'leridir ve birini yanlış ayarlamak dashboard\'da sağlıklı görünen ama hiç offset commit etmeyen bir consumer üretir. Java açısından, ilişki Spring Data JPA ile ham JDBC arasındaki ilişkiyle birebir aynıdır: üretkenlik için soyutlamayı kullanırsınız ama soyutlamanın sızdığı %5\'lik durumları debug etmek için altta yatan driver\'ı anlamanız gerekir. QA için bu şu anlama gelir: bir `@KafkaListener` bileşenine karşı yapılan herhangi bir entegrasyon testi, işlemden sonra offset\'lerin gerçekten commit edildiğini de doğrulamalıdır — işleyen ama commit etmeyen bir consumer, bir sonraki yeniden başlatmada tetiklenmeyi bekleyen bir yeniden işleme bombasıdır.',
          },
          {
            type: 'code',
            language: 'yaml',
        ...
        blocks: [
          {
            type: 'simple-box',
            emoji: '🔗',
            content: 'Kafka, dağıtık bir sistemde her gönderenin paketini bıraktığı ve her alıcının paketini aldığı merkezi bir postane gibi dağıtılmış sistemdeki konumunu işgal eder: hiçbir gönderici alıcının ev adresini bilmek zorunda değildir — postane yönlendirmeyi, depolamayı ve teslimat garantilerini yönetir. Bu karşılaştırmayı yapısal olarak doğru kılan şey, Kafka\'nın her servis çifti arasında oturmasıdır: microservice\'ler birbirini doğrudan çağırmak yerine event\'leri Kafka\'ya gönderir, bu da yeni bir consumer eklemek için producer\'ı hiç değiştirmemeniz anlamına gelir — sadece topic\'e abone olun. Peki bir dağıtık log üzerine neden tüm bir ekosistem (Kafka Streams, Schema Registry, Debezium, Strimzi) inşa edilir? Çünkü her entegrasyon farklı bir güvenilirlik sorununu çözer: Schema Registry bozulan API değişikliklerinin consumer\'ları çökertmesini engeller, Debezium yük altında değişiklikleri kaçıran polling döngüsünü ortadan kaldırır, Kafka Streams harici bir veritabanı olmadan durumsal işlemeyi mümkün kılar ve Strimzi cluster yönetimini Kubernetes\'te bildirimsel hale getirir. Java açısından, bu ekosistem Spring framework\'ün kendisini yansıtır: core (Kafka) temel konuları yönetir, modüller (Streams, Connect) domain\'e özgü güç ekler ve her modül bağımsız olarak benimsenebilir. QA için ekosistemi anlamak operasyonel önem taşır: bir Elasticsearch indeksi veritabanıyla senkrondan çıktığında, kök neden neredeyse her zaman bir Debezium connector gecikmesi veya Schema Registry uyumsuzluğudur.',
          },
    ...
    void siparisSonrasindaStokConsumerMesajAlir() throws Exception {
        ...
        assertThat(records.iterator().next().value().getSiparisId()).isEqualTo(1L);
          ...
          { type: 'heading', text: 'Schema Registry: Tip Güvenliği' },
          {
            type: 'simple-box',
            emoji: '📋',
            content: 'Schema Registry, dağıtık bir ekipteki her yazılım geliştiricinin API değişikliklerini tek bir paylaşılan OpenAPI dokümanına kaydetmesi gerektiği bir sisteme benzer — kimse diğer ekiplere haber vermeden alanları yeniden adlandıramaz veya kaldıramaz, aksi takdirde build kırılır. Avro veya Protobuf\'un Schema Registry olmadan neden yetmediğini sormak yerinde olur: serialization formatı, şemanın versiyonlanması ve uyumluluk yönetimi olmadan hâlâ kırılgan kalır — üretici yeni bir alanı zorunlu yapabilir ve tüm consumer\'ları sessizce çökertebilir. Java açısından, Schema Registry Kafka entegrasyonuna Hibernate Validator\'ın REST API\'ye eklediği şeyi katar: bir istemci geçersiz bir şema yayımlamaya çalıştığında, sunucu bunu reddetmeden önce yapılandırılmış kurallara göre doğrular. Bu yapı QA mühendisliğinde kritik önem taşır: Avro + Schema Registry olmadan bir Kafka pipeline\'da "şema drift" tespiti mümkün değildir — bir producer alanı yeniden adlandırdığında consumer\'lar boş değerler veya exception\'larla karşılaşır ve hatanın kökeni log\'da gizlenir; Schema Registry varsa bu değişiklik deploy anında reddedilir ve asla üretime ulaşmaz.',
          },
          { type: 'heading', text: 'Debezium: Change Data Capture (CDC)' },
          {
            type: 'simple-box',
...
# siparisler  0        2               2               0  ← lag 0 = sağlıklı`,
          ...
          { type: 'heading', text: 'Dead Letter Topic (DLT): Zehirli Mesajları Yönet' },
          {
            type: 'simple-box',
            emoji: '☠️',
            content: 'Dead Letter Topic, bozuk bir ürünün fabrika hattını tamamen durdurmak yerine ayrı bir karantina kutusuna yönlendirilmesi gibi çalışır: bant durmaz, diğer ürünler işlenmeye devam eder ve karantina kutusundaki bozuk ürünler sonradan incelenip yeniden işlenir. DLT olmadan neden consumer\'ın sonsuza kadar takılacağını sormak gerekir: Kafka offset-tabanlı çalıştığından, consumer başarıyla commit edemediği bir mesajı tekrar tekrar işlemeye çalışır — partition tüm consumer group için durmuş olur ve bu "zehirli mesaj" sorunu üretim ortamlarında saatlerce tespit edilemeyen bir kesintiye dönüşebilir. Java açısından DLT, Spring\'in `@ExceptionHandler` zinciriyle birlikte çalışan bir circuit breaker gibidir: belirli sayıda `@RetryableTopic` denemesinden sonra mesaj DLT topic\'ine yönlendirilir ve ayrı bir `@DltHandler` metodu devralır — `try-catch` bloğunun `finally` yerine `catch` dalında farklı bir iş akışı başlatmasına benzer. QA için DLT kritik bir test senaryosudur: bozuk bir mesaj production\'a girerse retry döngüsü bir saat içinde yüzlerce hata log\'u üretebilir; DLT varlığının test edilmesi bu tür sessiz hizmet kesintilerini önleyen kaliteli sistem testinin merkezindedir.',
          },
          {
            type: 'code',
            language: 'java',
