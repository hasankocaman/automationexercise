  ...
  en: {
    ...
    sections: [
      // ── 0. INTRODUCTION ──────────────────────────────────────────────────
      {
        ...
        blocks: [
          { type: 'simple-box', emoji: '⚡', content: "Imagine a highway stress-test: a city engineer doesn't wait for rush hour to discover the bridge collapses under 10,000 cars — they build a simulation first, measure the breaking point in a controlled environment, and reinforce before the real load ever arrives. JMeter does exactly this for your server: it sends thousands of virtual HTTP requests in parallel so you can measure latency, throughput, and failure rates long before real users are affected. But here's the question worth sitting with: if you already have Selenium and pytest catching functional bugs, why do you also need a separate load testing tool? Because those tools only ever run with a single simulated user — they will never reveal that a database connection pool silently exhausts under 500 concurrent logins, or that an innocuous product-price query becomes an N+1 bottleneck that collapses at scale. In Java terms, you already know ExecutorService and CountDownLatch for multi-thread orchestration — JMeter gives you the same parallel-execution power as a zero-code GUI, where Thread Groups replace Runnable classes and Listeners replace your custom metrics collectors. The QA business truth: every flash sale, product launch, or marketing blast that hits production without a prior load test is a gamble where the stakes are downtime, lost revenue, and permanently damaged user trust — and those failures always happen at the worst possible moment." },
    ...
    sections: [
      {
        ...
        blocks: [
          { type: 'simple-box', emoji: '⚡', content: "Bir köprü mühendisini düşün: köprünün 10.000 araç altında çöküp çökmeyeceğini anlamak için gerçek trafik yoğunluğunu beklemiyor — kontrollü bir ortamda simülasyon yapıyor, kırılma noktasını ölçüyor ve gerçek yük gelmeden önce takviye ediyor. JMeter, sunucun için tam olarak bunu yapar: binlerce sanal HTTP isteğini paralel göndererek latency, throughput ve hata oranlarını gerçek kullanıcılar etkilenmeden ölçer. Ama şu soruyu kafana tak: Selenium ve pytest zaten fonksiyonel hataları yakalıyorsa, neden ayrıca bir yük testi aracına ihtiyaç var? Çünkü bu araçlar her zaman yalnızca tek bir sanal kullanıcıyla çalışır — 500 eş zamanlı kullanıcı altında database connection pool'un sessizce tükendiğini ya da ürün fiyat sorgusunun ölçekte N+1 felakete dönüştüğünü asla ortaya çıkaramazlar. Java açısından bakarsak: ExecutorService ve CountDownLatch ile çok iş parçacıklı orkestrasyon zaten bildiğin bir şey — JMeter, Thread Group'larla aynı paralel yürütme gücünü sıfır kod yazarak, grafik arayüzle sağlar; Runnable sınıflarının yerini Thread Group, özel metrik toplayıcıların yerini Listener alır. QA'nın iş gerçeği şu: öncesinde yük testi yapılmadan production'a çıkan her flash sale, ürün lansmanı veya pazarlama patlaması, kesinti ve kalıcı kullanıcı güveni kaybı riskiyle oynanan bir kumardan başka bir şey değildir." },
      ...
      },
      {
        ...
        blocks: [
          { type: 'simple-box', emoji: '📦', content: "JMeter kurulumu, taşınabilir bir Java CLI aracı kurmanın yapısal olarak aynısıdır — Apache Maven binary zip'ini indirip çıkarmak, bin/ dizinini PATH'e eklemek ve ardından terminalden çağırmak gibi düşün. JMeter da tamamen aynı şekilde çalışır: kurulum sihirbazı yok, sistem düzeyinde değişiklik yok, sadece açtığın bir dizin ve çağırdığın bir shell script. Peki neden JMeter kendi başına çalışan bir araç yerine Java'yı önkoşul olarak istiyor? Çünkü JMeter'ın kendisi bir Java uygulamasıdır — her Thread Group gerçek JVM thread'leri oluşturur, her sampler kaputun altında Java'nın HttpClient veya NIO kütüphanelerini kullanır ve tüm plugin ekosistemi Java bytecode üzerine inşa edilmiştir. Yani Selenium veya REST Assured testlerin için zaten bir JDK kuruluysa, ek setup maliyetin sıfırdır. Bunu k6 ile karşılaştır (Go runtime gerektirir) ya da Locust ile (Python + pip bağımlılıkları gerektirir) — JMeter'ın Java temeli, Java odaklı QA takımları için doğrudan bir avantajdır. QA operasyon gerçeği: yanlış yapılandırılmış veya hatalı versiyondaki bir JDK, JMeter'ın test çalıştırmanın başında sessizce başarısız olmasına ya da tahmin edilemez davranmasına neden olur — her zaman staging ortamına yönlendirmeden önce java -version komutunu doğrula, çünkü başlayamayan bir araç, hiç araç olmamaktan daha tehlikelidir." },
...
echo $JAVA_HOME`
      ...
      },
      {
        ...
        blocks: [
          { type: 'simple-box', emoji: '📚', content: "Bir JMeter test planını katı bir üç-rol hiyerarşisine sahip tiyatro yapımı gibi düşün. Thread Group, cast direktörüdür — sahneye kaç aktörün (sanal kullanıcı) çıkacağını ve ne hızla yürüyerek gireceğini (ramp-up) tam olarak o belirler. Her Sampler, aktörlerin izlediği senaryo satırıdır: /api/login'e bir HTTP isteği, /api/checkout'a bir POST, her çağrıda tek bir belirli eylem. Listener ise arkadaki kamera ekibidir; performansın kendisini hiç bozmadan her yanıt süresini, her durum kodunu ve her aktarılan byte'ı yakalar. Ama şu daha derin soruyu düşün: Thread Group 500 kullanıcı içeriyorsa ve Sampler tek bir endpoint'e gidiyorsa, bileşenlerin ağaçtaki sırası neden gerçekten önemlidir? Çünkü JMeter ağacı yukarıdan aşağıya katı bir ebeveyn-çocuk ilişkisiyle değerlendirir — Thread Group'un içine yerleştirilen bir Config Element yalnızca o grubun Sampler'larına uygulanırken, aynı eleman Test Plan kökünde küresel olarak geçerlidir. Java'daki değişken kapsam kurallarıyla birebir aynıdır: bir method bloğu içinde tanımlanan alan ile sınıf düzeyinde tanımlanan alan. Bu durumun yarattığı QA riski şudur: bir geliştirici CSV Data Set Config'i yanlış ağaç düzeyine ekler, her sanal kullanıcı farklı satır yerine aynı kimlik bilgileri satırını okur ve 500 kullanıcılık testin aslında tek bir paylaşımlı oturumla sunucuya çarpmaya devam eder — production'da patlayacak olan satır kilitleme ve oturum yarışması hatalarını tamamen gizler." },
...
// - En iyisi: test sonrası üret`,
      ...
      },
      {
        ...
        blocks: [
          { type: 'simple-box', emoji: '🚀', content: "JMeter'ın GUI'sinde yük testi çalıştırmak, aynı anda VisualVM ekleyerek, heap dump kaydederek ve 30 tarayıcı sekmesini açık tutarak bir JVM üzerinde performans kıyaslaması yapmak gibidir — ölçüm aracının kendisi en büyük gürültü kaynağına dönüşür. Non-GUI modu, tüm Swing render katmanını soyar ve mevcut tüm thread'leri ile belleği yük üretmeye adar; işte bu yüzden production kalitesindeki testler her zaman komut satırından headless çalışmalıdır. Peki neden birden fazla makineye dağıtılmış test yapıya daha da ileriye gidilir? Çünkü tek bir JMeter controller node, 4 çekirdekli bir makinede yaklaşık 300–500 aktif thread'e kadar güvenilir biçimde hizmet verebilir — bunun ötesinde kendi scheduler'ı zamanlama tutarsızlıkları yaratmaya başlar. Hedef 5.000 eş zamanlı kullanıcıysa, her biri toplam yükün bir bölümünü üreten ve tek bir controller tarafından koordine edilen birden fazla agent makinesine ihtiyaç vardır. Java concurrency terimleriyle düşününce: tek bir ThreadPoolExecutor'ı, worker'ların farklı fiziksel sunucularda yaşadığı bir fork/join havuzuyla değiştirmek gibidir. Junior ve senior mühendisleri birbirinden ayıran ileri seviye QA becerisi ise şudur: Regular Expression Extractor veya JSON Extractor ile dinamik veri çıkarma — bu araçlar CSRF token, session ID, OAuth kodu gibi değerleri bir yanıttan yakalayıp bir sonraki isteğe otomatik olarak enjekte eder. Bu olmadan 500 kullanıcıyı doğrulayan bir test, her sonraki çağrı eski veya boş bir auth token taşıdığı için ilk istekten sonra sessizce başarısız olur." },
...
HTML Dashboard (-e -o)
  ...
  → Çalışma zamanı overhead yok`,
      ...
      // ── 4. GERÇEK HAYAT ───────────────────────────────────────────────────
      {
        ...
        blocks: [
          { type: 'simple-box', emoji: '🛠️', content: "Bir Formula 1 takımının yarış günü öncesinde arabayı nasıl doğruladığını düşün: 100.000 kişinin önünde start grid'inde ilk kez tam gaz sürmüyorlar — özel bir pistte kontrollü bir stres protokolünden geçiriyorlar, her sensörü ölçümlüyorlar ve yük altında aşınan veya arıza veren parçaları orada düzeltiyorlar. Satış öncesi bir JMeter yük testi, senin backend'in için işte o özel pist seansıdır. Bir kampanya veya lansman öncesinde kendin sorman gereken kritik soru şudur: Selenium testleri zaten yeşilse ve unit testler geçiyorsa, sistem doğrulanmış değil mi? Hayır — çünkü fonksiyonel testler (Selenium, pytest, JUnit) tam olarak bir sanal kullanıcıyla çalışır; yani tek bir istek için 200ms yanıt veren bir Spring Boot uygulaması, database connection pool tükenmesi, eş zamanlılık altında katmerlenen N+1 sorguları veya yanlış yapılandırılmış @Async executor'da thread starvation nedeniyle 500 eş zamanlı kullanıcı için 8 saniyelik timeout sergileyebilir. HikariCP havuz boyutlandırmasına ve Spring'in varsayılan thread modeline aşina Java geliştiricileri burada doğrudan bir avantaja sahiptir: JMeter'ın Aggregate Report'u darboğazın havuz sınırında mı, sorgunun kendisinde mi, yoksa uygulama katmanında mı olduğunu direkt ortaya koyar. QA'nın iş gerçeği: staging yük testinde bir checkout darboğazını keşfetmenin maliyeti sıfır kesinti ve bir konfigürasyon değişikliğidir; bunu Black Friday flash sale sırasında keşfetmenin maliyeti ise dakika başına binlerle ölçülen gelir kaybı, kamuoyuna açık bir incident raporu ve geri dönülmesi zor marka hasarıdır." },
...
#    altında böyle görünür.`
      ...
      // ── 5. EKOSİSTEM ──────────────────────────────────────────────────────
      {
        title: '🔗 Ekosistem',
        blocks: [
          { type: 'simple-box', emoji: '🔗', content: "JMeter'ı bir bilimsel ölçüm sistemindeki sensör modülü gibi düşün — tek bir şeyde son derece iyidir: yük üretmek ve ham yanıt verisi kaydetmek. Ama tezgahın üzerinde tek başına oturan bir sensör, onu belirli bir zaman çizelgesinde tetikleyen, okumalarını bir zaman serisi veri tabanında depolayan ve üzerinde harekete geçilebilecek trend çizgileri görüntüleyen bir sisteme bağlanmadıkça sana hiçbir işe yarayan bilgi vermez. İşte bu çevreleyen sistemi tam olarak Jenkins, Docker, InfluxDB ve Grafana sağlar. QA mühendislerinin düşüncesindeki yaygın bir boşluğu ortaya çıkaran soru şudur: JMeter zaten her çalıştırmanın ardından HTML raporu üretiyorsa, neden üstüne bir de InfluxDB ve Grafana'ya ihtiyaç var? Çünkü tek bir çalıştırmanın HTML raporu bir fotoğraftır — sistemin bugün nasıl performans gösterdiğini söyler. InfluxDB + Grafana ise bir video verir — P99 latency'nin son 30 deployment boyunca sessizce bozulup bozulmadığını, kullanıcıların fark edeceği veya herhangi bir raporun işaretleyeceği eşiği çok önce geçip geçmediğini gösterir. Java ekosistemi terimleriyle bu durum, bir System.out.println(result) debug ifadesi ile doğru yapılandırılmış bir Micrometer + Prometheus + Grafana gözlemlenebilirlik yığını arasındaki farkı yansıtır — biri tek bir soruyu bir kez yanıtlar, diğeri soru sorulması gerekmeden önce uyarır. QA operasyon sonucu: JMeter'ı yalnızca büyük sürümler öncesinde manuel olarak çalıştıran bir ekip, büyük sürümün planlanmadığı yılın 51 haftasında kör uçar; sessizce birikim yapan performans regresyonları yalnızca son teslim tarihi öncesinde düzeltmek için çok geç olduğunda yüzeye çıkar." },
...
# "sende hangi JMeter sürümü var?" hata ayıklaması sıfır.`
      ...
      // ── 6. INTERVIEW Q&A ─────────────────────────────────────────────────
      {
        ...
        blocks: [
          { type: 'simple-box', emoji: '💼', content: "Bir JMeter mülakatı, kelime bilgisi sınavı değil — teşhis simülasyonudur. Mülakat yapan kişi, gerçek bir yük testinden gelen sayılara bakıp sistemin içinde ne olduğu hakkında akıl yürütüp yürütemediğini anlamaya çalışıyor. Kan tahlili okuyan bir doktoru düşün: yalnızca 'bu değer yüksek' diyebilen doktor, 'bu P99 değeri, o hata oranı paterniyle birleşince bir kaynak tükenme olayına işaret ediyor, yavaş bir algoritmaya değil — işte nasıl doğrularım ve düzeltirim' diyebilen doktordan daha az işe yarar. Adayların çoğunu şaşırtan soru şudur: Aggregate Report ortalama yanıt süresi 300ms, ama P99 12.000ms gösteriyorsa, sistem iyi performans mı gösteriyor, kötü mü? Tanımları ezberleyen adaylar 'ortalama iyi' der. Kıdemli mühendisler şunu söyler: '12 saniyelik P99, 100 istekten birinin takıldığı anlamına geliyor — bu, ortalamanın tamamen maskelediği bir connection pool timeout'u, GC pause'u ya da lock contention paternidir; ve 1.000 kullanıcılı bir sistemde bu, saniyede 10 kişinin 12 saniyelik donma yaşaması demektir.' Java terimleriyle bu, HashMap.get()'in amortize O(1) olduğunu bilmek ile hash çarpışma fırtınaları altında O(n)'e bozunduğunu bilmek arasındaki farka eşdeğerdir — ortalama durum istatistiği patolojik durumu tamamen gizler. QA kariyer gerçeği: JMeter metriklerini mimari kök nedenlere çevirebilen ve somut düzeltmeler önerebilen mühendisler (HikariCP havuz boyutu, GC ayarı, indeks optimizasyonu, circuit breaker eşikleri), QA otomasyon mühendisliğinden performans mühendisliği liderliğine yükselen mühendislerdir." },
      ...
      "explanation": "Ortalama değer (mean), sistemin genel durumu hakkında yanıltıcı olabilir. P95'in ortalamadan çok daha yüksek çıkması, nadir görülen ancak ciddi olan performans sorunlarını veya kuyruk birikimlerini işaret eder ve mutlaka analiz edilmelidir."
          ...
          {
            type: 'interview-questions',
              relatedTopicId: 'jmeter-interview-tr',
            topic: 'JMeter',
            questions: [
              // ── TEMEL ──────────────────────────────────────────
