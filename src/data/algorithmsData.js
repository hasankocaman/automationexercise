export const algorithmsData = {
        ...
        sections: [
            {
                ...
                accent: "violet",
                tag: "Problem solving",
                analogy: "Bir yemek tarifi gibi dusun — ama mekanizma detayi kritik: tarif 'malzemeleri karıstır ve pisir' diyemez, 'yumurtayi sekeriyle 3 dakika kup kup cirp, ardından 180°C firinda 35 dakika pisir' demek zorundadir; aksi hâlde her seferinde farkli bir kek cikar. QA'da da 'checkout'u test et' demek bu kadar belirsizdir — 'giris yapmamis kullanici, suresi dolmus kuponu olan urunle odeme denemeli; sistem 400 hatasi dondurmeli ve kullanici bilgilendirilmeli' gibi net, sirali, edge case'leri belli adimlar olmalidir. Peki deneyimli bir QA muhendisi adimlari zaten biliyor — neden bu kadar detayli yazmak zorunda? Cünkü yarin o olmayacak ve tarif yoksa ayni test iki kisinin elinde iki farkli sonuc üretir ya da flaky görünür.",
    ...
    return BugDecision.ADD_TO_BACKLOG;                 // Geri kalanlar backlog'a eklenir
            ...
            },
            {
                ...
                accent: "emerald",
                tag: "Priority queue",
                analogy: "Hastanede acil servis triaji gibi dusun — ama mekanizma su: hemsire siray yönetirken kronometreye degil, hayati tehlike göstergelerine bakar; solunum problemi olan hasta bekletilmez, bas agrisi olan bekletilir. Bu 'hissiyat' karari degil, onceden tanimlı kriter listesidir. Bug triage'da da 'kimin bildirdigi' veya 'kimin daha sesli bagrdiginin' degil, 'kullanici ne kaybeder, alternatif var mi, ne siklikla olur' sorularinin kriterleri belirlemesi gerekir. Neden hepsine ASAP demek yetmez? Çünkü gelistiricinin dikkati tek bir noktaya yönelebilir; hepsi 'kritik' ise hiçbiri gercekten ilk sirada bitmez ve release riske girer.",
...
assert bugs.get(0).isReleaseBlocker();                   // Ilk siradaki bug release blocker olmali`,
            ...
            },
            {
                ...
                accent: "cyan",
                tag: "Search space",
                analogy: "Sicak-soguk oyununda her tahminden sonra alanin yarisini elemek gibi dusun — ama bu sadece 'soguk' veya 'sicak' demek degil; her adimda hangi YARI'yi atabileceginize karar vermek. Bu karar ancak arama alaninin SIRALI olmasi durumunda dogru calisir: odalar rastgele dagilmis olsa 'saga bak' sana yanlis yari attirir. QA'da da commit'ler zaman sirasinda, test data degerleri artarak sirali olmalidir — aksi hâlde binary search yanlis tarafi eler ve arama yanlis sonuca varir. Peki son 5 commit'e bakip bulmak yeterli degil mi? Çünkü bug 3 hafta önce girilmis olabilir; 50 commit araliginda yaklasik 6 adimda bulabilirken 50 ayri build kosturmak ciddi zaman ve kaynak israfıdır.",
...
Commit firstBadCommit = commits.get(low);               // Ilk bozan commit bulundu`,
            ...
            },
            {
                ...
                accent: "blue",
                tag: "BFS / DFS",
                analogy: "Metro haritasinda istasyon istasyon gezmek gibi dusun — ama mekanizma kritik: harita olmadan hangi hattan nereye gecilebilecegini bilemezsin; bazi istasyonlara sadece aktarma istasyonundan gidilir, dogrudan hat yoktur. QA'da da checkout flow'unda 'Home -> Cart -> Payment' dogrudan gidilebilir ama 'Login -> Cart' alternatif bir yoldur; bu ikinci yolu test etmezsen oturumu kapanmis kullanicinin sepet davranisi kapsamsiz kalir. Happy path'i test ettim, bu yetmez mi? Çünkü production incident'lerinin büyük kismi istisnai yollarda — payment retry, coupon error, session timeout — ortaya cikar; bu kenarlari coverage haritasinda görmeden test tamamlanmis sayilamaz.",
    ...
    for (Page next : graph.get(current)) {              // Bu sayfadan gidilebilen sayfalari gez
        if (visited.add(next)) {                        // Daha once gezilmediyse isaretle
            ...
            },
            {
                ...
                analogy: "Trafik lambasi gibi dusun — ama mekanizma su: 'kirmizi -> sari -> yesil' sirali ve gecisler kurala baglidir; 'kirmizidan direkt yesil'e gecmek fiziksel olarak mumkun degil. Butonda da ayni kural gecerli: 'disabled -> ready -> loading -> submitted' sirasi vardir; 'disabled' iken tiklamak olur gibi görünse de ElementNotInteractableException veya sessiz no-op ile sonuclanir. Zaten 'wait.until(visible)' yaziyorum, ne farki var? Çünkü buton gorünür olmak, tıklanabilir olmak anlamina gelmez; test 'ready' state'ini beklemeden click atarsa %30 oraninda flaky olur ve bu hata sadece yavas CI makinelerinde görünür.",
...
assertEquals(LoginState.SUBMITTED, state);              // Beklenen son state dogrulanir`,
            ...
            },
            {
                ...
                accent: "rose",
                tag: "Big O",
                analogy: "Süpermarket kasasinda tek sira gibi dusun — ama mekanizma: 10 musteri varken 1 kasa yeterlidir; 10.000 musteride tek kasa collapsar çünkü her yeni musteri için bekleme suresi DOGRUSAL artar. Nested loop da ayni: 100 API kaydina karsi 100 DB kaydini karsilastirmak 10.000 islem demektir, 1.000'e çikinca 1.000.000 isleme calisir. Peki lokal 3 saniyede gecen test neden CI'da 8 dakikaya cikiyor? Çünkü lokal 200 kayitla, CI production-benzeri 50.000 kayitla çalistiriliyor; O(n^2) bu ölcek farkinda katlanarak büyür ve test suite timeout'a düser.",
...
for (String dbId : databaseIds) {                         // Ilk liste tekrar gezilir
        ...
        sections: [
            {
                ...
                accent: "violet",
                tag: "Problem solving",
                analogy: "Think of a recipe — but the mechanism matters: a recipe cannot just say 'mix and bake'; it must say 'beat the egg with sugar for 3 minutes until pale, then bake at 180°C for 35 minutes', otherwise every batch comes out differently. In QA, 'test checkout' is just as vague — the step must be 'a logged-out user attempts payment with an expired coupon; the system must return 400 and inform the user'. Why is writing this level of detail mandatory even for an experienced QA engineer? Because they will not always be there; without a clear recipe the same test in two people's hands produces two different results — or looks flaky.",
    ...
    return BugDecision.ADD_TO_BACKLOG;                 // Everything else goes to backlog
            ...
            },
            {
                ...
                accent: "emerald",
                tag: "Priority queue",
                analogy: "Think of emergency triage — but the mechanism is key: the nurse does not look at arrival time; they check vitals against a predefined criteria list. A breathing problem skips the queue not by intuition but by explicit rule. In bug triage, 'who reported it' or 'who is loudest in the meeting' should never be the criterion — 'what does the user lose, is there a workaround, how often does it happen' should drive priority. Why can't you just mark everything ASAP? Because developers can focus on one thing at a time; if everything is critical, nothing actually finishes first and release risk compounds silently.",
...
assert bugs.get(0).isReleaseBlocker();                   // The first bug must block release`,
            ...
            },
            {
                ...
                accent: "cyan",
                tag: "Search space",
                analogy: "It is like a hot-cold game where each guess removes half the remaining space — but the mechanism is precise: each step tells you which HALF to discard, and that decision is only valid when the space is ORDERED. If rooms were placed randomly, 'this half is colder' gives you no useful information at all. In QA, commits must be ordered by time and the pass/fail behavior must have a clear boundary — otherwise binary search discards the wrong half and sends you in the wrong direction. Why isn't it enough to check the last 5 commits? Because a regression may have been introduced 3 weeks ago; across 50 commits you can find it in roughly 6 steps instead of running 50 separate builds.",
...
Commit firstBadCommit = commits.get(low);               // First bad commit is found`,
            ...
            },
            {
                ...
                accent: "blue",
                tag: "BFS / DFS",
                analogy: "Think of a metro map — but the mechanism matters: without the map you cannot know which line connects to which station; some stations are only reachable via a transfer hub, with no direct route. In QA, 'Home -> Cart -> Payment' is a direct path, but 'Login -> Cart' is an alternate route; if you skip testing it, a returning user's cart behavior remains uncovered. Why isn't testing the happy path enough? Because most production incidents appear on the exceptional paths — payment retry, coupon error, session timeout — and without the graph model you cannot even see which edges are untested.",
    ...
    for (Page next : graph.get(current)) {              // Visit pages reachable from here
        if (visited.add(next)) {                        // If it was not visited before
            ...
            },
            {
                ...
                accent: "amber",
                tag: "Deterministic UI",
                analogy: "Think of traffic lights — but the mechanism is strict: 'red -> yellow -> green' is the legal sequence; going directly from red to green is physically impossible. The same rule applies to a login button: 'disabled -> ready -> loading -> submitted' is the required path; clicking while 'disabled' may appear to succeed but results in an ElementNotInteractableException or a silent no-op. If you are already writing 'wait.until(visible)', what difference does state make? Because visible does not mean clickable; a test that clicks before reaching 'ready' will be flaky roughly 30% of the time — and that failure only appears on slower CI machines, making it hard to reproduce.",
...
assertEquals(LoginState.SUBMITTED, state);              // Assert the expected final state`,
            ...
            },
            {
                ...
                accent: "rose",
                tag: "Big O",
                analogy: "Think of a supermarket checkout — but the mechanism: with 10 customers one lane is fine; with 10,000 a single lane collapses because wait time grows PROPORTIONALLY for each new arrival. A nested loop works the same way: comparing 100 API records against 100 DB records means 10,000 operations; at 1,000 records each that becomes 1,000,000. Why does a 3-second local test turn into an 8-minute CI job? Because local runs use 200 rows while CI runs production-like data with 50,000 rows; O(n^2) explodes at that scale and the suite times out before it can report results.",
