        ...
        blocks: [
          ...
          pmInterviewPractice,
          {
            ...
            questions: [
              ...
              { level: 'intermediate', q: "A collection runner shows all requests as PASS but a bug slipped into production anyway. What is the most likely mistake in the test scripts?", a: "The most common cause is a request that only checks pm.response.to.have.status(200) and nothing about the response BODY — the server returned 200 with wrong or empty data, and the suite never noticed because status alone is not proof of correctness. The fix is asserting on the actual payload, e.g. pm.expect(pm.response.json().items.length).to.be.above(0), not just the status code. This is the same lesson as a Java REST Assured suite that only asserts .statusCode(200) — it is a green suite that verifies almost nothing." },
        ...
        blocks: [
          ...
          pmInterviewPractice,
          {
            ...
            questions: [
              ...
              { level: 'intermediate', q: { tr: 'Koleksiyon çalıştırıcısı tüm istekleri PASS gösteriyor ama yine de bir bug production\'a sızdı. Test script\'lerindeki en olası hata nedir?', en: 'A collection runner shows all requests as PASS but a bug slipped into production anyway. What is the most likely mistake in the test scripts?' }, a: { tr: 'En yaygın neden, sadece pm.response.to.have.status(200) kontrol edip yanıt GÖVDESİNE hiç bakmayan bir isteğin varlığıdır — sunucu 200 ile yanlış veya boş veri döndürmüş, ama suite bunu hiç fark etmemiştir çünkü tek başına status kodu doğruluğun kanıtı değildir. Çözüm, gerçek gövdeyi doğrulamaktır, örn. pm.expect(pm.response.json().items.length).to.be.above(0) — sadece status kodu değil. Bu, sadece .statusCode(200) doğrulayan bir Java REST Assured suite\'inin dersiyle aynıdır: yeşil görünen ama neredeyse hiçbir şeyi doğrulamayan bir suite.', en: 'The most common cause is a request that only checks pm.response.to.have.status(200) and nothing about the response BODY — the server returned 200 with wrong or empty data, and the suite never noticed because status alone is not proof of correctness. The fix is asserting on the actual payload, e.g. pm.expect(pm.response.json().items.length).to.be.above(0), not just the status code. This is the same lesson as a Java REST Assured suite that only asserts .statusCode(200) — it is a green suite that verifies almost nothing.' } },
