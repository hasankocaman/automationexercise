...
export const cypressData = {
  tr: {
    ...
    seoAnswer: 'Cypress, JavaScript ve TypeScript ile yazılan, test kodunu tarayıcının kendi içinde çalıştıran açık kaynaklı bir uçtan uca test otomasyon aracıdır. Selenium\'un aksine ayrı bir WebDriver süreci kullanmaz; bu sayede komutlar arasında otomatik retry-ability sağlar ve testin her adımını zaman içinde geri sararak (time-travel) izlemene izin verir. En yaygın kullanım alanı, modern JavaScript tabanlı web uygulamalarının hem uçtan uca hem de component seviyesinde test edilmesidir.',
  ...
  en: {
    ...
    seoAnswer: 'Cypress is an open-source end-to-end testing tool written in JavaScript and TypeScript that runs your test code directly inside the browser. Unlike Selenium, it does not rely on a separate WebDriver process, which gives it automatic retry-ability between commands and lets you time-travel through every step a test took. Its most common use is testing modern JavaScript web applications at both the end-to-end and component level.',
