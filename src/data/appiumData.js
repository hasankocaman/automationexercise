...
const appiumNodeJdkInstallStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'winget install OpenJS.NodeJS.LTS Node.js kurar…', en: 'winget install OpenJS.NodeJS.LTS installs…' }, detail: { tr: '`winget install OpenJS.NodeJS.LTS` Node.js kurar — bu, Appium Server\'ın kendisinin YAZILDIĞI dil çalışma ortamıdır, senin test kodunla İLGİSİ yoktur.', en: '`winget install OpenJS.NodeJS.LTS` installs Node.js — this is the runtime the Appium Server ITSELF is written in, it has NOTHING to do with your test code language.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'winget install Microsoft.OpenJDK.21…', en: 'winget install Microsoft.OpenJDK.21…' }, detail: { tr: '`winget install Microsoft.OpenJDK.21` AYRI olarak JDK kurar — bu, SENİN Java test kodunu (Appium Java Client) derleyip çalıştıracak ortamdır.', en: '`winget install Microsoft.OpenJDK.21` installs the JDK SEPARATELY — this is the environment that compiles and runs YOUR Java test code (Appium Java Client).' } },
    { id: 3, icon: '3️⃣', label: { tr: 'node --version ve java -version ile…', en: 'Moving to the next step WITHOUT…' }, detail: { tr: '`node --version` ve `java -version` ile doğrulama YAPILMADAN bir sonraki adıma geçmek, hangi bileşenin EKSİK olduğunu ANLAMADAN ilerlemek demektir.', en: 'Moving to the next step WITHOUT verifying with `node --version` and `java -version` means proceeding WITHOUT knowing which component is MISSING.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'JAVA_HOME Registry değişkeni AYRICA…', en: 'The JAVA_HOME Registry variable is set…' }, detail: { tr: '`JAVA_HOME` Registry değişkeni AYRICA ayarlanır çünkü Android SDK araçları ve Appium, JDK\'nın TAM olarak nerede durduğunu bu değişkenden OKUR — `java` komutu PATH\'te çalışsa bile JAVA_HOME eksikse bazı araçlar başarısız OLUR.', en: 'The `JAVA_HOME` Registry variable is set SEPARATELY because Android SDK tools and Appium READ exactly where the JDK lives from this variable — even if `java` works on PATH, some tools FAIL without JAVA_HOME.' } },
  ],
}
...
const appiumLinuxInstallStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'curl -fsSL…', en: 'curl -fsSL…' }, detail: { tr: '`curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -` Ubuntu\'nun kendi paket deposundaki (genelde ESKİ) Node sürümü yerine, NodeSource\'un GÜNCEL 20.x deposunu sisteme EKLER.', en: '`curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -` ADDS NodeSource\'s CURRENT 20.x repository to the system, instead of Ubuntu\'s own (usually OUTDATED) Node version.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'Bu script ÇALIŞTIKTAN SONRA sudo…', en: 'AFTER this script runs…' }, detail: { tr: 'Bu script ÇALIŞTIKTAN SONRA `sudo apt-get install -y nodejs` artık DOĞRU (20.x) sürümü kurar — script ÇALIŞMADAN aynı komut eski bir sürüm kurardı.', en: 'AFTER this script runs, `sudo apt-get install -y nodejs` now installs the CORRECT (20.x) version — WITHOUT the script, the same command would install an old version.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'sudo apt-get install -y openjdk-21-jdk…', en: 'sudo apt-get install -y openjdk-21-jdk…' }, detail: { tr: '`sudo apt-get install -y openjdk-21-jdk` Node kurulumundan TAMAMEN BAĞIMSIZ ayrı bir komuttur — Linux\'ta paket yöneticisi her bileşeni TEK TEK, farklı kaynaklardan yönetir.', en: '`sudo apt-get install -y openjdk-21-jdk` is a COMPLETELY SEPARATE command from the Node install — on Linux the package manager handles each component INDIVIDUALLY, from different sources.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'node --version && java -version iki…', en: 'node --version && java -version CHAINS…' }, detail: { tr: '`node --version && java -version` iki komutu `&&` ile ZİNCİRLER — İLK komut BAŞARISIZ olursa ikinci komut hiç ÇALIŞMAZ, bu da hangi kurulumun EKSİK olduğunu net gösterir.', en: '`node --version && java -version` CHAINS two commands with `&&` — if the FIRST command FAILS, the second never RUNS, clearly showing which install is MISSING.' } },
  ],
}
...
const appiumAndroidSdkSetupStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'Android Studio kurulumu SADECE bir IDE getirir…', en: 'Installing Android Studio ONLY brings an IDE…' }, detail: { tr: 'Android Studio kurulumu SADECE bir IDE getirir — asıl komut satırı araçları (`adb`, `emulator`) "SDK Command-line Tools" paketiyle AYRICA kurulmalıdır.', en: 'Installing Android Studio ONLY brings an IDE — the actual command-line tools (`adb`, `emulator`) must be installed SEPARATELY via the "SDK Command-line Tools" package.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'ANDROID_HOME değişkeni Android SDK\'nın…', en: 'The ANDROID_HOME variable points to the…' }, detail: { tr: '`ANDROID_HOME` değişkeni Android SDK\'nın KÖK klasörünü işaret eder — `adb`, `emulator` gibi araçlar bu klasörün İÇİNDEKİ alt dizinlerde YAŞAR.', en: 'The `ANDROID_HOME` variable points to the ROOT folder of the Android SDK — tools like `adb` and `emulator` LIVE in subdirectories INSIDE this folder.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'PATH\'e SADECE platform-tools (adb için)…', en: 'ONLY platform-tools (for adb) and…' }, detail: { tr: 'PATH\'e SADECE `platform-tools` (adb için) ve `emulator` (emulator komutu için) EKLENİR — SDK\'nın TÜMÜ değil, sadece komut satırından çağıracağın İKİ alt klasör.', en: 'ONLY `platform-tools` (for adb) and `emulator` (for the emulator command) are ADDED to PATH — not the WHOLE SDK, just the TWO subfolders you\'ll call from the command line.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'adb version doğrulaması BAŞARISIZ olursa…', en: 'If adb version verification FAILS…' }, detail: { tr: '`adb version` doğrulaması BAŞARISIZ olursa, sorun ADB\'nin kurulu olmaması DEĞİL, genelde `ANDROID_HOME`/`PATH`\'in yanlış ayarlanmış OLMASIDIR — terminal de YENİDEN başlatılmalıdır ki değişiklikler ETKİN olsun.', en: 'If `adb version` verification FAILS, the problem is usually NOT that ADB isn\'t installed, but that `ANDROID_HOME`/`PATH` is set INCORRECTLY — the terminal must also be RESTARTED for changes to take EFFECT.' } },
  ],
}
...
const appiumServerInstallStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'npm install -g appium SADECE Appium…', en: 'npm install -g appium installs ONLY the…' }, detail: { tr: '`npm install -g appium` SADECE Appium Server\'ın ÇEKİRDEĞİNİ kurar — bu çekirdek HİÇBİR platformu (Android/iOS) doğrudan otomatikleştiremez.', en: '`npm install -g appium` installs ONLY the Appium Server CORE — this core CANNOT automate any platform (Android/iOS) directly by itself.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'appium driver install uiautomator2…', en: 'appium driver install uiautomator2 ADDS…' }, detail: { tr: '`appium driver install uiautomator2` Android desteğini AYRI bir eklenti olarak EKLER — bu, Appium 2\'nin her şeyi tek pakette getiren yapısından FARKLI bir mimaridir (plugin mimarisi).', en: '`appium driver install uiautomator2` ADDS Android support as a SEPARATE plugin — this is a DIFFERENT architecture (plugin-based) from Appium 2\'s everything-in-one-package structure.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'appium driver install xcuitest SADECE…', en: 'appium driver install xcuitest WORKS…' }, detail: { tr: '`appium driver install xcuitest` SADECE Mac\'te ÇALIŞIR çünkü iOS simülatörleri ve XCUITest framework\'ü Apple\'ın kendi araçlarına BAĞIMLIDIR.', en: '`appium driver install xcuitest` WORKS ONLY on Mac because iOS simulators and the XCUITest framework DEPEND on Apple\'s own tooling.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'appium komutu SUNUCUYU başlatır ve…', en: 'The appium command STARTS the server…' }, detail: { tr: '`appium` komutu SUNUCUYU başlatır ve `http://127.0.0.1:4723`\'te DİNLEMEYE başlar — bu terminal AÇIK kalmalı, test kodun bu adrese HTTP isteği gönderecek.', en: 'The `appium` command STARTS the server and begins LISTENING on `http://127.0.0.1:4723` — this terminal must stay OPEN, your test code will send HTTP requests to this address.' } },
  ],
}
...
const appiumDoctorStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'appium-doctor --android önceki TÜM…', en: 'appium-doctor --android verifies ALL…' }, detail: { tr: '`appium-doctor --android` önceki TÜM kurulum adımlarını (Node, JDK, ANDROID_HOME, adb, UIAutomator2 driver) TEK bir komutla SIRAYLA doğrular — her birini elle ayrı ayrı KONTROL etmen gerekmez.', en: '`appium-doctor --android` verifies ALL previous install steps (Node, JDK, ANDROID_HOME, adb, UIAutomator2 driver) IN SEQUENCE with ONE command — no NEED to check each one manually.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'HER satırın başındaki ✔ işareti o…', en: 'The ✔ mark at the start of EACH line…' }, detail: { tr: 'HER satırın başındaki ✔ işareti o BİLEŞENİN doğru kurulduğunu gösterir — bir tanesi ✖ (kırmızı) ise, kurulumun TAM OLARAK hangi adımda EKSİK kaldığı ANINDA görünür.', en: 'The ✔ mark at the start of EACH line shows that COMPONENT is correctly installed — if one shows ✖ (red), you INSTANTLY see EXACTLY which install step is MISSING.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'Bu araç HİÇBİR ŞEYİ otomatik DÜZELTMEZ…', en: 'This tool FIXES NOTHING automatically…' }, detail: { tr: 'Bu araç HİÇBİR ŞEYİ otomatik DÜZELTMEZ — sadece TEŞHİS koyar; düzeltme kararı (örn. hangi JDK sürümünü kurman gerektiği) hâlâ SANA aittir.', en: 'This tool FIXES NOTHING automatically — it only DIAGNOSES; the fix decision (e.g. which JDK version to install) is still YOURS to make.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'appium-doctor --ios SADECE Mac\'te ANLAMLIDIR…', en: 'appium-doctor --ios is MEANINGFUL ONLY on Mac…' }, detail: { tr: '`appium-doctor --ios` SADECE Mac\'te ANLAMLIDIR — Windows/Linux\'ta iOS geliştirme araçları (Xcode) zaten hiç MEVCUT olamaz.', en: '`appium-doctor --ios` is MEANINGFUL ONLY on Mac — on Windows/Linux, iOS development tools (Xcode) can never EXIST in the first place.' } },
  ],
}
...
const appiumAvdSetupStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'emulator -list-avds sadece DİSKTE…', en: 'emulator -list-avds lists only virtual…' }, detail: { tr: '`emulator -list-avds` sadece DİSKTE TANIMLI (oluşturulmuş ama henüz ÇALIŞMIYOR olabilecek) sanal cihazları listeler — bu, "hangi AVD\'ler VAR" sorusuna cevap verir.', en: '`emulator -list-avds` lists only virtual devices DEFINED on disk (created but maybe not YET running) — it answers "which AVDs EXIST".' } },
    { id: 2, icon: '2️⃣', label: { tr: 'emulator -avd Pixel_7_API_33…', en: 'emulator -avd Pixel_7_API_33…' }, detail: { tr: '`emulator -avd Pixel_7_API_33 -no-snapshot-load` belirtilen AVD\'yi TEMİZ bir durumdan (önceki oturumun kaydını YÜKLEMEDEN) BAŞLATIR — snapshot yüklemek daha hızlıdır ama bazen ESKİ/BOZUK bir duruma döner.', en: '`emulator -avd Pixel_7_API_33 -no-snapshot-load` STARTS the specified AVD from a CLEAN state (WITHOUT loading the previous session\'s snapshot) — loading a snapshot is faster but can sometimes restore a STALE/BROKEN state.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'adb devices ise şu ANDA GERÇEKTEN BAĞLI…', en: 'adb devices lists devices CURRENTLY…' }, detail: { tr: '`adb devices` ise şu ANDA GERÇEKTEN BAĞLI olan (çalışan emülatör veya USB\'li fiziksel cihaz) aygıtları listeler — "hangi AVD\'ler var" ile "hangi cihaz ŞU AN erişilebilir" FARKLI sorulardır.', en: '`adb devices` lists devices CURRENTLY ACTUALLY CONNECTED (a running emulator or a physical device over USB) — "which AVDs exist" and "which device is REACHABLE right now" are DIFFERENT questions.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Çıktıda emulator-5554   device satırının…', en: 'Seeing the emulator-5554   device line…' }, detail: { tr: 'Çıktıda `emulator-5554   device` satırının GÖRÜNMESİ, Appium\'un bu emülatöre bağlanabileceğinin TEK kanıtıdır — sadece emülatör PENCERESİNİN açık olması YETERLİ değildir, `adb` onu TANIMALIDIR.', en: 'Seeing the `emulator-5554   device` line in the output is the ONLY proof Appium can connect to this emulator — just having the emulator WINDOW open is NOT enough, `adb` must RECOGNIZE it.' } },
  ],
}
...
const appiumWdioTsSetupStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'npm init -y boş bir package.json OLUŞTURUR…', en: 'npm init -y CREATES an empty package.json…' }, detail: { tr: '`npm init -y` boş bir `package.json` OLUŞTURUR — bu, projenin bağımlılıklarını takip edecek TEMEL dosyadır, henüz hiçbir paket YÜKLENMEMİŞTİR.', en: '`npm init -y` CREATES an empty `package.json` — this is the BASE file that will track the project\'s dependencies, no package is INSTALLED yet.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'npx wdio config interaktif bir SİHİRBAZDIR…', en: 'npx wdio config is an interactive WIZARD…' }, detail: { tr: '`npx wdio config` interaktif bir SİHİRBAZDIR — sorulara (framework, servisler, reporter seçimi) verdiğin cevaplara göre `wdio.conf.ts` dosyasını OTOMATİK üretir, elle yazmana GEREK kalmaz.', en: '`npx wdio config` is an interactive WIZARD — based on your answers (framework, services, reporter choice) it AUTO-generates `wdio.conf.ts`, no NEED to hand-write it.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'Manuel kurulum alternatifi AYNI paketleri…', en: 'The manual install alternative adds the…' }, detail: { tr: 'Manuel kurulum alternatifi AYNI paketleri sihirbazsız, TEK TEK `npm install` ile ekler — sihirbazın perde arkasında NE yaptığını anlamak istersen bu YOLU izlersin.', en: 'The manual install alternative adds the SAME packages ONE BY ONE with `npm install`, without the wizard — you\'d follow this PATH if you want to understand what the wizard does BEHIND the scenes.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'npx wdio --version doğrulaması…', en: 'npx wdio --version verification PROVES…' }, detail: { tr: '`npx wdio --version` doğrulaması, kurulumun TAMAMLANDIĞINI ve bir sonraki adımda (`wdio.conf.ts` düzenleme) çalışmaya HAZIR olduğunu KANITLAR.', en: '`npx wdio --version` verification PROVES the install is COMPLETE and ready to work on the next step (editing `wdio.conf.ts`).' } },
  ],
}
...
const appiumCapabilitiesTypeSafeStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'new UiAutomator2Options() ile başlayan ZİNCİR…', en: 'The CHAIN starting with new…' }, detail: { tr: '`new UiAutomator2Options()` ile başlayan ZİNCİR, eski `DesiredCapabilities` + `capability("key", "value")` string-tabanlı yaklaşımın YERİNİ alır — `.setPlatformName(...)` gibi TİP GÜVENLİ metotlar, yanlış yazılmış bir key\'in DERLEME zamanında YAKALANMASINI sağlar.', en: 'The CHAIN starting with `new UiAutomator2Options()` REPLACES the old string-based `DesiredCapabilities` + `capability("key", "value")` approach — TYPE-SAFE methods like `.setPlatformName(...)` let a misspelled key be CAUGHT at COMPILE time.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'W3C STANDART capability\'ler (platformName…', en: 'W3C STANDARD capabilities (platformName…' }, detail: { tr: 'W3C STANDART capability\'ler (`platformName`, `deviceName`) HİÇBİR önek ALMAZ, ama Appium\'a ÖZGÜ olanlar (`automationName`, `app`) java-client tarafından OTOMATİK `appium:` önekiyle GÖNDERİLİR — bunu elle yazmana gerek YOKTUR.', en: 'W3C STANDARD capabilities (`platformName`, `deviceName`) get NO prefix, but Appium-SPECIFIC ones (`automationName`, `app`) are AUTOMATICALLY sent with the `appium:` prefix by java-client — you don\'t hand-write this.' } },
    { id: 3, icon: '3️⃣', label: { tr: '.setApp(...) VEYA .setAppPackage(...) +…', en: '.setApp(...) OR .setAppPackage(...) +…' }, detail: { tr: '`.setApp(...)` VEYA `.setAppPackage(...)` + `.setAppActivity(...)` — İKİ FARKLI senaryo: APK dosyası HENÜZ cihazda değilse `setApp` onu YÜKLER, zaten kuruluysa package+activity DOĞRUDAN uygulamayı AÇAR.', en: '`.setApp(...)` OR `.setAppPackage(...)` + `.setAppActivity(...)` — TWO DIFFERENT scenarios: if the APK is NOT YET on the device, `setApp` INSTALLS it; if already installed, package+activity DIRECTLY OPENS the app.' } },
    { id: 4, icon: '4️⃣', label: { tr: '.setNoReset(false) HER testten önce…', en: '.setNoReset(false) RESETS app data before…' }, detail: { tr: '`.setNoReset(false)` HER testten önce uygulama verisini SIFIRLAR — bu, bir testin BIRAKTIĞI verinin bir SONRAKİ testi ETKİLEMEMESİNİ garanti eder, testler birbirinden İZOLE kalır.', en: '`.setNoReset(false)` RESETS app data before EVERY test — this guarantees data LEFT by one test doesn\'t AFFECT the NEXT test, keeping tests ISOLATED from each other.' } },
  ],
}
...
const appiumJavaLocatorsStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'AppiumBy.id(...) EN HIZLI ve GÜVENİLİR…', en: 'AppiumBy.id(...) is the FASTEST and…' }, detail: { tr: '`AppiumBy.id(...)` EN HIZLI ve GÜVENİLİR yöntemdir — Android\'in `resource-id`\'sini DOĞRUDAN hedefler, tıpkı Selenium\'daki `By.id()` gibi ama paket adı ÖNEKİYLE.', en: '`AppiumBy.id(...)` is the FASTEST and MOST RELIABLE method — it DIRECTLY targets Android\'s `resource-id`, just like Selenium\'s `By.id()` but WITH a package-name prefix.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'AppiumBy.accessibilityId(...) sadece…', en: 'AppiumBy.accessibilityId(...) is NOT just…' }, detail: { tr: '`AppiumBy.accessibilityId(...)` sadece bir locator DEĞİLDİR — TalkBack/VoiceOver gibi erişilebilirlik araçlarının da KULLANDIĞI aynı `content-desc` alanını hedeflediği için, bu locator\'ı kullanmak dolaylı yoldan erişilebilirlik TESTİ de yapar.', en: '`AppiumBy.accessibilityId(...)` is NOT just a locator — because it targets the SAME `content-desc` field that accessibility tools like TalkBack/VoiceOver USE, using this locator indirectly performs accessibility TESTING too.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'androidUIAutomator ile yazılan…', en: 'The…' }, detail: { tr: '`androidUIAutomator` ile yazılan `UiSelector().scrollable(true)).scrollIntoView(...)` zinciri, Selenium\'da OLMAYAN bir yetenektir — mobil ekranda görünmeyen bir elemana OTOMATİK kaydırarak ULAŞIR.', en: 'The `UiSelector().scrollable(true)).scrollIntoView(...)` chain written via `androidUIAutomator` is a capability Selenium does NOT have — it AUTOMATICALLY scrolls to REACH an element not currently visible on the mobile screen.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'AppiumBy.xpath(...) yorum satırında "last…', en: 'AppiumBy.xpath(...) is MARKED "last resort…' }, detail: { tr: '`AppiumBy.xpath(...)` yorum satırında "last resort, slow!" diye İŞARETLİDİR — 7 stratejinin EN SONUNA konması tesadüf değildir, öncelik SIRASINI da GÖSTERİR.', en: '`AppiumBy.xpath(...)` is MARKED "last resort, slow!" in the comment — being placed LAST among the 7 strategies is not a coincidence, it also SHOWS the priority ORDER.' } },
  ],
}
...
const appiumTsLocatorsStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: '$(\'~Login Button\')…', en: '$(\'~Login Button\')…' }, detail: { tr: '`$(\'~Login Button\')` — baştaki `~` işareti WebdriverIO\'nun ÖZEL sözdizimidir ve Java\'daki `AppiumBy.accessibilityId("Login Button")` ile AYNI şeyi yapar, sadece ÇOK daha kısa yazılır.', en: '`$(\'~Login Button\')` — the leading `~` is WebdriverIO\'s SPECIAL syntax and does the SAME thing as Java\'s `AppiumBy.accessibilityId("Login Button")`, just written MUCH shorter.' } },
    { id: 2, icon: '2️⃣', label: { tr: '$(\'android=new UiSelector()...\')…', en: '$(\'android=new UiSelector()...\')…' }, detail: { tr: '`$(\'android=new UiSelector()...\')` — `android=` ÖNEKİ, string içindeki geri kalan kısmın DOĞRUDAN Android\'in native UiSelector sözdizimi olduğunu WebdriverIO\'ya SÖYLER.', en: '`$(\'android=new UiSelector()...\')` — the `android=` PREFIX TELLS WebdriverIO that the rest of the string is DIRECTLY Android\'s native UiSelector syntax.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'get loginButton() bir GETTER olarak tanımlanır…', en: 'get loginButton() is defined as a GETTER…' }, detail: { tr: '`get loginButton()` bir GETTER olarak tanımlanır, bir DEĞİŞKEN olarak değil — bu, HER erişimde elementi TAZE bulmayı sağlar; sayfa yeniden render olsa bile eski/geçersiz bir referansa TAKILMAZSIN.', en: '`get loginButton()` is defined as a GETTER, not a VARIABLE — this ensures the element is found FRESH on EVERY access; even if the page re-renders, you never get STUCK with a stale reference.' } },
    { id: 4, icon: '4️⃣', label: { tr: '$$(\'android.widget.LinearLayout\')[1]…', en: '$$(\'android.widget.LinearLayout\')[1]…' }, detail: { tr: '`$$(\'android.widget.LinearLayout\')[1]` — Java\'daki `AppiumBy.className()`\'in TypeScript\'teki KARŞILIĞI çoklu-eleman versiyonudur; `$$` ÇOĞUL sorgu anlamına gelir, `$` TEKİL.', en: '`$$(\'android.widget.LinearLayout\')[1]` — this is the TypeScript multi-element COUNTERPART of Java\'s `AppiumBy.className()`; `$$` means a PLURAL query, `$` a SINGULAR one.' } },
  ],
}
...
const appiumLoginPagePomStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: '@AndroidFindBy(id = "...") annotation\'ı…', en: 'The @AndroidFindBy(id = "...")…' }, detail: { tr: '`@AndroidFindBy(id = "...")` annotation\'ı, Selenium\'daki `@FindBy`\'ın Appium karşılığıdır — elementi ANINDA aramaz, sadece BİR LOCATOR TANIMI yapar.', en: 'The `@AndroidFindBy(id = "...")` annotation is the Appium counterpart of Selenium\'s `@FindBy` — it does NOT search for the element IMMEDIATELY, it only DEFINES a locator.' } },
    { id: 2, icon: '2️⃣', label: { tr: 'PageFactory.initElements(new…', en: 'PageFactory.initElements(new…' }, detail: { tr: '`PageFactory.initElements(new AppiumFieldDecorator(driver), this)` bu annotation\'ları GERÇEK `WebElement` referanslarına ÇEVİRİR — bu satır ÇALIŞMADAN `emailInput` gibi alanlar `null` KALIR.', en: '`PageFactory.initElements(new AppiumFieldDecorator(driver), this)` CONVERTS these annotations into ACTUAL `WebElement` references — WITHOUT this line running, fields like `emailInput` stay `null`.' } },
    { id: 3, icon: '3️⃣', label: { tr: 'WebDriverWait alanı LoginPage\'in kendi…', en: 'The WebDriverWait field is defined…' }, detail: { tr: '`WebDriverWait` alanı LoginPage\'in kendi İÇİNDE tanımlanır — böylece bu sayfayı kullanan HER metot, elementin hazır olmasını beklemek için AYRI AYRI wait nesnesi OLUŞTURMAK zorunda kalmaz.', en: 'The `WebDriverWait` field is defined INSIDE LoginPage itself — so EVERY method using this page doesn\'t have to CREATE a SEPARATE wait object to wait for element readiness.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Bu POM sınıfı hiçbir ASSERTION İÇERMEZ…', en: 'This POM class contains NO ASSERTIONS…' }, detail: { tr: 'Bu POM sınıfı hiçbir ASSERTION İÇERMEZ — sadece "sayfada NELER var, NASIL etkileşime girilir" bilgisini taşır; DOĞRULAMA görevi test sınıfına AİTTİR, bu ayrım POM\'un TEMEL kuralıdır.', en: 'This POM class contains NO ASSERTIONS — it only carries "WHAT exists on the page, HOW to interact"; the VERIFICATION job BELONGS to the test class, this separation is the CORE rule of POM.' } },
  ],
}
...
const appiumLoginTestJUnitStep = {
  ...
  steps: [
    { id: 1, icon: '1️⃣', label: { tr: 'extends BaseTest ile LoginTest…', en: 'By extends BaseTest…' }, detail: { tr: '`extends BaseTest` ile `LoginTest`, driver kurulumunu/kapatmasını TEKRAR YAZMAZ — bu sorumluluk zaten BaseTest\'te merkezi olarak ÇÖZÜLMÜŞTÜR, LoginTest sadece iş MANTIĞINA odaklanır.', en: 'By `extends BaseTest`, `LoginTest` does NOT rewrite driver setup/teardown — this responsibility is ALREADY centrally SOLVED in BaseTest, LoginTest focuses only on business LOGIC.' } },
    { id: 2, icon: '2️⃣', label: { tr: '@BeforeEach openLogin() HER test…', en: '@BeforeEach openLogin() CREATES a fresh…' }, detail: { tr: '`@BeforeEach openLogin()` HER test metodundan ÖNCE yeni bir `LoginPage` nesnesi OLUŞTURUR — bu, bir testteki durumun BİR SONRAKİ teste SIZMAMASINI sağlar.', en: '`@BeforeEach openLogin()` CREATES a fresh `LoginPage` object BEFORE EVERY test method — this ensures state from one test does NOT LEAK into the NEXT.' } },
    { id: 3, icon: '3️⃣', label: { tr: '@TestMethodOrder(OrderAnnotation.class)…', en: '@TestMethodOrder(OrderAnnotation.class)…' }, detail: { tr: '`@TestMethodOrder(OrderAnnotation.class)` + `@Order(1)`/`@Order(2)` testlerin ÇALIŞMA sırasını GARANTİ eder — bu SIRALAMA rastgele değildir: önce BAŞARILI giriş, sonra HATALI giriş test edilir, mantıksal bir İLERLEME izler.', en: '`@TestMethodOrder(OrderAnnotation.class)` + `@Order(1)`/`@Order(2)` GUARANTEE test execution order — this ORDERING is not random: SUCCESSFUL login is tested first, then FAILED login, following a logical PROGRESSION.' } },
    { id: 4, icon: '4️⃣', label: { tr: 'Fluent zincir…', en: 'The fluent chain…' }, detail: { tr: 'Fluent zincir `loginPage.typeEmail(...).typePassword(...).tapLogin()` — HER metot `this` (veya bir SONRAKİ sayfayı) DÖNDÜRÜR, bu sayede testin OKUNUŞU adım adım bir SENARYO gibi AKAR.', en: 'The fluent chain `loginPage.typeEmail(...).typePassword(...).tapLogin()` — EVERY method RETURNS `this` (or the NEXT page), so the test READS like a step-by-step SCENARIO.' } },
  ],
}
