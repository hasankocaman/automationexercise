    ...
    "blocks": [
      {
        ...
        "emoji": "📦",
        "content": {
          "tr": "Subquery'yi bir soruşturma zinciri gibi düşün: 'Ortalamadan yavaş testler hangileri?' diye sormadan önce birinin 'ortalama kaç?' sorusunu cevaplamış olması gerekir. İç sorgu bu ilk cevabı üretir, dış sorgu onun üzerine karar verir.\n\nPeki neden bu iki adımı tek bir WHERE koşuluna sıkıştıramıyoruz — `WHERE duration_ms > AVG(duration_ms)` neden çalışmaz? Çünkü WHERE satırları TEK TEK süzerken tablonun tamamının ortalaması henüz hesaplanmamıştır; ortalama bütünü görmeyi ister, WHERE ise parçayı. Subquery tam olarak bu sıra problemini çözer.\n\nJava'da bunu iki ayrı satırda çözersin: önce `double avg = list.stream().mapToInt(T::getDuration).average().orElse(0)`, sonra `list.stream().filter(t -> t.duration > avg)`. Aradaki fark şu: Java'da ara sonucu tutan bir **değişken** vardır, SQL'de yoktur — iç sorgu o değişkenin yerini tutar ve her çalıştırmada yeniden hesaplanır.\n\nQA tarafında bunun karşılığı somut: gece koşan regression suite'inin 400 satırlık raporunu Excel'e taşıyıp elle ortalama almak yerine, pipeline tek sorguyla 'ortalamayı yukarı çeken testler' listesini üretir. Yavaş test = flaky test adayıdır; timeout sınırına yaklaşan testler bir sonraki release'de rastgele kırılmaya başlar.",
          "en": "Think of a subquery as a chain of interrogation: before you can ask \"which tests are slower than average?\", someone must already have answered \"what IS the average?\". The inner query produces that first answer; the outer query then makes a decision on top of it.\n\nSo why can't we compress both steps into a single WHERE clause — why does `WHERE duration_ms > AVG(duration_ms)` fail? Because WHERE filters rows ONE AT A TIME, while the average needs the whole table to exist first; the aggregate wants the whole, WHERE only ever sees a part. A subquery exists precisely to fix that ordering problem.\n\nIn Java you solve it with two separate statements: `double avg = list.stream().mapToInt(T::getDuration).average().orElse(0)`, then `list.stream().filter(t -> t.duration > avg)`. The real difference: Java gives you a **variable** holding the intermediate result; SQL does not — the inner query takes that variable's place and is re-evaluated on every run.\n\nFor a QA engineer this is concrete: instead of exporting a 400-row nightly regression report into Excel to average it by hand, the pipeline answers \"which tests drag the average up\" in one query. A slow test is a flaky test candidate — anything creeping toward the timeout boundary starts failing at random in the next release."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🎯",
        "content": {
          "tr": "Bu üç operatörü bir arşiv memurunun üç farklı arama refleksi gibi düşün. LIKE, dosya adının bir parçasını hatırlayan memurdur: 'adında Login geçen ne varsa getir'. BETWEEN, takvimden aralık işaretleyen memurdur: 'ocak ayının tamamı'. IN ise elinde hazır bir liste tutar: 'şu üç durumdan biri olan her şey'.\n\nPeki neden SQL bunlar için ayrı operatörler tanımlamış — hepsini OR ile yazmak zaten mümkünken? Çünkü `status='FAIL' OR status='SKIP' OR status='ERROR'` üç satırda okunabilir ama on beş değerde okunamaz hâle gelir; üstelik birini yanlışlıkla `AND` yazdığında sorgu **sessizce sıfır satır** döner, hata vermez. IN, o hatayı yapmanı imkânsız kılar. Aynı şekilde BETWEEN, `>= AND <=` çiftinde sık yapılan 'sınırı dahil etmeyi unutma' hatasını ortadan kaldırır.\n\nJava'daki karşılıkları tanıdık: LIKE ≈ `String.contains()` / `matches()`, BETWEEN ≈ `d >= lo && d <= hi` aralık kontrolü, IN ≈ `List.of(...).contains(status)`. Fark şu ki Java'da bu kontrolü veriyi belleğe **çektikten sonra** yaparsın; SQL'de ise veritabanının içinde, index kullanılarak yapılır — 2 milyon satırlık test geçmişinde bu fark saniyeler değil dakikalardır.\n\nQA bağlamında bu üçlü günlük ekmektir: 'son sprint'te adında Checkout geçen ve FAIL/ERROR durumundaki koşumlar' sorusu, bir bug raporunu 'ara sıra kırılıyor' iddiasından çıkarıp 'şu tarih aralığında 12 kez kırıldı' kanıtına dönüştürür.",
          "en": "Think of these three operators as an archive clerk's three different search reflexes. LIKE is the clerk who half-remembers the filename: \"bring me anything with Login in it\". BETWEEN is the clerk marking a range on a calendar: \"all of January\". IN is the clerk holding a pre-written list: \"anything in one of these three states\".\n\nSo why did SQL define dedicated operators at all, when OR can already express every one of them? Because `status='FAIL' OR status='SKIP' OR status='ERROR'` reads fine at three values and becomes unreadable at fifteen — and if you typo one of them into `AND`, the query returns **zero rows silently** instead of raising an error. IN makes that mistake impossible. BETWEEN does the same for the classic off-by-one of `>= AND <=`, where people forget the boundary is inclusive.\n\nThe Java equivalents are familiar: LIKE ≈ `String.contains()` / `matches()`, BETWEEN ≈ a `d >= lo && d <= hi` range check, IN ≈ `List.of(...).contains(status)`. The difference is where the work happens: in Java you filter **after** pulling the data into memory; in SQL the engine filters inside the database, using an index. Across two million rows of test history that gap is measured in minutes, not seconds.\n\nIn QA work this trio is daily bread: \"runs from the last sprint whose name contains Checkout and whose status is FAIL or ERROR\" turns a bug report from \"it breaks sometimes\" into \"it broke 12 times in this date range\" — a claim into evidence."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🪟",
        "content": {
          "tr": "GROUP BY'ı bir sınıf fotoğrafını yakıp yerine tek bir 'sınıf ortalaması' kâğıdı asmak gibi düşün: özet elinde kalır, öğrenciler kaybolur. Window fonksiyonu ise fotoğrafı olduğu gibi bırakır ve her öğrencinin göğsüne küçük bir etiket iliştirir: 'sınıf ortalaması 62, sen 78, sınıfta 3. sıradasın'. Satırlar durur, bağlam eklenir.\n\nPeki neden GROUP BY tek başına yetmiyor — ortalamayı bir kez hesaplayıp sonuçları elde birleştirmek varken neden yeni bir kavram öğreniyoruz? Çünkü 'her testin kendi environment ortalamasına göre nerede durduğu' sorusunda satır ile özet AYNI çıktıda yan yana durmak zorundadır. GROUP BY ile bunu istediğinde tabloyu kendisiyle tekrar JOIN'lemen gerekir; sorgu iki katına çıkar, JOIN koşulunu kaçırdığında da sonuç sessizce çoğalır.\n\nJava'da bunun karşılığı iki geçiştir: önce `Collectors.groupingBy(..., averagingInt(...))` ile bir Map çıkarırsın, sonra listeyi tekrar dolaşıp her elemanı o Map'teki değerle karşılaştırırsın. `OVER (PARTITION BY environment)` tam olarak bu iki geçişi tek ifadede yapar — ara Map yoktur, ikinci döngü yoktur.\n\nQA'de bu, flaky test avının temel aracıdır: `ROW_NUMBER() OVER (PARTITION BY test_name ORDER BY run_date DESC)` ile her testin SON koşumunu ayıklarsın; `AVG(duration_ms) OVER (PARTITION BY environment)` ile 'staging'de normal ama prod'da ortalamanın iki katı süren' testleri yakalarsın. Bu ikincisi çoğu zaman testin değil, ortamın bozuk olduğunun ilk kanıtıdır.",
          "en": "Think of GROUP BY as burning the class photo and pinning up a single sheet that says \"class average: 62\" — you keep the summary and lose the students. A window function leaves the photo intact and clips a small label onto each student instead: \"class average 62, you scored 78, you rank 3rd\". The rows survive; context is added.\n\nSo why isn't GROUP BY enough — why learn a new concept when you could compute the average once and stitch the results together yourself? Because the question \"where does each test stand relative to its own environment's average?\" requires the row and the summary to sit side by side in the SAME result. With GROUP BY you'd have to join the table back onto itself; the query doubles in size, and the moment you get the join condition wrong the rows multiply silently.\n\nIn Java the equivalent is two passes: build a Map with `Collectors.groupingBy(..., averagingInt(...))`, then walk the list again comparing each element against that Map. `OVER (PARTITION BY environment)` collapses both passes into one expression — no intermediate Map, no second loop.\n\nFor QA this is the core flaky-hunting tool: `ROW_NUMBER() OVER (PARTITION BY test_name ORDER BY run_date DESC)` isolates each test's LATEST run, and `AVG(duration_ms) OVER (PARTITION BY environment)` surfaces tests that behave normally on staging but run at twice the average in prod. That second one is usually the first hard evidence that the environment is broken, not the test."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🧱",
        "content": {
          "tr": "CTE'yi bir yemek tarifindeki 'ön hazırlık' bölümü gibi düşün. Kötü yazılmış tarif tek cümledir: 'soğanı doğrayıp kavurduğun karışıma, ayrı bir kapta ezdiğin domatesi ekleyerek elde ettiğin sosu makarnaya dök.' İyi tarif ise adımlara böler: 1) sos hazırla, 2) makarnayı haşla, 3) birleştir. `WITH yavas_testler AS (...)` tam olarak o numaralı adımdır — ara sonuca **isim** verir.\n\nPeki bu sadece kozmetik bir düzenleme mi; iç içe subquery ile aynı sonucu üretiyorsa neden zahmet edelim? İki gerçek fark var. Birincisi okuma yönü: iç içe subquery **içeriden dışarıya** okunur, insan beyni ise yukarıdan aşağıya okur — üç seviye iç içe geçmiş bir sorguda hangi parantezin kimin olduğunu 6 ay sonra sen bile çıkaramazsın. İkincisi tekrar kullanım: aynı ara sonuca sorgunun iki ayrı yerinde ihtiyaç duyduğunda subquery'yi kopyalayıp yapıştırman gerekir, CTE'yi ise adıyla iki kez çağırırsın. Kopyalanan sorgu, birini güncelleyip diğerini unutunca sessizce yanlış rapor üreten türden bir borçtur.\n\nJava'daki karşılığı doğrudan: uzun bir zinciri `var slowTests = ...; var baseline = ...; return compare(slowTests, baseline);` diye ara değişkenlere bölmek. CTE, SQL'in `local variable`'ıdır — ama Java'nın aksine tek bir ifadenin ömrü kadar yaşar, bir sonraki sorguda yoktur.\n\nQA raporlamasında bu, 'geçen haftaya göre yavaşlayan testler' gibi çok adımlı soruların tek okunabilir sorguya sığması demektir: CTE1 bu haftanın ortalamaları, CTE2 geçen haftanınki, ana sorgu farkı alır. Bu sorgu bir kez yazılıp nightly pipeline'a konduğunda, performans regresyonunu kimse fark etmeden önce yakalar.",
          "en": "Think of a CTE as the \"prep\" section of a recipe. A badly written recipe is one sentence: \"pour over the pasta the sauce you obtained by adding the tomatoes you crushed separately to the onions you diced and sautéed.\" A good one numbers the steps: 1) make the sauce, 2) boil the pasta, 3) combine. `WITH slow_tests AS (...)` is exactly that numbered step — it gives the intermediate result a **name**.\n\nBut is this purely cosmetic? If a nested subquery produces the same result, why bother? Two real differences. First, reading direction: nested subqueries read **inside-out**, while human beings read top-down — three levels deep, not even you will know which parenthesis belongs to what six months later. Second, reuse: when two parts of the query need the same intermediate result, a subquery must be copy-pasted, whereas a CTE is simply called twice by name. Copy-pasted SQL is the kind of debt that silently produces a wrong report the day you update one copy and forget the other.\n\nThe Java parallel is direct: breaking a long chain into `var slowTests = ...; var baseline = ...; return compare(slowTests, baseline);`. A CTE is SQL's local variable — except that, unlike Java's, it lives only as long as the single statement it belongs to.\n\nIn QA reporting this is what lets a multi-step question like \"which tests got slower than last week?\" fit into one readable query: CTE1 holds this week's averages, CTE2 last week's, and the main query takes the difference. Written once and wired into the nightly pipeline, it catches performance regression before anyone notices it by hand."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🏦",
        "content": {
          "tr": "Transaction'ı bir nikâh töreni gibi düşün: 'evet' iki taraftan da gelmeden kimse evli sayılmaz, biri yarıda vazgeçerse tören yarım kalmaz — hiç olmamış sayılır. Havalede de aynısı geçerlidir: paranın A hesabından çıkması VE B hesabına girmesi tek bir bütündür; ikisinin arasında elektrik kesilirse veritabanı yarım işi kabul etmez, `ROLLBACK` ile başlangıç anına döner.\n\nPeki 'yarım kalırsa geri al' fikri bu kadar bariz iken, neden bunu uygulama kodunda kendimiz yapamıyoruz — hata yakalayıp ters UPDATE atmak yetmez mi? Yetmez, ve nedeni düşündürücüdür: telafi kodunun kendisi de çökebilir. Para çıktıktan sonra process öldüyse, o telafi UPDATE'ini çalıştıracak kimse kalmamıştır. Transaction'ın gücü, garantiyi **senin kodundan alıp** veritabanının kendi kurtarma günlüğüne (WAL/redo log) taşımasıdır — orada sözü tutacak olan, çöken sürecin kendisi değildir.\n\nJava'daki `synchronized` bloğu yüzeyde benzer görünür ama farkı bilmek kritiktir: `synchronized` sadece **izolasyon** verir (aynı anda iki thread girmesin), atomiklik veya kalıcılık vermez — JVM çökerse yarım kalan iş yarım kalır. Transaction ACID'in dördünü birden verir: Atomic, Consistent, Isolated, Durable.\n\nQA tarafında transaction en çok test verisi hazırlarken işe yarar ve en çok orada unutulur: `BEGIN` ile açılıp `COMMIT`/`ROLLBACK` ile kapatılmayan bir session, tabloyu kilitli bırakır ve o tabloya dokunan diğer bütün testler timeout ile düşer. Sabah gelip 'dün gece 40 test aynı anda kırılmış' dediğin senaryoların klasik sebebi budur — testlerin hiçbiri bozuk değildir, biri kilidi bırakmamıştır. Tersi de bir tekniktir: testi transaction içinde koşup sonunda `ROLLBACK` çekmek, veritabanını test öncesi hâline birebir döndürür.",
          "en": "Think of a transaction as a wedding ceremony: nobody is married until the \"I do\" comes from both sides, and if one side walks out mid-way the ceremony isn't half-done — it is treated as never having happened. A bank transfer works the same way: money leaving account A and arriving in account B is a single indivisible whole; if the power dies in between, the database refuses the half-finished state and `ROLLBACK` restores the starting point.\n\nBut if \"undo it if it half-fails\" is such an obvious idea, why can't we just do it in application code — catch the exception and issue a compensating UPDATE? You can't, and the reason is worth sitting with: the compensating code can crash too. If the process dies right after the money left, there is nobody left alive to run that reversal. A transaction's real power is that it moves the guarantee **out of your code** and into the database's own recovery log (WAL/redo) — where the thing keeping the promise is not the process that crashed.\n\nJava's `synchronized` block looks similar on the surface, and the difference matters: `synchronized` gives you **isolation** only (no two threads inside at once). It gives no atomicity and no durability — if the JVM dies, half-done work stays half-done. A transaction gives all four ACID properties: Atomic, Consistent, Isolated, Durable.\n\nIn QA work transactions matter most when preparing test data, and that is exactly where they get forgotten: a session opened with `BEGIN` and never closed with `COMMIT`/`ROLLBACK` leaves the table locked, and every other test touching that table dies on timeout. That is the classic cause of \"40 tests all failed at once last night\" — none of the tests were broken; one of them never released a lock. The inverse is a technique in its own right: run the test inside a transaction and `ROLLBACK` at the end, and the database is restored byte-for-byte to its pre-test state."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "📚",
        "content": {
          "tr": "Index'i kitabın arkasındaki dizin gibi düşün: 500 sayfayı baştan sona taramak yerine dizine bakar, 'checkout → sayfa 287' dersin. VIEW ise bambaşka bir şeydir — kitabın kendisine değil, sık okuduğun bölüme koyduğun bir **ayraç**tır: içeriği çoğaltmaz, sadece oraya ulaşmanın kısa yolunu saklar.\n\nPeki index bu kadar iyiyse, neden veritabanı her sütuna otomatik index koymuyor? Cevap bedelde: index okumayı hızlandırırken **yazmayı yavaşlatır**. Her INSERT/UPDATE, tablonun yanı sıra o tablodaki bütün index ağaçlarını da güncellemek zorundadır — kitaba yeni bir paragraf eklerken arkadaki dizini de elden geçirmek gibi. Beş index'li bir tabloya toplu veri yüklemek, index'sizine göre kat kat yavaştır. Bu yüzden index bir 'ne kadar çok o kadar iyi' ayarı değil, okuma/yazma dengesi kararıdır.\n\nJava'da bu takas tanıdıktır: `HashMap` aramada `ArrayList`'i açık ara döver, ama karşılığında bellek harcar ve her `put` için hash hesaplar. Index de aynı fikrin diskteki karşılığıdır — bedava hız yoktur, sadece yer değiştirmiş maliyet vardır.\n\nQA açısından iki somut yüzü var. Birincisi: test verisi hazırlarken 100 bin satır INSERT eden setup script'in beklenmedik biçimde dakikalarca sürüyorsa, şüpheleneceğin ilk şey hedef tablodaki index sayısıdır. İkincisi ve daha kritiği: performans testinde staging veritabanında olup production'da olmayan (ya da tersi) bir index, senin 'sorgu 40 ms' ölçümünü canlıda 4 saniyeye çevirir — yani test ortamıyla production'ın index şeması aynı değilse, ölçtüğün performans gerçek değildir. VIEW ise QA raporlarında tekrar eden o 6 tablolu JOIN'i tek isme indirip herkesin aynı tanımı kullanmasını sağlar; 'benim raporumda 12, seninkinde 9 bug çıkıyor' tartışmalarının çoğu farklı JOIN yazımından doğar.",
          "en": "Think of an index as the index at the back of a book: instead of scanning all 500 pages you look it up — \"checkout → page 287\". A VIEW is a different animal altogether: not the book, but a **bookmark** on a section you keep returning to. It duplicates no content; it only stores the shortcut to getting there.\n\nSo if indexes are that good, why doesn't the database just index every column automatically? The answer is in the price: an index speeds up reads and **slows down writes**. Every INSERT/UPDATE must now update the table plus every index tree on it — like revising the back-of-book index each time you add a paragraph. Bulk-loading into a table with five indexes is many times slower than into one with none. An index is not a \"more is better\" dial; it is a read/write trade-off decision.\n\nThe trade is familiar from Java: `HashMap` crushes `ArrayList` on lookup, but pays for it in memory and in hashing on every `put`. An index is the same idea projected onto disk — there is no free speed, only relocated cost.\n\nFor QA this has two concrete faces. First: when a setup script inserting 100k rows of test data unexpectedly takes minutes, the index count on the target table is the first thing to suspect. Second and more dangerous: in performance testing, an index that exists on staging but not in production (or the reverse) turns your \"40 ms query\" into 4 seconds on live — if the index schema of your test environment doesn't match production, the performance you measured isn't real. VIEWs, meanwhile, collapse that recurring 6-table JOIN in QA reports into a single name everyone shares; most \"my report says 12 bugs, yours says 9\" arguments come from two people writing the JOIN differently."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🔒",
        "content": {
          "tr": "SQL Injection'ı, bir sekretere telefonda talimat dikte etmek gibi düşün. Sekretere dersin ki: 'Şu kişiye dosyayı gönder — adı: Ahmet'. Karşı taraf ismini söylerken 'Ahmet, ve bu arada bütün arşivi de faksla' derse, sekreter bunun ismin bir parçası mı yoksa yeni bir emir mi olduğunu ayırt edemez; çünkü ikisi de aynı ses kanalından, aynı cümlenin içinde gelmiştir. Veritabanı da string birleştirmeyle kurulmuş bir sorguda tam olarak bu sağırlıktadır: `' OR '1'='1` yazan kullanıcının girdisi, artık veri değil **komut** olur.\n\nPeki neden bu sorun 'tehlikeli karakterleri temizleyerek' çözülmüyor — tırnak işaretlerini filtrelemek yetmez mi? Yetmediği yıllarca acı biçimde öğrenildi: kara liste her zaman eksiktir (encoding varyasyonları, Unicode benzerleri, çok baytlı karakter setleri, yorum işaretleri). Asıl çözüm karakterleri kovalamak değil, **kanalı ayırmaktır**: parametreli sorguda cümlenin iskeleti veritabanına önce ve tek başına gider, kullanıcı verisi sonra ayrı bir kutuda ulaşır. Veri hangi karakteri içerirse içersin artık komut olma ihtimali yoktur — çünkü komutun okunacağı an çoktan geçmiştir.\n\nJava'daki `PreparedStatement` tam olarak bunun için vardır ve fark tek satırdadır: `\"... WHERE user='\" + name + \"'\"` savunmasızdır, `\"... WHERE user=?\"` + `setString(1, name)` ise yapısal olarak bağışıktır. Python'da `cursor.execute(sql, (name,))`, aynı fikir.\n\nQA açısından burada net bir sorumluluk var: bu, 'güvenlik ekibinin işi' diye devredilebilecek bir konu değil, her login/arama/filtre alanında koşulması gereken bir negatif test senaryosudur. `' OR '1'='1' --` girdisiyle giriş yapılabiliyorsa bu bir bug değil, incident'tır; ve bu tür açıklar production'a çıktıktan sonra değil, test aşamasında yakalandığında maliyeti binde birdir. Bir de şu kritik kural: injection testleri asla canlı veri üzerinde değil, izole bir test veritabanında koşulur.",
          "en": "Think of SQL Injection as dictating an instruction to a receptionist over the phone. You say: \"send the file to this person — name: Ahmet.\" If the caller pronounces their name as \"Ahmet, and by the way also fax over the entire archive,\" the receptionist has no way to tell where the name ended and a new order began — both arrived through the same channel, inside the same sentence. A query built by string concatenation is exactly that deaf: input like `' OR '1'='1` stops being data and becomes a **command**.\n\nSo why isn't this solved by \"sanitizing dangerous characters\" — isn't filtering quotes enough? The industry learned the hard way that it isn't: a blacklist is always incomplete (encoding variants, Unicode look-alikes, multi-byte charsets, comment markers). The real fix is not chasing characters but **separating the channels**: with a parameterized query the skeleton of the statement reaches the database first, alone, and the user's data arrives afterwards in its own box. Whatever characters that data contains, it can no longer become a command — the moment for reading commands has already passed.\n\nJava's `PreparedStatement` exists for precisely this, and the difference is one line: `\"... WHERE user='\" + name + \"'\"` is defenceless, while `\"... WHERE user=?\"` plus `setString(1, name)` is structurally immune. Python's `cursor.execute(sql, (name,))` is the same idea.\n\nFor QA there is a clear ownership claim here: this is not something to hand off as \"the security team's job\" — it is a negative test case that belongs on every login, search and filter field you own. If `' OR '1'='1' --` logs you in, that is not a bug, it is an incident; and caught in test rather than after release, it costs a thousandth as much. One hard rule though: injection tests are run against an isolated test database, never against live data."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🧪",
        "content": {
          "tr": "QA'de SQL'i, bir restoran müfettişinin mutfağa girme yetkisi gibi düşün. Müşteri masasında yemek güzel görünüyor olabilir, garson 'her şey yolunda' diyor olabilir — müfettiş yine de arkaya geçip buzdolabının sıcaklığına bakar. UI senin masan, veritabanı ise mutfaktır: ekranda 'Ödeme başarılı ✅' yazması, `orders` tablosuna gerçekten `status='PAID'` yazıldığı anlamına gelmez.\n\nPeki UI zaten doğru mesajı gösteriyorsa, veritabanına bakmak fazladan bir titizlik değil mi? Tam tersine — en pahalı bug'lar tam olarak bu boşlukta yaşar. Ekran başarı gösterip kayıt yarım yazıldığında testin YEŞİL yanar, kullanıcı da sipariş verdiğini sanır; hata haftalar sonra muhasebe mutabakatında ortaya çıkar. UI assertion'ı 'kullanıcıya ne söylendiğini' doğrular, SQL assertion'ı 'gerçekte ne olduğunu'. İkisi ayrı sorulardır ve sadece birincisini sormak, testin kapsamını sandığından çok daha dar bırakır.\n\nJava tarafındaki karşılığı tanıdıktır: Selenium veya REST Assured ile response'u doğrularsın, ardından JDBC ile aynı kaydı veritabanından çekip `assertEquals` yaparsın. Fark şu ki HTTP response uygulamanın **iddiası**, veritabanı satırı ise **sonucu**dur.\n\nSQL bu işte iki uçta durur: testten ÖNCE INSERT/UPDATE ile bilinen bir başlangıç durumu kurarsın (böylece test rastgele production verisine bağımlı olmaz — flaky testlerin bir numaralı sebebi budur), testten SONRA SELECT ile gerçekten ne yazıldığını doğrularsın. Bir de üçüncü an vardır: bug araştırırken. 'Ara sıra kırılıyor' diyen bir raporu, `SELECT ... WHERE created_at BETWEEN ...` ile zaman aralığına oturttuğunda çoğu zaman kırılmanın deploy saatiyle çakıştığını görürsün.",
          "en": "Think of SQL in QA as a restaurant inspector's right to walk into the kitchen. The plate at the table may look perfect and the waiter may insist everything is fine — the inspector still goes behind the counter and checks the fridge temperature. The UI is your table; the database is the kitchen. \"Payment successful ✅\" on screen does not mean `status='PAID'` was actually written to the `orders` table.\n\nSo if the UI already shows the right message, isn't checking the database just extra fussiness? Quite the opposite — the most expensive bugs live precisely in that gap. When the screen reports success but the record is written half-way, your test goes GREEN and the customer believes the order exists; the problem surfaces weeks later during financial reconciliation. A UI assertion verifies *what the user was told*; a SQL assertion verifies *what actually happened*. Those are two different questions, and asking only the first leaves your test far narrower than you think.\n\nThe Java parallel is familiar: verify the response with Selenium or REST Assured, then pull the same record over JDBC and `assertEquals` on it. The difference being that the HTTP response is the application's **claim**, while the database row is the **outcome**.\n\nSQL sits at both ends of this job: BEFORE the test you INSERT/UPDATE a known starting state (so the test never depends on whatever random production data happens to exist — the number one source of flaky tests), and AFTER the test you SELECT to confirm what was really written. There is a third moment too: bug investigation. Pin a vague \"it breaks sometimes\" report to a time range with `SELECT ... WHERE created_at BETWEEN ...` and you will often find the breakage lines up exactly with a deploy."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        "type": "simple-box",
        "emoji": "🔗",
        "content": "Think of the database ecosystem as a vehicle fleet: SQLite is a folding bicycle — it fits in your bag, goes anywhere, needs no setup; PostgreSQL is the articulated lorry — it demands a loading dock, a licence and maintenance, but it carries the heavy work; MySQL sits between them, the van under most web applications.\n\nSo if they all speak SQL, why does the choice matter at all — what happens if you test on one and ship on another? This is exactly where it bites: every engine layers its own dialect on top of standard SQL. Auto-incrementing keys are `AUTOINCREMENT` in SQLite, `SERIAL`/`IDENTITY` in PostgreSQL, `AUTO_INCREMENT` in MySQL; date functions, JSON support, even whether an empty string is distinct from NULL, all vary. Run your tests on SQLite while the application runs on PostgreSQL, and green tests can still break in production — because what you measured was not the application's behaviour, but the intersection of two engines' behaviour.\n\nThe Java parallel is immediate: each engine ships a JDBC driver, and the only thing that changes in `DriverManager.getConnection(url, ...)` is the URL — but that \"same interface\" comfort does not mean there is no dialect underneath. ORM layers (Hibernate, SQLAlchemy, Prisma) try to hide the difference, and the hardest bugs are born exactly where they fail to.\n\nThe practical QA rule: SQLite is acceptable at the fast smoke/unit layer, but integration and regression tests should run on the SAME engine as production, ideally the same major version. The cheap way to get that is well established — spin up a real PostgreSQL as a Docker testcontainer."
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        "type": "simple-box",
        "emoji": "🚨",
        "content": "Think of database error messages as the warning lights on a car's dashboard. The light does not tell you what is broken — it tells you where to look. When the oil-pressure light comes on, the problem is not the light; the mistake is ignoring it and driving on.\n\nSo if the message already states the problem, why is root-cause analysis a separate job? Because the database reports the **symptom**, not the **cause**. `FOREIGN KEY constraint failed` says \"you referenced a row that doesn't exist\" — but the real cause is usually a parent record your own test deleted, or two tests sharing an ID. `database is locked` says \"someone else is holding it\"; the real cause is a setup function that forgot to `COMMIT`. Fix the symptom (drop the constraint, raise the timeout) and the error goes quiet while the cause grows — until it detonates in production instead.\n\nYou already have this reflex in Java: seeing a `NullPointerException`, you don't just add a null check, you read the stack trace upward asking why that value was null. Do the same with database errors; the difference is that SQL gives you no stack trace — your evidence is the query itself, the affected row count, and whether a transaction was left open.\n\nFor QA this section targets the nastiest class of failure in the field: the mistakes that raise NO error at all. `UNIQUE constraint failed` at least shouts; an UPDATE that silently touches 0 rows says nothing and your test goes green. Hence the rule: don't settle for catching exceptions — assert on the affected row count too."
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        "type": "simple-box",
        "emoji": "☕",
        "content": "Think of JDBC as an interpreter standing between two people who speak different languages. Java speaks in objects (a `TestResult` instance, `List<T>`, type safety); SQL speaks in rows and columns. JDBC sits in the middle, taking a sentence from one side and rendering it for the other — and as with every interpretation, some nuance is lost in transit.\n\nSo if there's an interpreter, why learn SQL at all — can't we stay on the Java side with an ORM? You can, right up until something gets slow. An engineer who cannot read the SQL their ORM generates will not spot the N+1 problem firing 101 queries for 100 records; it raises no error, the page just gets slower. If you don't understand what the interpreter is saying, you cannot tell when it mistranslates.\n\nThe mappings between the two worlds are familiar but not one-to-one: `Connection` = a session, `PreparedStatement` = a parameterized query (and a structural defence against SQL Injection), `ResultSet` = a cursored result set, `conn.setAutoCommit(false)` + `commit()`/`rollback()` = transaction boundaries. The most commonly missed difference is resource management: Java's GC collects objects, but it does not collect a `Connection` or `ResultSet` you left open — unclosed by `try-with-resources`, they drain the connection pool until the application starts reporting \"cannot connect to database\" while the database is perfectly healthy.\n\nFor a QA engineer writing Selenium or REST Assured tests in Java, this section is a daily reference: you prepare data over JDBC during setup, then during assertion you compare what the UI/API claims against what the database actually holds."
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🏋️",
        "content": {
          "tr": "SQL öğrenmek, yüzmeyi kitaptan öğrenmek gibi değil — suya girmek gerekir. Kitap sana kulaç tekniğini anlatabilir, ama nefesini ne zaman alacağını ancak bir kez yutkunduğunda öğrenirsin. Editörü aç, sorguyu çalıştır, hata al, düzelt.\n\nPeki neden okuyarak öğrenmek burada özellikle yetersiz kalıyor — sözdizimi bu kadar basitken? Çünkü SQL'in zorluğu sözdiziminde değil, **sessiz yanlışlarında**dır. Yanlış yazılmış bir Java kodu derlenmez, ekrana kırmızı bir hata basar; yanlış yazılmış bir SQL sorgusu ise çoğu zaman gayet düzgün çalışır ve sana YANLIŞ bir tablo döndürür. JOIN koşulunu unuttuğunda hata almazsın — 200 satır yerine 40.000 satır alırsın. `WHERE x = NULL` yazdığında hata almazsın — sıfır satır alırsın. Bu tür hataları kitap okuyarak değil, ancak beklediğin sonuçla gerçek sonucu yan yana koyarak fark edersin.\n\nJava'da derleyici senin ilk gözden geçireninidir; SQL'de o rol boştur ve o boşluğu doldurması gereken kişi sensin. Bu yüzden alışkanlık edin: yıkıcı bir sorgu yazmadan önce aynı WHERE ile bir SELECT koş, dönen satır sayısına bak.\n\nQA açısından bu pratik yapma biçimi doğrudan işe dönüşür: mülakatta 'şu sorguyu yaz' dendiğinde ezberden değil, kafanda çalıştırarak cevap verirsin; sahada ise pipeline'ı bekletmeden bug'ın kök nedenini bulan kişi olursun. Her alıştırmayı önce kendin çözmeye çalış, çözümü ancak kendi cevabını yazdıktan sonra aç.",
          "en": "Learning SQL from reading alone is like learning to swim from a book — you have to get in the water. A book can describe the stroke, but you only learn when to breathe after you have swallowed once. Open the editor, run the query, hit the error, fix it.\n\nSo why is reading especially insufficient here, when the syntax is this simple? Because SQL's difficulty is not in its syntax but in its **silent wrongness**. Bad Java code refuses to compile and prints a red error; a bad SQL query usually runs perfectly and hands you the WRONG table. Forget a join condition and you get no error — you get 40,000 rows instead of 200. Write `WHERE x = NULL` and you get no error — you get zero rows. You do not catch that class of mistake by reading; you catch it by putting the result you expected next to the result you got.\n\nIn Java the compiler is your first reviewer; in SQL that role is vacant, and you are the one who has to fill it. So build the habit: before any destructive statement, run a SELECT with the same WHERE and look at the row count.\n\nFor QA this way of practising converts directly into work: when an interview asks you to \"write this query\", you answer by running it in your head rather than reciting it; on the job you become the person who finds the root cause without holding up the pipeline. Try every exercise yourself first, and only open the solution once your own answer is written."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "🛠️",
        "content": {
          "tr": "DBeaver'ı veritabanının dosya yöneticisi gibi düşün. Terminalden `psql` ile bağlanmak, klasörleri `dir`/`ls` komutlarıyla gezmeye benzer: güçlüdür ama her adımda ne aradığını önceden bilmen gerekir. DBeaver ise pencereyi açar — tabloları soldaki ağaçta görürsün, çift tıklarsın, veriyi Excel gibi tarar, sütuna sağ tıklayıp sıralarsın.\n\nPeki komut satırı zaten her şeyi yapabiliyorken görsel bir araca neden ihtiyaç var — bu tembellik değil mi? Asıl fark hız değil, **keşif**tir. Şemayı bilmediğin bir veritabanına düştüğünde, `psql`'de doğru soruyu sorabilmek için önce tabloların adını, sütunlarını ve foreign key ilişkilerini bilmen gerekir; yani cevabı bulmak için cevabı biliyor olman gerekir. DBeaver'ın ER diyagramı bu kısır döngüyü kırar: hangi tablonun hangisine bağlandığını tek bakışta görürsün. Öğrenirken bu fark, saatler ile dakikalar arasındaki farktır.\n\nJava tarafından bakınca konumu nettir: IntelliJ'in Database sekmesiyle aynı işi yapar, ama tek bir IDE'ye ve tek bir projeye bağlı değildir — SQLite, PostgreSQL, MySQL, Oracle'a aynı arayüzden bağlanırsın. JDBC driver kavramı da aynen geçerlidir; DBeaver ilk bağlantıda o driver'ı senin için indirir.\n\nQA açısından en kritik kullanımı şudur: bir test kırıldığında, otomasyon kodunu değiştirmeden veritabanına bakıp 'kayıt gerçekten yazılmış mı, hangi status ile yazılmış' sorusunu saniyeler içinde cevaplarsın. Ama tam da bu kolaylık yüzünden bir kural koy: production bağlantısını mümkünse read-only kullanıcıyla ekle. Görsel araçta yanlışlıkla çalıştırılan WHERE'siz bir UPDATE, terminaldekiyle aynı yıkımı yapar — sadece daha kolay tıklanır.",
          "en": "Think of DBeaver as a file manager for your database. Connecting through `psql` in a terminal is like browsing folders with `dir`/`ls`: powerful, but it assumes you already know what you're looking for at every step. DBeaver opens a window instead — tables appear in a tree on the left, you double-click one, scan the data like a spreadsheet, right-click a column to sort.\n\nSo if the command line can already do everything, why use a GUI at all — isn't that just laziness? The real difference isn't speed, it's **discovery**. Dropped into a database whose schema you don't know, `psql` requires you to know the table names, columns and foreign keys before you can ask the right question — you must already know the answer to find the answer. DBeaver's ER diagram breaks that loop: you see at a glance what links to what. While learning, that difference is the difference between hours and minutes.\n\nFrom the Java side its place is clear: it does the same job as IntelliJ's Database tab, but it isn't tied to one IDE or one project — SQLite, PostgreSQL, MySQL and Oracle all connect through the same interface. The JDBC driver concept carries over unchanged; DBeaver downloads that driver for you on first connection.\n\nFor QA the critical use is this: when a test fails, you answer \"was the record actually written, and with what status?\" in seconds, without touching the automation code. But precisely because it is that easy, set yourself a rule: add production connections through a read-only user wherever possible. A WHERE-less UPDATE run by accident in a GUI does exactly the same damage as in a terminal — it is just easier to click."
        }
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        ...
        "emoji": "💼",
        "content": {
          "tr": "SQL mülakatını, ehliyet sınavının direksiyon kısmı gibi düşün. Teorik sınavda 'kırmızı ışık ne demektir?' diye sorulur; direksiyonda ise seni yağmurlu bir günde dar bir sokağa sokup park ettirirler. İkincisini ezberle geçemezsin, çünkü sorulan şey bilgi değil, bilgiyi baskı altında kullanabilme becerisidir.\n\nPeki tanımları bilmek neden yetmiyor — 'INNER JOIN ile LEFT JOIN farkını' doğru anlatan biri bu işi yapamaz mı? Mülakatı yapan kişinin gerçekte öğrenmeye çalıştığı şu: sen bir sorunla karşılaştığında nereden başlıyorsun? 'LEFT JOIN sol tabloyu korur' cümlesini herkes ezberleyebilir; ama 'raporda 12 testçi görünmesi gerekirken 9 görünüyor' dendiğinde ilk refleksi JOIN tipini sorgulamak olan kişi azdır. Bu yüzden sorular giderek senaryolaşır: 'gece koşumunda 400 test FAIL oldu, veritabanına bakarak nereden başlarsın?'\n\nJava mülakatlarındaki dengi tanıdıktır: `HashMap` nasıl çalışır sorusundan çok, 'bu koda `ConcurrentModificationException` neden düşüyor' sorusu ayırt edicidir. Her iki dilde de ayırt edici olan tanım değil, hata karşısındaki düşünce sırasıdır.\n\nBu bölümdeki sorular bu yüzden senaryo tabanlıdır ve cevaplarında düşünce zincirini görürsün: hangi sorguyu neden yazdın, dönen sonucu nasıl yorumladın, bir sonraki adımda neyi elediniz. Kendi cevabını yazmadan çözümü açma — mülakattaki gerçek fark, cevabı bilmekle cevabı üretebilmek arasındadır.",
          "en": "Think of a SQL interview as the driving part of a licence exam. The written test asks \"what does a red light mean?\"; the driving test puts you in a narrow street in the rain and asks you to park. You cannot pass the second one by memorisation, because what is being tested is not knowledge but the ability to use knowledge under pressure.\n\nSo why aren't definitions enough — can't someone who correctly explains \"the difference between INNER JOIN and LEFT JOIN\" do the job? What the interviewer is actually trying to learn is: where do you start when something is wrong? Anyone can memorise \"LEFT JOIN preserves the left table\"; far fewer people, told that \"the report shows 9 testers where it should show 12\", reach for the join type as their first instinct. That is why the questions drift toward scenarios: \"400 tests failed in last night's run — starting from the database, where do you look?\"\n\nThe Java equivalent is familiar: \"how does `HashMap` work\" separates fewer candidates than \"why does this code throw `ConcurrentModificationException`\". In both languages the discriminator is not the definition, it is the order in which you think when facing a failure.\n\nThe questions in this section are therefore scenario-based, and their answers show the chain of reasoning: which query you wrote and why, how you read the result, what you eliminated next. Don't open the solution before writing your own answer — in a real interview the gap that matters is between knowing the answer and being able to produce it."
        }
      },
      {
        "type": "text",
    ...
    "blocks": [
      {
        "type": "simple-box",
        "emoji": "🔗",
        "content": "Veritabanı ekosistemini bir araç filosu gibi düşün: SQLite bir katlanır bisiklettir — çantana koyar, her yere götürürsün, kurulum istemez; PostgreSQL ise TIR'dır — yükleme rampası, sürücü belgesi ve bakım ister ama ağır işi o taşır; MySQL ikisinin arasında, çoğu web uygulamasının altındaki minibüstür.\n\nPeki hepsi SQL konuşuyorsa, hangisini kullandığın gerçekten fark eder mi? Sorunun bam teli burada: standart SQL'in üzerine her motor kendi lehçesini ekler. SQLite'ta `AUTOINCREMENT`, PostgreSQL'de `SERIAL`/`IDENTITY`, MySQL'de `AUTO_INCREMENT`'tır; tarih fonksiyonları, JSON desteği ve hatta boş string ile NULL'ın ayrımı motordan motora değişir. Testlerini SQLite üzerinde koşup uygulamayı PostgreSQL'de çalıştırdığında, yeşil geçen testler production'da kırılabilir — çünkü ölçtüğün şey uygulamanın davranışı değil, iki farklı motorun davranışının kesişimidir.\n\nJava tarafındaki karşılığı doğrudan tanıdıktır: her motorun bir JDBC driver'ı vardır ve `DriverManager.getConnection(url, ...)` çağrısında değişen tek şey URL'dir — ama bu 'aynı arayüz' rahatlığı, altta lehçe farkı olmadığı anlamına gelmez. ORM katmanları (Hibernate, SQLAlchemy, Prisma) bu farkı gizlemeye çalışır; gizlemeyi başaramadığı yerde de en zor bug'lar orada doğar.\n\nQA için pratik kural şudur: hız gereken smoke/unit seviyesinde SQLite kabul edilebilir, ama entegrasyon ve regression testleri production ile AYNI motor ve mümkünse aynı major sürüm üzerinde koşmalıdır. Bunu ucuza sağlamanın yolu bellidir: Docker ile testcontainer olarak gerçek PostgreSQL ayağa kaldırmak."
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        "type": "simple-box",
        "emoji": "🚨",
        "content": "Veritabanı hata mesajlarını, arabanın gösterge panelindeki uyarı ışıkları gibi düşün. Işık sana arızayı söylemez — nereye bakman gerektiğini söyler. 'Yağ basıncı' ışığı yandığında sorun ışıkta değildir; yanlış yapılan şey, ışığı görmezden gelip yola devam etmektir.\n\nPeki hata mesajı zaten sorunu yazıyorsa, neden kök neden aramak diye ayrı bir iş var? Çünkü veritabanı sana **belirtiyi** söyler, **sebebi** değil. `FOREIGN KEY constraint failed` mesajı 'olmayan bir kayda referans verdin' der; ama bunun gerçek sebebi çoğu zaman testin sildiği bir parent kaydı, ya da iki testin aynı ID'yi paylaşmasıdır. `database is locked` mesajı 'başkası tutuyor' der; gerçek sebep ise `COMMIT` çağırmayı unutmuş bir setup fonksiyonudur. Belirtiyi düzeltirsen (constraint'i kaldırmak, timeout'u artırmak) hata susar ama sebep büyür — ve bir sonraki sefer production'da patlar.\n\nJava'da bu refleks zaten var: `NullPointerException` gördüğünde null kontrolü eklemekle yetinmez, 'bu değer neden null geldi' diye stack trace'i yukarı doğru okursun. Veritabanı hatalarında da aynısını yap; fark şu ki SQL'de stack trace yoktur — onun yerine sorgunun kendisi, etkilenen satır sayısı ve transaction'ın açık kalıp kalmadığı senin izlerindir.\n\nQA açısından bu bölüm doğrudan sahadaki en sinsi hata sınıfını hedefler: hata VERMEYEN yanlışlar. `UNIQUE constraint failed` en azından bağırır; sessizce 0 satır güncelleyen bir UPDATE ise hiçbir şey söylemez ve testin yeşil yanar. Bu yüzden kural: yalnızca exception yakalamakla yetinme, etkilenen satır sayısını da assert et."
      },
      {
        "type": "heading",
    ...
    "blocks": [
      {
        "type": "simple-box",
        "emoji": "☕",
        "content": "JDBC'yi, iki farklı dili konuşan iki kişi arasındaki tercüman gibi düşün. Java nesnelerle konuşur (`TestResult` nesnesi, `List<T>`, tip güvenliği); SQL ise satır ve sütunlarla. JDBC arada durur, cümleyi bir yandan alır öbür yana çevirir — ve her tercümede olduğu gibi, bazı nüanslar yolda kaybolur.\n\nPeki tercüman varsa neden SQL öğrenmek zorundayız — ORM kullanıp Java tarafında kalamaz mıyız? Kalabilirsin, taa ki bir şey yavaşlayana kadar. ORM'in ürettiği sorguyu okuyamayan bir mühendis, 100 kayıt için 101 sorgu atan N+1 problemini fark edemez; hata mesajı da vermez, sadece sayfa yavaşlar. Tercümanın ne dediğini anlamıyorsan, yanlış çevirdiğini de anlayamazsın.\n\nİki dünya arasındaki eşleşmeler tanıdıktır ama birebir değildir: `Connection` = oturum, `PreparedStatement` = parametreli sorgu (ve SQL Injection'a karşı yapısal savunma), `ResultSet` = imleçli sonuç kümesi, `conn.setAutoCommit(false)` + `commit()`/`rollback()` = transaction sınırları. En sık atlanan fark ise kaynak yönetimidir: Java'da GC nesneleri toplar, ama açık kalan bir `Connection` veya `ResultSet`'i toplamaz — bunlar `try-with-resources` ile kapatılmazsa connection pool tükenir ve uygulama, veritabanı ayaktayken 'veritabanına bağlanılamıyor' demeye başlar.\n\nJava ile Selenium veya REST Assured testleri yazan bir QA mühendisi için bu bölüm günlük başvuru kılavuzudur: test setup'ında veriyi JDBC ile hazırlar, assertion aşamasında UI/API'nin iddiasını veritabanındaki gerçekle karşılaştırırsın."
      },
      {
        "type": "heading",
