...
const sections = [
  // ── 0. INTRO & WHY ──────────────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      {
        type: 'heading',
        text: { tr: 'QA Mühendisi Nasıl Olunur?', en: 'How Do You Become a QA Engineer?' }
      },
      {
        type: 'simple-box',
        emoji: '🕵️',
        content: {
          tr: "QA mühendisi olmak, bina denetçisi olmaya benzer — mimar (geliştirici) değil. Mimar, doğru olduğuna inandığı bir plana göre tasarlar ve inşa eder; denetçinin TÜM işi, planın bir yerde başarısız olduğunu VARSAYIP o yeri bulmaktır. Bir geliştirici kod yazarken \"bu çalışıyor mu?\" diye sorar — kendi mantığını kontrol eder. QA mühendisi ise \"bu HANGİ koşullarda çalışmıyor?\" diye sorar — kendi mantığına karşı aktif olarak çalışır. Bu, aynı becerinin zayıf bir versiyonu değil, gerçekten farklı bir zihinsel alışkanlıktır.\n\nDüşündürücü soru: geliştiriciler zaten kendi kodlarını test ediyorken (unit test yazıyorken), neden ayrı bir QA rolüne ihtiyaç var? Çünkü bir kişinin kendi varsayımlarındaki köşe durumu (edge case) yine kendi kör noktasıdır — kodu yazan kişi, o kodun NEREDE yanlış gideceğini genelde göremez, çünkü zaten \"doğru\" düşündüğü şekilde yazmıştır.\n\nKarşılaştır: bir geliştirici \"kullanıcı doğru e-posta girerse ne olur?\" sorusuyla ilerler; bir test mühendisi aynı forma \"boş bırakırsam, 300 karakter yapıştırırsam, emoji girersem, aynı anda iki kere gönder'e basarsam ne olur?\" diye yaklaşır. İkisi de mühendislik ama farklı bir hipotez kurma biçimi.\n\nQA açısından somut örnek: Knight Capital'in 45 dakikada 440 milyon dolar kaybettiği olayda kod \"çalışıyordu\" — geliştiricinin sorduğu soruya doğru cevap veriyordu. Kayıp, kimsenin \"eski bir bayrak yanlışlıkla yeniden açılırsa ne olur?\" sorusunu sormamış olmasından geldi. Bu soruyu sormak, tam olarak QA'in işidir.",
          en: "Becoming a QA engineer is like becoming a building inspector rather than the architect. The architect (developer) designs and builds according to a plan they believe is correct; the inspector's entire job is to ASSUME the plan has failed somewhere and go find where. A developer writing code asks \"does this work?\" — checking their own reasoning. A QA engineer asks \"under which conditions does this NOT work?\" — actively working against their own reasoning. That is not a weaker version of the same skill; it is a genuinely different mental habit.\n\nA question worth pausing on: developers already test their own code with unit tests, so why does a separate QA role exist at all? Because a person's own blind spot around edge cases stays their own blind spot — the person who wrote the code usually cannot see where it will break, precisely because they wrote it believing it was correct.\n\nCompare: a developer moves forward with \"what happens if the user enters a valid email?\" A test engineer approaches the same form with \"what if I leave it empty, paste 300 characters, type an emoji, or click submit twice at once?\" Both are engineering, just a different way of forming a hypothesis.\n\nThe concrete QA stakes: in the incident where Knight Capital lost $440 million in 45 minutes, the code \"worked\" — it correctly answered the question the developer had asked. The loss came from nobody asking \"what happens if an old flag gets accidentally re-enabled?\" Asking exactly that question is the job of QA."
        }
      },
      {
        type: 'text',
        content: {
          tr: "Java'da sertifikalı bir öğrenme yolun vardır: Syntax → OOP → Collections → Generics → Concurrency; her katman bir öncekine oturur, atlarsan üstündeki anlamsızlaşır. Test otomasyonunda da aynı hiyerarşi geçerlidir: **Temel Test Kavramları** (bu sayfadaki QA/QC farkı, test türleri, SDLC) → **Programlama Dili** (Java veya Python/TypeScript) → **Test Framework'ü** (pytest, JUnit gibi) → **Otomasyon Aracı** (Selenium veya Playwright) → **CI/CD Entegrasyonu** (Jenkins, GitHub Actions). Bu sırayı atlayıp doğrudan \"Selenium öğreneyim\" diyen biri, birkaç hafta sonra locator yazmayı değil, Java'nın kendisini öğrenmesi gerektiğini keşfeder.\n\nManuel testten otomasyona geçiş bir sıfırlama değil, bir üst katman eklemektir: manuel test sırasında geliştirdiğin \"bu senaryo neyi kırar?\" sezgisi, otomasyonda HANGİ testin yazılmaya değer olduğuna karar verirken doğrudan işine yarar — bu sezgiyi olmayan biri, teknik olarak çalışan ama hiçbir gerçek riski yakalamayan testler yazar.\n\nİlk işe girerken beklenen seviye genelde şudur: temel bir programlama dili bilgisi, bir otomasyon aracıyla basit bir test senaryosu yazabilme, SQL ile veri doğrulayabilme ve bir bug'ı net biçimde raporlayabilme. Uzmanlık (framework mimarisi kurmak, CI pipeline'ı yönetmek) işe girdikten SONRA, gerçek bir kod tabanı üzerinde gelişir — bunu öğrenmeden işe giremeyeceğini düşünmek yaygın ama yanlış bir varsayımdır.",
          en: "In Java you have a certified learning path: Syntax → OOP → Collections → Generics → Concurrency; each layer builds on the last, and skipping one makes the next meaningless. The same hierarchy applies to test automation: **Basic Testing Concepts** (the QA vs QC distinction, testing types, SDLC covered on this page) → **Programming Language** (Java or Python/TypeScript) → **Test Framework** (pytest, JUnit) → **Automation Tool** (Selenium or Playwright) → **CI/CD Integration** (Jenkins, GitHub Actions). Someone who skips this order and jumps straight to \"let me learn Selenium\" usually discovers a few weeks in that what they actually need to learn is Java itself, not locators.\n\nMoving from manual testing to automation is not a reset, it is adding a layer on top: the \"what breaks this scenario?\" instinct you build during manual testing directly feeds into deciding WHICH tests are worth automating — someone without that instinct writes tests that technically pass but catch no real risk.\n\nThe level expected at a first job is usually this: basic programming language knowledge, the ability to write a simple test scenario with an automation tool, validating data with SQL, and reporting a bug clearly. Expertise — building framework architecture, running a CI pipeline — develops AFTER you're hired, on a real codebase. Assuming you can't get hired without already knowing that is a common but wrong assumption."
        }
      },
      {
        ...
        items: [
          ...
          {
            q: { tr: 'QA mühendisi nasıl olunur?', en: 'How do you become a QA engineer?' },
            a: {
              tr: 'Sırayla ilerle: önce temel test kavramları (QA/QC farkı, test türleri), sonra bir programlama dili (Java veya Python/TypeScript), sonra bir test framework\'ü, sonra bir otomasyon aracı (Selenium/Playwright), en son CI/CD entegrasyonu. Manuel test deneyimi bir dezavantaj değil — "bu senaryo neyi kırar?" sezgisi otomasyonda hangi testin yazılmaya değer olduğuna karar vermeni sağlar.',
              en: 'Progress in order: first basic testing concepts (QA vs QC, testing types), then a programming language (Java or Python/TypeScript), then a test framework, then an automation tool (Selenium/Playwright), and CI/CD integration last. Manual testing experience is not a disadvantage — the "what breaks this scenario?" instinct is exactly what helps you decide which tests are worth automating.',
            },
          },
          {
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          { icon: '🗂️', route: '/qa-shop-backlog', label: { tr: 'Backlog: Gereksinimden Test\'e', en: 'Backlog: From Requirement to Test' }, desc: { tr: 'Gerçek bir şirketteki zincir: iş gereksinimi, epic, business story ve ayrı frontend/backend story\'leri.', en: 'The chain as it works in a real company: business requirement, epic, business story and separate frontend and backend stories.' } },
