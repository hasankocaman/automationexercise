    ...
    blocks: [
      ...
      playwrightInterviewPractice,
      {
        ...
        questions: [
          ...
          { level: 'intermediate', q: 'Regresyon suite\'inde 40 testten 3\'ü rastgele "element not found" ile kırılıyor ama elle koşunca hepsi geçiyor. Kök nedeni nasıl bulursun?', a: 'Önce paralel worker sayısını 1\'e indirip tekrar koştur — eğer geçiyorsa sorun paylaşılan state\'tir (aynı test verisini kullanan testler birbirini bozuyor), auto-wait sorunu değil. Geçmiyorsa trace viewer aç (npx playwright show-trace) ve o adımda DOM\'un gerçekte ne durumda olduğunu gör. Java\'da JUnit\'in paralel testlerde @ResourceLock ile aynı veriye erişimi sınırlaması gibi, Playwright\'ta her teste kendi test.describe.configure({ mode: "serial" }) veya izole test verisi (her test kendi kaydını oluşturur) vermek kalıcı çözümdür.' },
          { level: 'intermediate', q: 'Bir locator sayfada BİRDEN FAZLA elemente denk geliyor ve `.click()` "strict mode violation" hatası veriyor. Nasıl çözersin?', a: 'Playwright\'ın "strict mode"u kasıtlıdır — hangi elemente tıklandığı BELİRSİZSE testi durdurur, rastgele ilkini seçip sessizce yanlış elemente tıklamaz. Çözüm: locator\'ı daraltmak (page.getByRole("button", { name: "Kaydet", exact: true })) veya .first()/.nth(n) ile AÇIKÇA hangisini istediğini belirtmek. Selenium\'da findElement() sessizce ilk eşleşeni döner — bu, kaç elementin eşleştiğini asla göstermeyen gizli bir hata kaynağıdır; Playwright bunu erken ve gürültülü patlatır.' },
          { level: 'intermediate', q: 'Bir test suite\'i lokalde 2 dakikada bitiyor ama CI\'da 8 dakika sürüyor ve bazen timeout\'a takılıyor. Neyi kontrol edersin?', a: 'İlk şüpheli: CI\'daki worker sayısı ve makine kaynakları — playwright.config.ts\'te workers: process.env.CI ? 2 : undefined gibi bir ayar CI\'ı kasıtlı olarak sınırlıyor olabilir, ama bu sürenin uzamasını açıklar, timeout\'u açıklamaz. İkinci adım: CI\'nın headless ortamının lokal makineden daha yavaş donanıma sahip olması — timeout değerlerini (expect.timeout, actionTimeout) CI\'a göre biraz gevşetmek gerekebilir. Java\'da Jenkins agent\'ları arası kaynak farkının Selenium testlerini yavaşlatması ile aynı sınıf sorundur — kök neden kod değil, ortamdır.' },
          { level: 'intermediate', q: 'Bir e-ticaret sepetinde "Ürün Ekle" butonuna tıkladıktan sonra sepet sayacının güncellenmesini nasıl test edersin (sabit bekleme kullanmadan)?', a: 'Sabit bir page.waitForTimeout(2000) YAZILMAZ — bu hem gereksiz yavaşlatır hem de yavaş bir günde hâlâ flaky kalır. Doğru yaklaşım: await expect(page.getByTestId("cart-count")).toHaveText("1") — Playwright bu assertion\'ı SAYAÇ GERÇEKTEN "1" olana kadar otomatik tekrar dener (varsayılan 5s içinde). Java Selenium\'da bu, WebDriverWait.until(ExpectedConditions.textToBe(...)) ile elle yazılması gereken bir şeydir; Playwright\'ta expect() zaten bunu İÇİNDE yapar.' },
          { level: 'intermediate', q: 'Bir formda "Gönder" butonuna tıklandıktan sonra sayfa YENİLENMİYOR ama arka planda bir API isteği atılıyor (SPA). Bu isteğin başarılı olduğunu nasıl doğrularsın?', a: 'UI\'daki bir yan etkiyi (örn. "Kaydedildi" mesajı) beklemek ilk tercih, ama API\'nin GERÇEKTEN doğru veriyle çağrıldığını doğrulamak için page.waitForResponse(url => url.includes("/api/submit") && url.status() === 200) kullanılır — isteğin hem doğru endpoint\'e hem 200 ile gittiğini garanti eder. Bu, sadece UI\'ya bakmanın kaçırabileceği bir senaryoyu (buton görsel olarak tepki verdi ama istek 500 döndü) yakalar. Java REST Assured ile API\'yi ayrıca test etmek yerine, burada UI ve API doğrulaması TEK testte birleşir.' },
    ...
    blocks: [
      ...
      playwrightInterviewPractice,
      {
        ...
        questions: [
          ...
          { level: 'intermediate', q: 'In your regression suite, 3 of 40 tests fail randomly with "element not found" but pass every time when run manually. How do you find the root cause?', a: 'First rerun with a single worker — if the tests pass, the issue is shared state (tests reading/writing the same test data race each other), not auto-wait. If they still fail, open the trace viewer (npx playwright show-trace) and inspect the DOM at that exact step. Similar to how JUnit\'s @ResourceLock restricts parallel tests from touching the same resource in Java — the durable fix is either test.describe.configure({ mode: "serial" }) or isolated test data per test (each test creates its own record).' },
          { level: 'intermediate', q: 'A locator matches MULTIPLE elements on the page and `.click()` throws a "strict mode violation" error. How do you fix it?', a: 'Playwright\'s "strict mode" is intentional — it stops the test when the target is AMBIGUOUS instead of silently clicking the first match. The fix is narrowing the locator (page.getByRole("button", { name: "Save", exact: true })) or explicitly saying which one you want with .first()/.nth(n). Selenium\'s findElement() silently returns the first match with no visibility into how many elements matched — a hidden source of bugs that Playwright surfaces loudly and early instead.' },
          { level: 'intermediate', q: 'A test suite finishes in 2 minutes locally but takes 8 minutes in CI and sometimes times out. What do you check?', a: 'First suspect: worker count and CI machine resources — a setting like workers: process.env.CI ? 2 : undefined in playwright.config.ts intentionally limits CI, which explains slower runs but not timeouts on its own. Second: CI\'s headless environment often runs on weaker hardware than your laptop, so timeout values (expect.timeout, actionTimeout) may need to be relaxed for CI specifically. This is the same class of problem as Jenkins agents with different resources slowing down Selenium tests in Java — the root cause is the environment, not the code.' },
          { level: 'intermediate', q: 'How do you test that an "Add to Cart" click updates the cart counter, without using a fixed wait?', a: 'You do NOT write a fixed page.waitForTimeout(2000) — it both slows the suite down and stays flaky on a slow day anyway. The right approach is await expect(page.getByTestId("cart-count")).toHaveText("1") — Playwright automatically retries this assertion until the counter is ACTUALLY "1" (within a default 5s window). In Java Selenium this requires manually writing WebDriverWait.until(ExpectedConditions.textToBe(...)); in Playwright, expect() already does this internally.' },
          { level: 'intermediate', q: 'On a form submit the page does NOT reload but an API call fires in the background (SPA). How do you verify that call actually succeeded?', a: 'Waiting for a UI side effect (e.g. a "Saved" message) is the first choice, but to verify the API was ACTUALLY called with a 200, use page.waitForResponse(url => url.includes("/api/submit") && url.status() === 200) — it guarantees the request hit the right endpoint AND returned success. This catches a scenario a UI-only check would miss: the button visually reacted but the request actually returned a 500. Instead of testing the API separately with Java REST Assured, UI and API verification merge into ONE test here.' },
