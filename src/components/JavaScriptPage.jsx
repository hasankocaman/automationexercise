...
const THUNDER_INTERVAL_MS = 10000 // js-lightning-flash CSS animasyonuyla aynı 10s döngü

/* ─── Event Loop Pipeline: JS tek thread'de nasıl asenkron çalışır ─── */
function JavaScriptPipeline({ isTr }) {
    const pipelineRef = useRef(null)
    const stages = [
        { label: isTr ? 'Call Stack (Çağrı Yığını)' : 'Call Stack', color: 'var(--js-role-accent)', bg: 'rgba(255,107,157,0.13)', border: 'rgba(255,107,157,0.38)', z: 5, ty: -30 },
        { label: isTr ? 'Web APIs (setTimeout/fetch)' : 'Web APIs (setTimeout/fetch)', color: 'var(--js-role-accent-2)', bg: 'rgba(77,232,194,0.12)', border: 'rgba(77,232,194,0.36)', z: 4, ty: -18 },
        { label: isTr ? 'Microtask Queue (Promise)' : 'Microtask Queue (Promises)', color: 'var(--js-role-muted)', bg: 'rgba(203,184,212,0.11)', border: 'rgba(203,184,212,0.30)', z: 3, ty: -8 },
        { label: isTr ? 'Callback Queue (Macrotask)' : 'Callback Queue (Macrotasks)', color: 'var(--js-role-success)', bg: 'rgba(111,191,115,0.11)', border: 'rgba(111,191,115,0.30)', z: 2, ty: 2 },
        { label: isTr ? 'Event Loop (Olay Döngüsü)' : 'Event Loop', color: 'var(--js-role-muted)', bg: 'rgba(203,184,212,0.08)', border: 'rgba(203,184,212,0.22)', z: 1, ty: 9 },
    ]
    ...
    return (
        <div className="js-pipeline js-pipeline-interactive" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="js-pipeline-title">{isTr ? 'Event Loop Hattı' : 'Event Loop Pipeline'}</div>
            <div ref={pipelineRef} className="js-stages js-stages-3d">
                {stages.map((stage, idx) => (
                    <div key={idx} className="js-stage" style={{ '--stage-z': stage.z, '--ty': `${stage.ty}px`, background: stage.bg, borderColor: stage.border, color: stage.color }}>
                        <span className="js-stage-dot" style={{ background: stage.color }} />
                        <span className="js-stage-label">{stage.label}</span>
                    </div>
                ))}
            </div>
        </div>
...
/* ─── Konsol & Event Loop Sıra Panosu Simülatörü ──────────────── */
function JavaScriptConsoleSimulator({ isTr }) {
    const [inputValue, setInputValue] = useState('')
    const [consoleOutput, setConsoleOutput] = useState(
        isTr
            ? 'Node konsoluna hoş geldiniz.\nÇalıştırmak için run yazın...'
            : 'Welcome to the Node console.\nType run to execute...'
    )
    const initialOrder = [
        { id: 1, text: "console.log('1: sync start')", kind: 'sync' },
        { id: 2, text: "setTimeout(() => log('2: timeout'), 0)", kind: 'macro' },
        { id: 3, text: "Promise.resolve().then(() => log('3: promise'))", kind: 'micro' },
        { id: 4, text: "console.log('4: sync end')", kind: 'sync' },
    ]
    ...
    function runDemo(raw) {
        ...
        timersRef.current = []
        // gerçek çalışma sırası: 1 (sync), 4 (sync), 3 (microtask), 2 (macrotask)
        const executionOrder = [1, 4, 3, 2]
        executionOrder.forEach((id, idx) => {
            const t = setTimeout(() => {
        ...
        const finalTimer = setTimeout(() => {
            setConsoleOutput(prev => prev + `\n> ${raw}\n1: sync start\n4: sync end\n3: promise\n2: timeout\n\n` + (
                isTr
                    ? '(Sync kod önce, sonra microtask/Promise, en son macrotask/setTimeout çalışır.)'
                    : '(Sync code runs first, then microtasks/Promises, then macrotasks/setTimeout.)'
            ))
        }, executionOrder.length * 350 + 100)
    ...
    function handleCommandSubmit(e) {
        ...
        if (cmd === 'run' || cmd.startsWith('node')) {
            runDemo(raw)
        } else {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + (
                isTr
    ...
    return (
        <div className="js-console-box js-reveal">
            <div className="js-console">
                <div className="js-console-header">
                    <span>node-console v1.0</span>
                    <span>{isTr ? 'ÇALIŞIYOR' : 'RUNNING'}</span>
                </div>
                <div className="js-console-body">
                    {consoleOutput}
                    <div className="js-console-input-row">
                        <span className="js-console-prompt">&gt;</span>
                        <input
                            type="text"
                            value={inputValue}
                            onChange={(e) => setInputValue(e.target.value)}
                            onKeyDown={handleCommandSubmit}
                            className="js-console-input"
                            placeholder={isTr ? 'Komut yazın (run)...' : 'Type a command (run)...'}
                        />
                    </div>
                </div>
            </div>
            <div className="js-order-board">
                {initialOrder.map(item => (
                    <div key={item.id} className={`js-order-item${doneIds.includes(item.id) ? ' done' : ''}`}>
                        <span className={`js-order-badge ${item.kind}`}>{item.kind === 'sync' ? '⚡' : item.kind === 'micro' ? '🔬' : '⏰'}</span>
                        <span>{item.text}</span>
                    </div>
                ))}
            </div>
...
/* ─── Stats + Event Loop Pipeline asimetrik hero banner ────────── */
function JavaScriptStatsBanner() {
    const { language } = useLanguage()
    const isTr = language === 'tr'

    const stats = [
        { target: 1995, suffix: '',  tr: 'Yayın Yılı',       en: 'Founded'          },
        { target: 1,    suffix: '',  tr: 'Ana Thread',        en: 'Main Thread'      },
        { target: 100,  suffix: '%', tr: 'Tarayıcı Desteği',  en: 'Browser Support'  },
        { target: 2,    suffix: '',  tr: 'Kuyruk Türü',       en: 'Queue Types'      },
    ]

    return (
        <div className="js-hero-banner-container">
            <div className="js-hero-banner">
                <JavaScriptPipeline isTr={isTr} />
                <div className="js-stats-bar">
                    {stats.map(s => (
                        <div key={s.tr} className="js-stat-item">
                            <div className="js-stat-number-wrap">
                                <span className="js-stat-num" data-target={s.target}>0</span>
                                {s.suffix && <span className="js-stat-suffix">{s.suffix}</span>}
                            </div>
                            <p className="js-stat-label">{isTr ? s.tr : s.en}</p>
                        </div>
                    ))}
                </div>
            </div>
            <JavaScriptConsoleSimulator key={isTr ? 'tr' : 'en'} isTr={isTr} />
        </div>
    )
}
...
function JavaScriptPage() {
    ...
    useEffect(() => {
        const wrapper = document.querySelector('.javascript-page')
        if (!wrapper) return
        ...
        const particles = []
        const pColors = ['#ff6b9d', '#4de8c2', '#ff9dc0', '#6fbf73', '#f0b84e']
        for (let i = 0; i < 20; i++) {
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                ...
                if (e.target.classList.contains('js-reveal')) {
                    requestAnimationFrame(() => requestAnimationFrame(() => e.target.classList.add('js-visible')))
                }
        ...
        function setupReveal(el) {
            if (el.dataset.jsReveal) return
            el.dataset.jsReveal = '1'
            const inVP = el.getBoundingClientRect().top < window.innerHeight
            if (!inVP && !noMotion) el.classList.add('js-reveal')
        ...
        function applyBlockClasses() {
            ...
            Array.from(card.children).forEach(child => {
                ...
                if (!child.classList.contains('js-block')) child.classList.add('js-block')
                setupReveal(child)
            })
        }
        applyBlockClasses()
        ...
        const statObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                if (!e.isIntersecting) return
                const item = e.target
                item.classList.remove('js-stat-pending')
                item.classList.add('js-stat-visible')
                const num = item.querySelector('.js-stat-num')
        ...
        wrapper.querySelectorAll('.js-stat-item').forEach((el, i) => {
            if (!noMotion) { el.classList.add('js-stat-pending'); el.style.transitionDelay = `${i * 0.12}s` }
            statObserver.observe(el)
        })

        const statFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.js-stat-item.js-stat-pending').forEach(el => {
                el.classList.remove('js-stat-pending')
                el.classList.add('js-stat-visible')
                const num = el.querySelector('.js-stat-num')
        ...
        if (heroH1) {
            ...
            heroH1.classList.add('js-glitch')
        }

        let currentMagBtn = null
        function applyMagnetic() {
            wrapper.querySelectorAll(
                '[data-testid="topic-back-btn"]:not(.js-magnetic-init), ' +
                '[data-testid="dark-mode-toggle"]:not(.js-magnetic-init)'
            ).forEach(btn => { btn.classList.add('js-magnetic-init', 'no-hover-scale') })
        }
        applyMagnetic()

        function onWrapperMouseMove(e) {
            if (noMotion) return
            const btn = e.target.closest('button.js-magnetic-init, a.js-magnetic-init')
        ...
        function onWrapperPointerDown(e) {
            ...
            btn.classList.remove('js-squash')
            void btn.offsetWidth
            btn.classList.add('js-squash')
            btn.addEventListener('animationend', () => btn.classList.remove('js-squash'), { once: true })
            const r = btn.getBoundingClientRect()
            const size = Math.max(r.width, r.height) * 1.8
            const span = document.createElement('span')
            span.className = 'js-ripple-span'
            span.style.width = span.style.height = `${size}px`
            span.style.left = `${e.clientX - r.left - size / 2}px`
            span.style.top = `${e.clientY - r.top - size / 2}px`
            span.style.background = 'rgba(242, 236, 247, 0.45)'
        ...
        const revealFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.js-reveal:not(.js-visible)').forEach(el => {
                const top = el.getBoundingClientRect().top
                if (top < window.innerHeight * 1.5) el.classList.add('js-visible')
            })
        }, 1500)
        ...
        function onScroll() {
            ...
            wrapper.style.setProperty('--js-scroll-y', `${sY * 0.08}px`)
            const totalHeight = document.documentElement.scrollHeight - window.innerHeight
            const progress = totalHeight > 0 ? (sY / totalHeight) * 100 : 0
            wrapper.style.setProperty('--scroll-percent', `${progress}%`)
            const pctEl = wrapper.querySelector('.js-wave-percent')
            if (pctEl) pctEl.textContent = `${Math.round(progress)}%`
            wrapper.querySelectorAll('.js-reveal:not(.js-visible)').forEach(el => {
                if (el.getBoundingClientRect().top < window.innerHeight + 120) {
                    requestAnimationFrame(() => el.classList.add('js-visible'))
                }
            })
        }
        ...
        return () => {
            ...
            if (heroH1) { heroH1.classList.remove('js-glitch'); heroH1.removeAttribute('data-text') }
            wrapper.querySelectorAll('.js-magnetic-init').forEach(btn => {
                btn.classList.remove('js-magnetic-init', 'no-hover-scale', 'js-squash')
                btn.style.removeProperty('transform')
                btn.style.removeProperty('transition')
            })
            ...
            wrapper.style.removeProperty('--js-scroll-y')
    ...
    const soundToggleButton = isLightMode ? (
        <button
            type="button"
            className={`px-2 md:px-3 py-1 md:py-1.5 rounded-lg font-semibold text-xs md:text-sm bg-white/20 text-white hover:bg-white/30 border border-white/30 transition-all duration-300${soundOn ? ' ring-2 ring-pink-300' : ''}`}
            onClick={handleToggleSound}
            title={language === 'tr' ? (soundOn ? 'Yağmur sesini kapat' : 'Yağmur sesini aç') : (soundOn ? 'Mute rain sound' : 'Unmute rain sound')}
            data-testid="javascript-sound-toggle"
        >
            {soundOn ? '🔊' : '🔇'}
        </button>
    ) : null
    ...
    return (
        <div className="javascript-page">
            <TopicPage
                ...
                extraBanner={<JavaScriptStatsBanner />}
                headerExtra={soundToggleButton}
            />
            <div
                className="js-wave-progress"
                onClick={scrollToTop}
                title={language === 'tr' ? 'Yukarı Git' : 'Scroll to Top'}
                data-testid="ocean-progress"
            >
                <div className="js-wave-water" />
                <span className="js-wave-percent">0%</span>
            </div>
        </div>
    )
}
