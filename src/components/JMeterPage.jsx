...
import '../jmeter-effects.css'
import '../night-sky-effects.css'

const SOUND_PREF_KEY = 'ambientSoundEnabled'
const THUNDER_INTERVAL_MS = 10000 // jm-lightning-flash CSS animasyonuyla aynı 10s döngü

/* ─── Load Test Execution Pipeline: bir yük testi nasıl çalışır ─── */
function JMeterPipeline({ isTr }) {
    const pipelineRef = useRef(null)
    const stages = [
        { label: isTr ? 'Thread Group (Sanal Kullanıcı)' : 'Thread Group (Virtual Users)', color: 'var(--jm-role-accent)', bg: 'rgba(255,94,58,0.13)', border: 'rgba(255,94,58,0.38)', z: 5, ty: -30 },
        { label: isTr ? 'Sampler (HTTP İsteği)' : 'Sampler (HTTP Request)', color: 'var(--jm-role-accent-2)', bg: 'rgba(143,163,184,0.12)', border: 'rgba(143,163,184,0.36)', z: 4, ty: -18 },
        { label: isTr ? 'Assertions (Doğrulama)' : 'Assertions', color: 'var(--jm-role-muted)', bg: 'rgba(201,194,217,0.11)', border: 'rgba(201,194,217,0.30)', z: 3, ty: -8 },
        { label: isTr ? 'Listener (Sonuç Toplama)' : 'Listener (Result Collection)', color: 'var(--jm-role-success)', bg: 'rgba(111,191,115,0.11)', border: 'rgba(111,191,115,0.30)', z: 2, ty: 2 },
        { label: isTr ? 'Rapor (Report)' : 'Report', color: 'var(--jm-role-muted)', bg: 'rgba(201,194,217,0.08)', border: 'rgba(201,194,217,0.22)', z: 1, ty: 9 },
    ]
    ...
    return (
        <div className="jm-pipeline jm-pipeline-interactive" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="jm-pipeline-title">{isTr ? 'Yük Testi Yürütme Hattı' : 'Load Test Execution'}</div>
            <div ref={pipelineRef} className="jm-stages jm-stages-3d">
                {stages.map((stage, idx) => (
                    <div key={idx} className="jm-stage" style={{ '--stage-z': stage.z, '--ty': `${stage.ty}px`, background: stage.bg, borderColor: stage.border, color: stage.color }}>
                        <span className="jm-stage-dot" style={{ background: stage.color }} />
                        <span className="jm-stage-label">{stage.label}</span>
                    </div>
                ))}
            </div>
        </div>
...
/* ─── JMeter Konsolu & Yük Artırma (Ramp-Up) Panosu ────────────── */
function JMeterConsoleSimulator({ isTr }) {
    const [inputValue, setInputValue] = useState('')
    const [consoleOutput, setConsoleOutput] = useState(
        isTr
            ? 'JMeter konsoluna hoş geldiniz.\nÇalıştırmak için jmeter -n -t plan.jmx yazın...'
            : 'Welcome to the JMeter console.\nType jmeter -n -t plan.jmx to run...'
    )
    const stages = [
        { id: 's1', label: isTr ? '10 thread başlatıldı' : '10 threads started', fill: '10%' },
        { id: 's2', label: isTr ? '50 thread\'e çıkıldı (ramp-up)' : 'ramped up to 50 threads', fill: '50%' },
        { id: 's3', label: isTr ? '100 thread — tam yük' : '100 threads — full load', fill: '100%' },
        { id: 's4', label: isTr ? 'Sonuçlar toplanıyor' : 'Collecting results', fill: '100%' },
    ]
    ...
    function runDemo(raw) {
        ...
        const finalTimer = setTimeout(() => {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + (
                isTr
                    ? 'summary = 1000 in 12s = 83.3/s Avg: 142 Min: 45 Max: 980 Err: 0.4%'
                    : 'summary = 1000 in 12s = 83.3/s Avg: 142 Min: 45 Max: 980 Err: 0.4%'
            ))
    ...
    return (
        <div className="jm-console-box jm-reveal">
            <div className="jm-console">
                <div className="jm-console-header">
                    <span>jmeter-console v1.0</span>
                    <span>{isTr ? 'ÇALIŞIYOR' : 'RUNNING'}</span>
                </div>
                <div className="jm-console-body">
                    {consoleOutput}
                    <div className="jm-console-input-row">
                        <span className="jm-console-prompt">$</span>
                        <input
                            type="text"
                            value={inputValue}
                            onChange={(e) => setInputValue(e.target.value)}
                            onKeyDown={handleCommandSubmit}
                            className="jm-console-input"
                            placeholder={isTr ? 'Komut yazın...' : 'Type a command...'}
                        />
                    </div>
                </div>
            </div>
            <div className="jm-order-board">
                {stages.map(s => (
                    <div key={s.id} className={`jm-order-item${doneIds.includes(s.id) ? ' done' : ''}`} style={{ '--fill': s.fill }}>
                        <span>{s.label}</span>
                        <span className="jm-order-bar"><span className="jm-order-bar-fill" /></span>
                    </div>
                ))}
            </div>
        </div>
...
/* ─── Stats + Load Test Pipeline asimetrik hero banner ─────────── */
function JMeterStatsBanner() {
    const { language } = useLanguage()
    const isTr = language === 'tr'

    const stats = [
        { target: 1998, suffix: '',  tr: 'Yayın Yılı',            en: 'Founded'              },
        { target: 100,  suffix: '%', tr: 'Açık Kaynak',            en: 'Open Source'          },
        { target: 5,    suffix: '+', tr: 'Sampler Türü',           en: 'Sampler Types'        },
        { target: 1000, suffix: '+', tr: 'Eşzamanlı Kullanıcı',    en: 'Concurrent Users'     },
    ]

    return (
        <div className="jm-hero-banner-container">
            <div className="jm-hero-banner">
                <JMeterPipeline isTr={isTr} />
                <div className="jm-stats-bar">
                    {stats.map(s => (
                        <div key={s.tr} className="jm-stat-item">
                            <div className="jm-stat-number-wrap">
                                <span className="jm-stat-num" data-target={s.target}>0</span>
                                {s.suffix && <span className="jm-stat-suffix">{s.suffix}</span>}
                            </div>
                            <p className="jm-stat-label">{isTr ? s.tr : s.en}</p>
                        </div>
                    ))}
                </div>
            </div>
            <JMeterConsoleSimulator key={isTr ? 'tr' : 'en'} isTr={isTr} />
        </div>
    )
}
...
function JMeterPage() {
    ...
    useEffect(() => {
        const wrapper = document.querySelector('.jmeter-page')
        if (!wrapper) return
        ...
        const particles = []
        const pColors = ['#ff5e3a', '#8fa3b8', '#ff8a66', '#6fbf73', '#f0b84e']
        for (let i = 0; i < 20; i++) {
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                ...
                if (e.target.classList.contains('jm-reveal')) {
                    requestAnimationFrame(() => requestAnimationFrame(() => e.target.classList.add('jm-visible')))
                }
        ...
        function setupReveal(el) {
            if (el.dataset.jmReveal) return
            el.dataset.jmReveal = '1'
            const inVP = el.getBoundingClientRect().top < window.innerHeight
            if (!inVP && !noMotion) el.classList.add('jm-reveal')
        ...
        function applyBlockClasses() {
            ...
            Array.from(card.children).forEach(child => {
                ...
                if (!child.classList.contains('jm-block')) child.classList.add('jm-block')
                setupReveal(child)
            })
        }
        applyBlockClasses()
        ...
        const statObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                if (!e.isIntersecting) return
                const item = e.target
                item.classList.remove('jm-stat-pending')
                item.classList.add('jm-stat-visible')
                const num = item.querySelector('.jm-stat-num')
        ...
        wrapper.querySelectorAll('.jm-stat-item').forEach((el, i) => {
            if (!noMotion) { el.classList.add('jm-stat-pending'); el.style.transitionDelay = `${i * 0.12}s` }
            statObserver.observe(el)
        })

        const statFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.jm-stat-item.jm-stat-pending').forEach(el => {
                el.classList.remove('jm-stat-pending')
                el.classList.add('jm-stat-visible')
                const num = el.querySelector('.jm-stat-num')
        ...
        if (heroH1) {
            ...
            heroH1.classList.add('jm-glitch')
        }

        let currentMagBtn = null
        function applyMagnetic() {
            wrapper.querySelectorAll(
                '[data-testid="topic-back-btn"]:not(.jm-magnetic-init), ' +
                '[data-testid="dark-mode-toggle"]:not(.jm-magnetic-init)'
            ).forEach(btn => { btn.classList.add('jm-magnetic-init', 'no-hover-scale') })
        }
        applyMagnetic()

        function onWrapperMouseMove(e) {
            if (noMotion) return
            const btn = e.target.closest('button.jm-magnetic-init, a.jm-magnetic-init')
        ...
        function onWrapperPointerDown(e) {
            ...
            btn.classList.remove('jm-squash')
            void btn.offsetWidth
            btn.classList.add('jm-squash')
            btn.addEventListener('animationend', () => btn.classList.remove('jm-squash'), { once: true })
            const r = btn.getBoundingClientRect()
            const size = Math.max(r.width, r.height) * 1.8
            const span = document.createElement('span')
            span.className = 'jm-ripple-span'
            span.style.width = span.style.height = `${size}px`
            span.style.left = `${e.clientX - r.left - size / 2}px`
            span.style.top = `${e.clientY - r.top - size / 2}px`
            span.style.background = 'rgba(236, 232, 247, 0.45)'
        ...
        function onContentMouseMove(e) {
            if (noMotion) return
            const block = e.target.closest('.jm-block')
        ...
        const revealFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.jm-reveal:not(.jm-visible)').forEach(el => {
                const top = el.getBoundingClientRect().top
                if (top < window.innerHeight * 1.5) el.classList.add('jm-visible')
            })
        }, 1500)
        ...
        function onScroll() {
            ...
            wrapper.style.setProperty('--jm-scroll-y', `${sY * 0.08}px`)
            const totalHeight = document.documentElement.scrollHeight - window.innerHeight
            const progress = totalHeight > 0 ? (sY / totalHeight) * 100 : 0
            wrapper.style.setProperty('--scroll-percent', `${progress}%`)
            const pctEl = wrapper.querySelector('.jm-wave-percent')
            if (pctEl) pctEl.textContent = `${Math.round(progress)}%`
            wrapper.querySelectorAll('.jm-reveal:not(.jm-visible)').forEach(el => {
                if (el.getBoundingClientRect().top < window.innerHeight + 120) {
                    requestAnimationFrame(() => el.classList.add('jm-visible'))
                }
            })
        }
        ...
        return () => {
            ...
            if (heroH1) { heroH1.classList.remove('jm-glitch'); heroH1.removeAttribute('data-text') }
            wrapper.querySelectorAll('.jm-magnetic-init').forEach(btn => {
                btn.classList.remove('jm-magnetic-init', 'no-hover-scale', 'jm-squash')
                btn.style.removeProperty('transform')
                btn.style.removeProperty('transition')
            })
            ...
            wrapper.style.removeProperty('--jm-scroll-y')
    ...
    const soundToggleButton = isLightMode ? (
        <button
            ...
            data-testid="jmeter-sound-toggle"
        >
            {soundOn ? '🔊' : '🔇'}
        </button>
    ) : null
    ...
    return (
        <div className="jmeter-page">
            <TopicPage
                ...
                extraBanner={<JMeterStatsBanner />}
                headerExtra={soundToggleButton}
            />
            <div
                className="jm-wave-progress"
                onClick={scrollToTop}
                title={language === 'tr' ? 'Yukarı Git' : 'Scroll to Top'}
                data-testid="ocean-progress"
            >
                <div className="jm-wave-water" />
                <span className="jm-wave-percent">0%</span>
            </div>
        </div>
    )
}
