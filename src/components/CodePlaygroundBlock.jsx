...
function PracticePanel({ starterCode, solutionCode, expected, isTr, darkMode, onPass, language }) {
    ...
    return (
        <div className={`mt-3 rounded-lg border p-3 ${panelCls(darkMode)}`}>
            <div className="mb-2 text-xs font-bold opacity-70">
                {isTr
                    ? `Bu alanda kodu kendin yazıp kontrollü şekilde sonucu görebilirsin. Gerçek ${language || 'kod'} derleyici/yorumlayıcısı değil; bu egzersizin beklenen çözümüyle karşılaştırır.`
                    : `Write the code yourself here and see a controlled result. This is not a real ${language || 'code'} compiler/interpreter; it compares against this exercise solution.`}
            </div>
            <textarea
                value={draft}
                onChange={(e) => { setDraft(e.target.value); setResult(null) }}
