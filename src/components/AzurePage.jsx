...
import '../azure-effects.css'
import '../night-sky-effects.css'

const SOUND_PREF_KEY = 'ambientSoundEnabled'
const THUNDER_INTERVAL_MS = 10000 // az-lightning-flash CSS animasyonuyla aynı 10s döngü

/* ─── Azure DevOps Pipeline Akışı: bir kod deposu nasıl yayınlanan release'e dönüşür ─── */
function AzurePipeline({ isTr }) {
    const pipelineRef = useRef(null)
    const stages = [
        { label: isTr ? 'Repos (Kod)' : 'Repos (Code)', color: 'var(--az-role-accent)', bg: 'rgba(43,136,216,0.13)', border: 'rgba(43,136,216,0.38)', z: 5, ty: -30 },
        { label: isTr ? 'Pipelines (Build/Test)' : 'Pipelines (Build/Test)', color: 'var(--az-role-amber)', bg: 'rgba(242,183,5,0.12)', border: 'rgba(242,183,5,0.36)', z: 4, ty: -18 },
        { label: isTr ? 'Test Plans' : 'Test Plans', color: 'var(--az-role-muted)', bg: 'rgba(198,211,230,0.11)', border: 'rgba(198,211,230,0.30)', z: 3, ty: -8 },
        { label: isTr ? 'Artifacts (Paketle)' : 'Artifacts (Package)', color: 'var(--az-role-success)', bg: 'rgba(74,222,128,0.11)', border: 'rgba(74,222,128,0.30)', z: 2, ty: 2 },
        { label: isTr ? 'Release (Yayınla)' : 'Release (Deploy)', color: 'var(--az-role-muted)', bg: 'rgba(198,211,230,0.08)', border: 'rgba(198,211,230,0.22)', z: 1, ty: 9 },
    ]
    ...
    return (
        <div className="az-pipeline az-pipeline-interactive" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="az-pipeline-title">{isTr ? 'Azure DevOps Pipeline Akışı' : 'Azure DevOps Pipeline Flow'}</div>
            <div ref={pipelineRef} className="az-stages az-stages-3d">
                {stages.map((stage, idx) => (
                    <div key={idx} className="az-stage" style={{ '--stage-z': stage.z, '--ty': `${stage.ty}px`, background: stage.bg, borderColor: stage.border, color: stage.color }}>
                        <span className="az-stage-dot" style={{ background: stage.color }} />
                        <span className="az-stage-label">{stage.label}</span>
                    </div>
                ))}
            </div>
        </div>
...
/* ─── az CLI Konsolu & Pipeline Aşama Panosu ────────── */
function AzureConsoleSimulator({ isTr }) {
    const [inputValue, setInputValue] = useState('')
    const [consoleOutput, setConsoleOutput] = useState(
        isTr
            ? 'Azure Pipelines konsoluna hoş geldiniz.\nÇalıştırmak için run-pipeline yazın...'
            : 'Welcome to the Azure Pipelines console.\nType run-pipeline to execute...'
    )
    const checks = [
        { id: 'c1', label: isTr ? 'Build: dotnet build başarılı' : 'Build: dotnet build succeeded' },
        { id: 'c2', label: isTr ? 'Test Plans: 63/63 test PASS' : 'Test Plans: 63/63 tests PASS' },
        { id: 'c3', label: isTr ? 'Artifacts: paket yayınlandı' : 'Artifacts: package published' },
    ]
    ...
    function runDemo(raw) {
        ...
        const finalTimer = setTimeout(() => {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + 'Pipeline run #47 -> Succeeded\n\n' + (isTr ? 'Release: staging ortamına dağıtıldı ✓' : 'Release: deployed to staging ✓'))
    ...
    return (
        <div className="az-console-box az-reveal">
            <div className="az-console">
                <div className="az-console-header">
                    <span>az pipelines</span>
                    <span>{isTr ? 'ÇALIŞIYOR' : 'RUNNING'}</span>
                </div>
                <div className="az-console-body">
                    {consoleOutput}
                    <div className="az-console-input-row">
                        <span className="az-console-prompt">$</span>
                        <input
                            type="text"
                            value={inputValue}
                            onChange={(e) => setInputValue(e.target.value)}
                            onKeyDown={handleCommandSubmit}
                            className="az-console-input"
                            placeholder={isTr ? 'Komut yazın...' : 'Type a command...'}
                        />
                    </div>
                </div>
            </div>
            <div className="az-order-board">
                {checks.map(c => (
                    <div key={c.id} className={`az-order-item${doneIds.includes(c.id) ? ' done' : ''}`}>
...
/* ─── Stats + Azure Pipeline asimetrik hero banner ──────────────── */
function AzureStatsBanner() {
    ...
    const stats = [
        ...
        { target: 63,   suffix: '',  tr: 'Örnek Test Sayısı',       en: 'Sample Test Count'    },
    ]

    return (
        <div className="az-hero-banner-container">
            <div className="az-hero-banner">
                <AzurePipeline isTr={isTr} />
                <div className="az-stats-bar">
                    {stats.map(s => (
                        <div key={s.tr} className="az-stat-item">
                            <div className="az-stat-number-wrap">
                                <span className="az-stat-num" data-target={s.target}>0</span>
                                {s.suffix && <span className="az-stat-suffix">{s.suffix}</span>}
                            </div>
                            <p className="az-stat-label">{isTr ? s.tr : s.en}</p>
                        </div>
                    ))}
                </div>
            </div>
            <AzureConsoleSimulator key={isTr ? 'tr' : 'en'} isTr={isTr} />
        </div>
    )
}
...
function AzurePage() {
    ...
    useEffect(() => {
        const wrapper = document.querySelector('.azure-page')
        if (!wrapper) return
        ...
        const particles = []
        const pColors = ['#2b88d8', '#7ec1f5', '#f2b705', '#f8d55c', '#4ade80']
        for (let i = 0; i < 20; i++) {
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                ...
                if (e.target.classList.contains('az-reveal')) {
                    requestAnimationFrame(() => requestAnimationFrame(() => e.target.classList.add('az-visible')))
                }
        ...
        function setupReveal(el) {
            if (el.dataset.azReveal) return
            el.dataset.azReveal = '1'
            const inVP = el.getBoundingClientRect().top < window.innerHeight
            if (!inVP && !noMotion) el.classList.add('az-reveal')
        ...
        function applyBlockClasses() {
            ...
            Array.from(card.children).forEach(child => {
                ...
                if (!child.classList.contains('az-block')) child.classList.add('az-block')
                setupReveal(child)
            })
        }
        applyBlockClasses()
        ...
        const statObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                if (!e.isIntersecting) return
                const item = e.target
                item.classList.remove('az-stat-pending')
                item.classList.add('az-stat-visible')
                const num = item.querySelector('.az-stat-num')
        ...
        wrapper.querySelectorAll('.az-stat-item').forEach((el, i) => {
            if (!noMotion) { el.classList.add('az-stat-pending'); el.style.transitionDelay = `${i * 0.12}s` }
            statObserver.observe(el)
        })

        const statFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.az-stat-item.az-stat-pending').forEach(el => {
                el.classList.remove('az-stat-pending')
                el.classList.add('az-stat-visible')
                const num = el.querySelector('.az-stat-num')
        ...
        if (heroH1) {
            ...
            heroH1.classList.add('az-glitch')
        }

        let currentMagBtn = null
        function applyMagnetic() {
            wrapper.querySelectorAll(
                '[data-testid="topic-back-btn"]:not(.az-magnetic-init), ' +
                '[data-testid="dark-mode-toggle"]:not(.az-magnetic-init)'
            ).forEach(btn => { btn.classList.add('az-magnetic-init', 'no-hover-scale') })
        }
        applyMagnetic()

        function onWrapperMouseMove(e) {
            if (noMotion) return
            const btn = e.target.closest('button.az-magnetic-init, a.az-magnetic-init')
        ...
        function onWrapperPointerDown(e) {
            ...
            btn.classList.remove('az-squash')
            void btn.offsetWidth
            btn.classList.add('az-squash')
            btn.addEventListener('animationend', () => btn.classList.remove('az-squash'), { once: true })
            const r = btn.getBoundingClientRect()
            const size = Math.max(r.width, r.height) * 1.8
            const span = document.createElement('span')
            span.className = 'az-ripple-span'
        ...
        function onContentMouseMove(e) {
            if (noMotion) return
            const block = e.target.closest('.az-block')
        ...
        const revealFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.az-reveal:not(.az-visible)').forEach(el => {
                const top = el.getBoundingClientRect().top
                if (top < window.innerHeight * 1.5) el.classList.add('az-visible')
            })
        }, 1500)
        ...
        function onScroll() {
            ...
            wrapper.style.setProperty('--az-scroll-y', `${sY * 0.08}px`)
            const totalHeight = document.documentElement.scrollHeight - window.innerHeight
            const progress = totalHeight > 0 ? (sY / totalHeight) * 100 : 0
            wrapper.style.setProperty('--scroll-percent', `${progress}%`)
            const pctEl = wrapper.querySelector('.az-wave-percent')
            if (pctEl) pctEl.textContent = `${Math.round(progress)}%`
            wrapper.querySelectorAll('.az-reveal:not(.az-visible)').forEach(el => {
                if (el.getBoundingClientRect().top < window.innerHeight + 120) {
                    requestAnimationFrame(() => el.classList.add('az-visible'))
                }
            })
        }
        ...
        return () => {
            ...
            if (heroH1) { heroH1.classList.remove('az-glitch'); heroH1.removeAttribute('data-text') }
            wrapper.querySelectorAll('.az-magnetic-init').forEach(btn => {
                btn.classList.remove('az-magnetic-init', 'no-hover-scale', 'az-squash')
                btn.style.removeProperty('transform')
                btn.style.removeProperty('transition')
            })
            ...
            wrapper.style.removeProperty('--az-scroll-y')
    ...
    const soundToggleButton = isLightMode ? (
        <button
            ...
            data-testid="azure-sound-toggle"
        >
            {soundOn ? '🔊' : '🔇'}
        </button>
    ) : null
    ...
    return (
        <div className="azure-page">
            <TopicPage
                ...
                extraBanner={<AzureStatsBanner />}
                headerExtra={soundToggleButton}
            />
            <div
                className="az-wave-progress"
                onClick={scrollToTop}
                title={language === 'tr' ? 'Yukarı Git' : 'Scroll to Top'}
                data-testid="ocean-progress"
            >
                <div className="az-wave-water" />
                <span className="az-wave-percent">0%</span>
            </div>
        </div>
    )
}
