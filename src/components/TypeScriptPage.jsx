...
const THUNDER_INTERVAL_MS = 10000 // ts-lightning-flash CSS animasyonuyla aynı 10s döngü

/* ─── TS Compile Pipeline: kaynak koddan çalışma zamanına ─── */
function TypeScriptPipeline({ isTr }) {
    const pipelineRef = useRef(null)
    const stages = [
        { label: isTr ? 'Kaynak Kod (.ts)' : 'Source Code (.ts)', color: 'var(--ts-role-accent)', bg: 'rgba(59,130,246,0.13)', border: 'rgba(59,130,246,0.38)', z: 5, ty: -30 },
        { label: isTr ? 'Type Checker (tsc)' : 'Type Checker (tsc)', color: 'var(--ts-role-accent-2)', bg: 'rgba(255,61,129,0.12)', border: 'rgba(255,61,129,0.36)', z: 4, ty: -18 },
        { label: isTr ? 'Tip Çıkarımı (Inference)' : 'Type Inference', color: 'var(--ts-role-muted)', bg: 'rgba(184,194,217,0.11)', border: 'rgba(184,194,217,0.30)', z: 3, ty: -8 },
        { label: isTr ? "JS'e Transpile" : 'Transpile to JS', color: 'var(--ts-role-success)', bg: 'rgba(111,191,115,0.11)', border: 'rgba(111,191,115,0.30)', z: 2, ty: 2 },
        { label: isTr ? 'Çalışma Zamanı (Runtime)' : 'Runtime', color: 'var(--ts-role-muted)', bg: 'rgba(184,194,217,0.08)', border: 'rgba(184,194,217,0.22)', z: 1, ty: 9 },
    ]
    ...
    return (
        <div className="ts-pipeline ts-pipeline-interactive" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="ts-pipeline-title">{isTr ? 'TS Derleme Hattı' : 'TS Compile Pipeline'}</div>
            <div ref={pipelineRef} className="ts-stages ts-stages-3d">
                {stages.map((stage, idx) => (
                    <div key={idx} className="ts-stage" style={{ '--stage-z': stage.z, '--ty': `${stage.ty}px`, background: stage.bg, borderColor: stage.border, color: stage.color }}>
                        <span className="ts-stage-dot" style={{ background: stage.color }} />
...
/* ─── tsc Konsolu & Tip Denetleyici Görselleştirici ────────── */
function TypeScriptConsoleSimulator({ isTr }) {
    const [inputValue, setInputValue] = useState('')
    const [consoleOutput, setConsoleOutput] = useState(
        isTr
            ? 'tsc konsoluna hoş geldiniz.\nDerlemek için tsc index.ts yazın...'
            : 'Welcome to the tsc console.\nType tsc index.ts to compile...'
    )
    ...
    function handleCommandSubmit(e) {
        ...
        if (cmd.startsWith('tsc')) {
            ...
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + (
                isTr
                    ? 'index.ts:4:5 - error TS2322: \'number\' türü \'string\' türüne atanamaz.\n\n4     let username: string = 42;\n          ~~~~~~~~\n\n1 hata bulundu.'
                    : "index.ts:4:5 - error TS2322: Type 'number' is not assignable to type 'string'.\n\n4     let username: string = 42;\n          ~~~~~~~~\n\nFound 1 error."
            ))
        } else {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + (
                isTr
    ...
    return (
        <div className="ts-console-box ts-reveal">
            <div className="ts-console">
                <div className="ts-console-header">
                    <span>tsc-console v1.0</span>
                    <span>{isTr ? 'ÇALIŞIYOR' : 'RUNNING'}</span>
                </div>
                <div className="ts-console-body">
                    {consoleOutput}
                    <div className="ts-console-input-row">
                        <span className="ts-console-prompt">$</span>
                        <input
                            type="text"
                            value={inputValue}
                            ...
                            placeholder={isTr ? 'Komut yazın (tsc index.ts)...' : 'Type a command (tsc index.ts)...'}
                        />
                    </div>
                </div>
            </div>
            <div className="ts-code-viewer">
                {codeLines.map((line, idx) => (
                    <div key={idx} className={`ts-code-line${errorLine === idx ? ' error-line' : ''}`}>{line}</div>
                ))}
                {errorLine !== null && (
                    <span className="ts-code-badge">TS2322</span>
                )}
            </div>
        </div>
    )
...
/* ─── Stats + TS Compile Pipeline asimetrik hero banner ───────── */
function TypeScriptStatsBanner() {
    const { language } = useLanguage()
    const isTr = language === 'tr'

    const stats = [
        { target: 2012, suffix: '',  tr: 'Yayın Yılı',           en: 'Founded'             },
        { target: 100,  suffix: '%', tr: 'JS Uyumluluğu',         en: 'JS Compatibility'    },
        { target: 20,   suffix: '+', tr: 'Utility Type',          en: 'Utility Types'       },
        { target: 5,    suffix: '+', tr: 'Popüler Editör Desteği',en: 'Popular Editor Support' },
    ]

    return (
        <div className="ts-hero-banner-container">
            <div className="ts-hero-banner">
                <TypeScriptPipeline isTr={isTr} />
                <div className="ts-stats-bar">
                    {stats.map(s => (
                        <div key={s.tr} className="ts-stat-item">
                            <div className="ts-stat-number-wrap">
                                <span className="ts-stat-num" data-target={s.target}>0</span>
                                {s.suffix && <span className="ts-stat-suffix">{s.suffix}</span>}
                            </div>
                            <p className="ts-stat-label">{isTr ? s.tr : s.en}</p>
                        </div>
                    ))}
                </div>
            </div>
            <TypeScriptConsoleSimulator key={isTr ? 'tr' : 'en'} isTr={isTr} />
        </div>
    )
}
...
function TypeScriptPage() {
    ...
    useEffect(() => {
        ...
        const particles = []
        const pColors = ['#3b82f6', '#ff3d81', '#60a5fa', '#6fbf73', '#f0b84e']
        for (let i = 0; i < 20; i++) {
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                ...
                if (e.target.classList.contains('ts-reveal')) {
                    requestAnimationFrame(() => requestAnimationFrame(() => e.target.classList.add('ts-visible')))
                }
        ...
        function setupReveal(el) {
            if (el.dataset.tsReveal) return
            el.dataset.tsReveal = '1'
            const inVP = el.getBoundingClientRect().top < window.innerHeight
            if (!inVP && !noMotion) el.classList.add('ts-reveal')
        ...
        function applyBlockClasses() {
            ...
            Array.from(card.children).forEach(child => {
                ...
                if (!child.classList.contains('ts-block')) child.classList.add('ts-block')
                setupReveal(child)
            })
        }
        applyBlockClasses()
        ...
        const statObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                if (!e.isIntersecting) return
                const item = e.target
                item.classList.remove('ts-stat-pending')
                item.classList.add('ts-stat-visible')
                const num = item.querySelector('.ts-stat-num')
        ...
        wrapper.querySelectorAll('.ts-stat-item').forEach((el, i) => {
            if (!noMotion) { el.classList.add('ts-stat-pending'); el.style.transitionDelay = `${i * 0.12}s` }
            statObserver.observe(el)
        })

        const statFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.ts-stat-item.ts-stat-pending').forEach(el => {
                el.classList.remove('ts-stat-pending')
                el.classList.add('ts-stat-visible')
                const num = el.querySelector('.ts-stat-num')
        ...
        if (heroH1) {
            ...
            heroH1.classList.add('ts-glitch')
        }

        let currentMagBtn = null
        function applyMagnetic() {
            wrapper.querySelectorAll(
                '[data-testid="topic-back-btn"]:not(.ts-magnetic-init), ' +
                '[data-testid="dark-mode-toggle"]:not(.ts-magnetic-init)'
            ).forEach(btn => { btn.classList.add('ts-magnetic-init', 'no-hover-scale') })
        }
        applyMagnetic()

        function onWrapperMouseMove(e) {
            if (noMotion) return
            const btn = e.target.closest('button.ts-magnetic-init, a.ts-magnetic-init')
        ...
        function onWrapperPointerDown(e) {
            ...
            btn.classList.remove('ts-squash')
            void btn.offsetWidth
            btn.classList.add('ts-squash')
            btn.addEventListener('animationend', () => btn.classList.remove('ts-squash'), { once: true })
            const r = btn.getBoundingClientRect()
            const size = Math.max(r.width, r.height) * 1.8
            const span = document.createElement('span')
            span.className = 'ts-ripple-span'
            span.style.width = span.style.height = `${size}px`
            span.style.left = `${e.clientX - r.left - size / 2}px`
            span.style.top = `${e.clientY - r.top - size / 2}px`
            span.style.background = 'rgba(232, 236, 247, 0.45)'
        ...
        const revealFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.ts-reveal:not(.ts-visible)').forEach(el => {
                const top = el.getBoundingClientRect().top
                if (top < window.innerHeight * 1.5) el.classList.add('ts-visible')
            })
        }, 1500)
        ...
        function onScroll() {
            ...
            wrapper.style.setProperty('--ts-scroll-y', `${sY * 0.08}px`)
            const totalHeight = document.documentElement.scrollHeight - window.innerHeight
            const progress = totalHeight > 0 ? (sY / totalHeight) * 100 : 0
            wrapper.style.setProperty('--scroll-percent', `${progress}%`)
            const pctEl = wrapper.querySelector('.ts-wave-percent')
            if (pctEl) pctEl.textContent = `${Math.round(progress)}%`
            wrapper.querySelectorAll('.ts-reveal:not(.ts-visible)').forEach(el => {
                if (el.getBoundingClientRect().top < window.innerHeight + 120) {
                    requestAnimationFrame(() => el.classList.add('ts-visible'))
                }
            })
        }
        ...
        return () => {
            ...
            if (heroH1) { heroH1.classList.remove('ts-glitch'); heroH1.removeAttribute('data-text') }
            wrapper.querySelectorAll('.ts-magnetic-init').forEach(btn => {
                btn.classList.remove('ts-magnetic-init', 'no-hover-scale', 'ts-squash')
                btn.style.removeProperty('transform')
                btn.style.removeProperty('transition')
            })
            ...
            wrapper.style.removeProperty('--ts-scroll-y')
    ...
    const soundToggleButton = isLightMode ? (
        <button
            type="button"
            className={`px-2 md:px-3 py-1 md:py-1.5 rounded-lg font-semibold text-xs md:text-sm bg-white/20 text-white hover:bg-white/30 border border-white/30 transition-all duration-300${soundOn ? ' ring-2 ring-blue-300' : ''}`}
            onClick={handleToggleSound}
            title={language === 'tr' ? (soundOn ? 'Yağmur sesini kapat' : 'Yağmur sesini aç') : (soundOn ? 'Mute rain sound' : 'Unmute rain sound')}
            data-testid="typescript-sound-toggle"
        >
            {soundOn ? '🔊' : '🔇'}
        </button>
    ) : null
    ...
    return (
        <div className="typescript-page">
            ...
            >
                <div className="ts-wave-water" />
                <span className="ts-wave-percent">0%</span>
            </div>
        </div>
    )
}
