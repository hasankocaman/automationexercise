...
import '../aws-effects.css'
import '../night-sky-effects.css'

const SOUND_PREF_KEY = 'ambientSoundEnabled'
const THUNDER_INTERVAL_MS = 10000 // aw-lightning-flash CSS animasyonuyla aynı 10s döngü

/* ─── QA CI/CD'de AWS Kullanım Akışı: bir kod push nasıl test raporuna dönüşür ─── */
function AWSPipeline({ isTr }) {
    const pipelineRef = useRef(null)
    const stages = [
        { label: isTr ? 'Kod Push' : 'Code Push', color: 'var(--aw-role-accent)', bg: 'rgba(255,153,0,0.13)', border: 'rgba(255,153,0,0.38)', z: 5, ty: -30 },
        { label: isTr ? 'CodeBuild (Test Çalıştır)' : 'CodeBuild (Run Tests)', color: 'var(--aw-role-teal)', bg: 'rgba(45,212,191,0.12)', border: 'rgba(45,212,191,0.36)', z: 4, ty: -18 },
        { label: isTr ? 'S3 (Rapor Sakla)' : 'S3 (Store Report)', color: 'var(--aw-role-muted)', bg: 'rgba(211,210,201,0.11)', border: 'rgba(211,210,201,0.30)', z: 3, ty: -8 },
        { label: isTr ? 'CloudWatch (Log/Metric)' : 'CloudWatch (Log/Metric)', color: 'var(--aw-role-success)', bg: 'rgba(74,222,128,0.11)', border: 'rgba(74,222,128,0.30)', z: 2, ty: 2 },
        { label: isTr ? 'SNS (Bildirim)' : 'SNS (Notification)', color: 'var(--aw-role-muted)', bg: 'rgba(211,210,201,0.08)', border: 'rgba(211,210,201,0.22)', z: 1, ty: 9 },
    ]
    ...
    return (
        <div className="aw-pipeline aw-pipeline-interactive" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="aw-pipeline-title">{isTr ? 'QA CI/CD\'de AWS Akışı' : 'AWS Flow in QA CI/CD'}</div>
            <div ref={pipelineRef} className="aw-stages aw-stages-3d">
                {stages.map((stage, idx) => (
                    <div key={idx} className="aw-stage" style={{ '--stage-z': stage.z, '--ty': `${stage.ty}px`, background: stage.bg, borderColor: stage.border, color: stage.color }}>
                        <span className="aw-stage-dot" style={{ background: stage.color }} />
...
/* ─── AWS CLI Konsolu & Build/Rapor Panosu ────────── */
function AWSConsoleSimulator({ isTr }) {
    const [inputValue, setInputValue] = useState('')
    const [consoleOutput, setConsoleOutput] = useState(
        isTr
            ? 'AWS CLI konsoluna hoş geldiniz.\nÇalıştırmak için start-build yazın...'
            : 'Welcome to the AWS CLI console.\nType start-build to execute...'
    )
    const checks = [
        { id: 'c1', label: isTr ? 'CodeBuild: 84/84 test PASS' : 'CodeBuild: 84/84 tests PASS' },
        { id: 'c2', label: isTr ? 'S3: test-report.html yüklendi' : 'S3: test-report.html uploaded' },
        { id: 'c3', label: isTr ? 'CloudWatch: metrik kaydedildi' : 'CloudWatch: metric recorded' },
    ]
    ...
    function runDemo(raw) {
        ...
        const finalTimer = setTimeout(() => {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + 'Build "qa-tests" -> SUCCEEDED\n\n' + (isTr ? 'SNS: Slack kanalına bildirim gönderildi ✓' : 'SNS: notification sent to Slack channel ✓'))
    ...
    return (
        <div className="aw-console-box aw-reveal">
            <div className="aw-console">
                <div className="aw-console-header">
                    <span>aws-cli v2</span>
                    <span>{isTr ? 'ÇALIŞIYOR' : 'RUNNING'}</span>
                </div>
                <div className="aw-console-body">
                    {consoleOutput}
                    <div className="aw-console-input-row">
                        <span className="aw-console-prompt">$</span>
                        <input
                            type="text"
                            value={inputValue}
                            onChange={(e) => setInputValue(e.target.value)}
                            onKeyDown={handleCommandSubmit}
                            className="aw-console-input"
                            placeholder={isTr ? 'Komut yazın...' : 'Type a command...'}
                        />
                    </div>
                </div>
            </div>
            <div className="aw-order-board">
                {checks.map(c => (
                    <div key={c.id} className={`aw-order-item${doneIds.includes(c.id) ? ' done' : ''}`}>
...
/* ─── Stats + AWS Pipeline asimetrik hero banner ──────────────── */
function AWSStatsBanner() {
    const { language } = useLanguage()
    const isTr = language === 'tr'

    const stats = [
        { target: 2006, suffix: '',  tr: 'Yayın Yılı',              en: 'Founded'              },
        { target: 200,  suffix: '+', tr: 'Servis Sayısı',           en: 'Services Available'   },
        { target: 5,    suffix: '',  tr: 'CI/CD Akış Aşaması',      en: 'CI/CD Flow Stages'    },
        { target: 84,   suffix: '',  tr: 'Örnek Test Sayısı',       en: 'Sample Test Count'    },
    ]

    return (
        <div className="aw-hero-banner-container">
            <div className="aw-hero-banner">
                <AWSPipeline isTr={isTr} />
                <div className="aw-stats-bar">
                    {stats.map(s => (
                        <div key={s.tr} className="aw-stat-item">
                            <div className="aw-stat-number-wrap">
                                <span className="aw-stat-num" data-target={s.target}>0</span>
                                {s.suffix && <span className="aw-stat-suffix">{s.suffix}</span>}
                            </div>
                            <p className="aw-stat-label">{isTr ? s.tr : s.en}</p>
                        </div>
                    ))}
                </div>
            </div>
            <AWSConsoleSimulator key={isTr ? 'tr' : 'en'} isTr={isTr} />
        </div>
    )
}
...
function AWSPage() {
    ...
    useEffect(() => {
        const wrapper = document.querySelector('.aws-page')
        if (!wrapper) return
        ...
        const particles = []
        const pColors = ['#ff9900', '#ffbf5c', '#2dd4bf', '#7ee9dc', '#4ade80']
        for (let i = 0; i < 20; i++) {
            const p = document.createElement('div')
            p.className = 'aw-particle'
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                ...
                if (e.target.classList.contains('aw-reveal')) {
                    requestAnimationFrame(() => requestAnimationFrame(() => e.target.classList.add('aw-visible')))
                }
        ...
        function setupReveal(el) {
            if (el.dataset.awReveal) return
            el.dataset.awReveal = '1'
            const inVP = el.getBoundingClientRect().top < window.innerHeight
            if (!inVP && !noMotion) el.classList.add('aw-reveal')
        ...
        function applyBlockClasses() {
            ...
            Array.from(card.children).forEach(child => {
                ...
                if (!child.classList.contains('aw-block')) child.classList.add('aw-block')
                setupReveal(child)
            })
        }
        applyBlockClasses()
        ...
        const statObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                if (!e.isIntersecting) return
                const item = e.target
                item.classList.remove('aw-stat-pending')
                item.classList.add('aw-stat-visible')
                const num = item.querySelector('.aw-stat-num')
        ...
        wrapper.querySelectorAll('.aw-stat-item').forEach((el, i) => {
            if (!noMotion) { el.classList.add('aw-stat-pending'); el.style.transitionDelay = `${i * 0.12}s` }
            statObserver.observe(el)
        })

        const statFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.aw-stat-item.aw-stat-pending').forEach(el => {
                el.classList.remove('aw-stat-pending')
                el.classList.add('aw-stat-visible')
                const num = el.querySelector('.aw-stat-num')
        ...
        if (heroH1) {
            ...
            heroH1.classList.add('aw-glitch')
        }

        let currentMagBtn = null
        function applyMagnetic() {
            wrapper.querySelectorAll(
                '[data-testid="topic-back-btn"]:not(.aw-magnetic-init), ' +
                '[data-testid="dark-mode-toggle"]:not(.aw-magnetic-init)'
            ).forEach(btn => { btn.classList.add('aw-magnetic-init', 'no-hover-scale') })
        }
        applyMagnetic()

        function onWrapperMouseMove(e) {
            if (noMotion) return
            const btn = e.target.closest('button.aw-magnetic-init, a.aw-magnetic-init')
        ...
        function onWrapperPointerDown(e) {
            ...
            btn.classList.remove('aw-squash')
            void btn.offsetWidth
            btn.classList.add('aw-squash')
            btn.addEventListener('animationend', () => btn.classList.remove('aw-squash'), { once: true })
            const r = btn.getBoundingClientRect()
            const size = Math.max(r.width, r.height) * 1.8
            const span = document.createElement('span')
            span.className = 'aw-ripple-span'
        ...
        function onContentMouseMove(e) {
            if (noMotion) return
            const block = e.target.closest('.aw-block')
        ...
        const revealFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.aw-reveal:not(.aw-visible)').forEach(el => {
                const top = el.getBoundingClientRect().top
                if (top < window.innerHeight * 1.5) el.classList.add('aw-visible')
            })
        }, 1500)
        ...
        function onScroll() {
            ...
            wrapper.style.setProperty('--aw-scroll-y', `${sY * 0.08}px`)
            const totalHeight = document.documentElement.scrollHeight - window.innerHeight
            const progress = totalHeight > 0 ? (sY / totalHeight) * 100 : 0
            wrapper.style.setProperty('--scroll-percent', `${progress}%`)
            const pctEl = wrapper.querySelector('.aw-wave-percent')
            if (pctEl) pctEl.textContent = `${Math.round(progress)}%`
            wrapper.querySelectorAll('.aw-reveal:not(.aw-visible)').forEach(el => {
                if (el.getBoundingClientRect().top < window.innerHeight + 120) {
                    requestAnimationFrame(() => el.classList.add('aw-visible'))
                }
            })
        }
        ...
        return () => {
            ...
            if (heroH1) { heroH1.classList.remove('aw-glitch'); heroH1.removeAttribute('data-text') }
            wrapper.querySelectorAll('.aw-magnetic-init').forEach(btn => {
                btn.classList.remove('aw-magnetic-init', 'no-hover-scale', 'aw-squash')
                btn.style.removeProperty('transform')
                btn.style.removeProperty('transition')
            })
            ...
            wrapper.style.removeProperty('--aw-scroll-y')
    ...
    return (
        <div className="aws-page">
            <TopicPage
                ...
                extraBanner={<AWSStatsBanner />}
                headerExtra={soundToggleButton}
            />
            <div
                className="aw-wave-progress"
                onClick={scrollToTop}
                title={language === 'tr' ? 'Yukarı Git' : 'Scroll to Top'}
                data-testid="ocean-progress"
            >
                <div className="aw-wave-water" />
                <span className="aw-wave-percent">0%</span>
            </div>
        </div>
    )
}
