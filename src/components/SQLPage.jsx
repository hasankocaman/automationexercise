...
const THUNDER_INTERVAL_MS = 10000 // sql-lightning-flash CSS animasyonuyla aynı 10s döngü

/* ─── SQL Sorgu Yürütme Sırası: yazılış sırası ≠ çalışma sırası ─── */
function SQLPipeline({ isTr }) {
    const pipelineRef = useRef(null)
    const stages = [
        { label: 'FROM', color: 'var(--sql-role-accent)', bg: 'rgba(232,163,61,0.13)', border: 'rgba(232,163,61,0.38)', z: 5, ty: -30 },
        { label: 'WHERE', color: 'var(--sql-role-accent-2)', bg: 'rgba(52,201,184,0.12)', border: 'rgba(52,201,184,0.36)', z: 4, ty: -18 },
        { label: 'GROUP BY / HAVING', color: 'var(--sql-role-muted)', bg: 'rgba(212,188,201,0.11)', border: 'rgba(212,188,201,0.30)', z: 3, ty: -8 },
        { label: 'SELECT', color: 'var(--sql-role-success)', bg: 'rgba(111,191,115,0.11)', border: 'rgba(111,191,115,0.30)', z: 2, ty: 2 },
        { label: 'ORDER BY', color: 'var(--sql-role-muted)', bg: 'rgba(212,188,201,0.08)', border: 'rgba(212,188,201,0.22)', z: 1, ty: 9 },
    ]
    ...
    return (
        <div className="sql-pipeline sql-pipeline-interactive" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="sql-pipeline-title">{isTr ? 'Gerçek Yürütme Sırası' : 'Real Execution Order'}</div>
            <div ref={pipelineRef} className="sql-stages sql-stages-3d">
                {stages.map((stage, idx) => (
                    <div key={idx} className="sql-stage" style={{ '--stage-z': stage.z, '--ty': `${stage.ty}px`, background: stage.bg, borderColor: stage.border, color: stage.color }}>
                        <span className="sql-stage-dot" style={{ background: stage.color }} />
                        <span className="sql-stage-label">{stage.label}</span>
                    </div>
                ))}
            </div>
        </div>
...
/* ─── SQL Konsolu & Yürütme Sırası Panosu ──────────────────────── */
function SQLConsoleSimulator({ isTr }) {
    const [inputValue, setInputValue] = useState('')
    const [consoleOutput, setConsoleOutput] = useState(
        isTr
            ? 'SQL konsoluna hoş geldiniz.\nÇalıştırmak için explain yazın...'
            : 'Welcome to the SQL console.\nType explain to run...'
    )
    // Yazılış sırası burada; execOrder gerçek çalışma sırasını gösterir
    const clauses = [
        { id: 'select', text: 'SELECT name, COUNT(*) AS cnt', execOrder: 5 },
        { id: 'from', text: 'FROM users', execOrder: 1 },
        { id: 'where', text: 'WHERE active = 1', execOrder: 2 },
        { id: 'groupby', text: 'GROUP BY name', execOrder: 3 },
        { id: 'having', text: 'HAVING cnt > 1', execOrder: 4 },
        { id: 'orderby', text: 'ORDER BY cnt DESC', execOrder: 6 },
    ]
    ...
    function runDemo(raw) {
        ...
        timersRef.current = []
        const sorted = [...clauses].sort((a, b) => a.execOrder - b.execOrder)
        sorted.forEach((c, idx) => {
            const t = setTimeout(() => setDoneIds(prev => [...prev, c.id]), idx * 320)
            timersRef.current.push(t)
        })
        const finalTimer = setTimeout(() => {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + (
                isTr
                    ? 'Yürütme sırası: FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY\n(SELECT ilk yazılır ama 5. çalışır!)'
                    : 'Execution order: FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY\n(SELECT is written first but runs 5th!)'
            ))
        }, sorted.length * 320 + 100)
    ...
    function handleCommandSubmit(e) {
        ...
        if (cmd === 'explain' || cmd.startsWith('select')) {
            runDemo(raw)
        } else {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + (
                isTr
                ...
                <div className="sql-console-body">
                    ...
                    <div className="sql-console-input-row">
                        ...
                        <input
                            ...
                            placeholder={isTr ? 'Komut yazın (explain)...' : 'Type a command (explain)...'}
                        />
                    </div>
                </div>
            </div>
            <div className="sql-order-board">
                {clauses.map(c => (
                    <div key={c.id} className={`sql-order-item${doneIds.includes(c.id) ? ' done' : ''}`}>
                        <span className="sql-order-step">{doneIds.includes(c.id) ? c.execOrder : ''}</span>
                        <span>{c.text}</span>
                    </div>
                ))}
            </div>
...
/* ─── Stats + SQL Pipeline asimetrik hero banner ───────────────── */
function SQLStatsBanner() {
    const { language } = useLanguage()
    const isTr = language === 'tr'

    const stats = [
        { target: 1974, suffix: '',  tr: 'Yayın Yılı',          en: 'Founded'            },
        { target: 6,    suffix: '',  tr: 'Yürütme Adımı',        en: 'Execution Steps'    },
        { target: 100,  suffix: '%', tr: 'Deklaratif',           en: 'Declarative'        },
        { target: 4,    suffix: '',  tr: 'CRUD İşlemi',          en: 'CRUD Operations'    },
    ]

    return (
        <div className="sql-hero-banner-container">
            <div className="sql-hero-banner">
                <SQLPipeline isTr={isTr} />
                <div className="sql-stats-bar">
                    {stats.map(s => (
                        <div key={s.tr} className="sql-stat-item">
                            <div className="sql-stat-number-wrap">
                                <span className="sql-stat-num" data-target={s.target}>0</span>
                                {s.suffix && <span className="sql-stat-suffix">{s.suffix}</span>}
                            </div>
                            <p className="sql-stat-label">{isTr ? s.tr : s.en}</p>
                        </div>
                    ))}
                </div>
            </div>
            <SQLConsoleSimulator key={isTr ? 'tr' : 'en'} isTr={isTr} />
        </div>
    )
}
...
function SQLPage() {
    ...
    useEffect(() => {
        ...
        const particles = []
        const pColors = ['#e8a33d', '#34c9b8', '#f4c06a', '#6fbf73', '#f0b84e']
        for (let i = 0; i < 20; i++) {
            const p = document.createElement('div')
            p.className = 'sql-particle'
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                ...
                if (e.target.classList.contains('sql-reveal')) {
                    requestAnimationFrame(() => requestAnimationFrame(() => e.target.classList.add('sql-visible')))
                }
        ...
        function setupReveal(el) {
            if (el.dataset.sqlReveal) return
            el.dataset.sqlReveal = '1'
            const inVP = el.getBoundingClientRect().top < window.innerHeight
            if (!inVP && !noMotion) el.classList.add('sql-reveal')
        ...
        function applyBlockClasses() {
            ...
            Array.from(card.children).forEach(child => {
                ...
                if (!child.classList.contains('sql-block')) child.classList.add('sql-block')
                setupReveal(child)
            })
        }
        applyBlockClasses()
        ...
        const statObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                if (!e.isIntersecting) return
                const item = e.target
                item.classList.remove('sql-stat-pending')
                item.classList.add('sql-stat-visible')
                const num = item.querySelector('.sql-stat-num')
        ...
        wrapper.querySelectorAll('.sql-stat-item').forEach((el, i) => {
            if (!noMotion) { el.classList.add('sql-stat-pending'); el.style.transitionDelay = `${i * 0.12}s` }
            statObserver.observe(el)
        })

        const statFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.sql-stat-item.sql-stat-pending').forEach(el => {
                el.classList.remove('sql-stat-pending')
                el.classList.add('sql-stat-visible')
                const num = el.querySelector('.sql-stat-num')
        ...
        if (heroH1) {
            ...
            heroH1.classList.add('sql-glitch')
        }

        let currentMagBtn = null
        function applyMagnetic() {
            wrapper.querySelectorAll(
                '[data-testid="topic-back-btn"]:not(.sql-magnetic-init), ' +
                '[data-testid="dark-mode-toggle"]:not(.sql-magnetic-init)'
            ).forEach(btn => { btn.classList.add('sql-magnetic-init', 'no-hover-scale') })
        }
        applyMagnetic()

        function onWrapperMouseMove(e) {
            if (noMotion) return
            const btn = e.target.closest('button.sql-magnetic-init, a.sql-magnetic-init')
        ...
        function onWrapperPointerDown(e) {
            ...
            btn.classList.remove('sql-squash')
            void btn.offsetWidth
            btn.classList.add('sql-squash')
            btn.addEventListener('animationend', () => btn.classList.remove('sql-squash'), { once: true })
            const r = btn.getBoundingClientRect()
            const size = Math.max(r.width, r.height) * 1.8
            const span = document.createElement('span')
            span.className = 'sql-ripple-span'
            span.style.width = span.style.height = `${size}px`
            span.style.left = `${e.clientX - r.left - size / 2}px`
            span.style.top = `${e.clientY - r.top - size / 2}px`
            span.style.background = 'rgba(245, 236, 241, 0.45)'
        ...
        const revealFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.sql-reveal:not(.sql-visible)').forEach(el => {
                const top = el.getBoundingClientRect().top
                if (top < window.innerHeight * 1.5) el.classList.add('sql-visible')
            })
        }, 1500)
        ...
        function onScroll() {
            ...
            wrapper.style.setProperty('--sql-scroll-y', `${sY * 0.08}px`)
            const totalHeight = document.documentElement.scrollHeight - window.innerHeight
            const progress = totalHeight > 0 ? (sY / totalHeight) * 100 : 0
            wrapper.style.setProperty('--scroll-percent', `${progress}%`)
            const pctEl = wrapper.querySelector('.sql-wave-percent')
            if (pctEl) pctEl.textContent = `${Math.round(progress)}%`
            wrapper.querySelectorAll('.sql-reveal:not(.sql-visible)').forEach(el => {
                if (el.getBoundingClientRect().top < window.innerHeight + 120) {
                    requestAnimationFrame(() => el.classList.add('sql-visible'))
                }
            })
        }
        ...
        return () => {
            ...
            if (heroH1) { heroH1.classList.remove('sql-glitch'); heroH1.removeAttribute('data-text') }
            wrapper.querySelectorAll('.sql-magnetic-init').forEach(btn => {
                btn.classList.remove('sql-magnetic-init', 'no-hover-scale', 'sql-squash')
                btn.style.removeProperty('transform')
                btn.style.removeProperty('transition')
            })
            ...
            wrapper.style.removeProperty('--sql-scroll-y')
    ...
    return (
        <div className="sql-page">
            ...
            >
                <div className="sql-wave-water" />
                <span className="sql-wave-percent">0%</span>
            </div>
        </div>
    )
}
