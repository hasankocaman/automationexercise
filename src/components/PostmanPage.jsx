...
import '../postman-effects.css'
import '../night-sky-effects.css'

const SOUND_PREF_KEY = 'ambientSoundEnabled'
const THUNDER_INTERVAL_MS = 10000 // pm-lightning-flash CSS animasyonuyla aynı 10s döngü

/* ─── API Request Lifecycle: bir istek nasıl işlenir ─── */
function PostmanPipeline({ isTr }) {
    const pipelineRef = useRef(null)
    const stages = [
        { label: isTr ? 'İstek Oluşturma (Builder)' : 'Request Builder', color: 'var(--pm-role-accent)', bg: 'rgba(255,107,91,0.13)', border: 'rgba(255,107,91,0.38)', z: 5, ty: -30 },
        { label: 'Pre-request Script', color: 'var(--pm-role-accent-2)', bg: 'rgba(61,219,184,0.12)', border: 'rgba(61,219,184,0.36)', z: 4, ty: -18 },
        { label: isTr ? 'Gönder (Send)' : 'Send', color: 'var(--pm-role-muted)', bg: 'rgba(183,197,219,0.11)', border: 'rgba(183,197,219,0.30)', z: 3, ty: -8 },
        { label: isTr ? 'Test Script (Doğrulama)' : 'Test Script', color: 'var(--pm-role-success)', bg: 'rgba(111,191,115,0.11)', border: 'rgba(111,191,115,0.30)', z: 2, ty: 2 },
        { label: isTr ? 'Yanıt (Response)' : 'Response', color: 'var(--pm-role-muted)', bg: 'rgba(183,197,219,0.08)', border: 'rgba(183,197,219,0.22)', z: 1, ty: 9 },
    ]
    ...
    return (
        <div className="pm-pipeline pm-pipeline-interactive" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="pm-pipeline-title">{isTr ? 'API İstek Yaşam Döngüsü' : 'API Request Lifecycle'}</div>
            <div ref={pipelineRef} className="pm-stages pm-stages-3d">
                {stages.map((stage, idx) => (
                    <div key={idx} className="pm-stage" style={{ '--stage-z': stage.z, '--ty': `${stage.ty}px`, background: stage.bg, borderColor: stage.border, color: stage.color }}>
                        <span className="pm-stage-dot" style={{ background: stage.color }} />
...
/* ─── Postman Konsolu & pm.test Sonuç Panosu ───────────────────── */
function PostmanConsoleSimulator({ isTr }) {
    const [inputValue, setInputValue] = useState('')
    const [consoleOutput, setConsoleOutput] = useState(
        isTr
            ? 'Postman konsoluna hoş geldiniz.\nGöndermek için send yazın...'
            : 'Welcome to the Postman console.\nType send to run the request...'
    )
    const checks = [
        { id: 'c1', label: isTr ? 'Status code 200' : 'Status code is 200' },
        { id: 'c2', label: isTr ? 'Yanıt süresi < 500ms' : 'Response time < 500ms' },
        { id: 'c3', label: isTr ? 'Body "success" içeriyor' : 'Body contains "success"' },
    ]
    ...
    function runDemo(raw) {
        ...
        const finalTimer = setTimeout(() => {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + 'HTTP/1.1 200 OK\n{ "status": "success", "data": {...} }\n\n' + (isTr ? '3/3 test geçti ✓' : '3/3 tests passed ✓'))
    ...
    function handleCommandSubmit(e) {
        ...
        if (cmd === 'send' || cmd.startsWith('get') || cmd.startsWith('post')) {
            runDemo(raw)
        } else {
            setConsoleOutput(prev => prev + `\n> ${raw}\n` + (
                isTr
    ...
    return (
        <div className="pm-console-box pm-reveal">
            <div className="pm-console">
                <div className="pm-console-header">
                    <span>postman-console v1.0</span>
                    <span>{isTr ? 'ÇALIŞIYOR' : 'RUNNING'}</span>
                </div>
                <div className="pm-console-body">
                    {consoleOutput}
                    <div className="pm-console-input-row">
                        <span className="pm-console-prompt">&gt;</span>
                        <input
                            type="text"
                            value={inputValue}
                            onChange={(e) => setInputValue(e.target.value)}
                            onKeyDown={handleCommandSubmit}
                            className="pm-console-input"
                            placeholder={isTr ? 'Komut yazın (send)...' : 'Type a command (send)...'}
                        />
                    </div>
                </div>
            </div>
            <div className="pm-order-board">
                {checks.map(c => (
                    <div key={c.id} className={`pm-order-item${doneIds.includes(c.id) ? ' done' : ''}`}>
                        <span className="pm-order-check">{doneIds.includes(c.id) ? '✓' : '○'}</span>
                        <span>{c.label}</span>
                    </div>
                ))}
            </div>
...
/* ─── Stats + API Pipeline asimetrik hero banner ───────────────── */
function PostmanStatsBanner() {
    const { language } = useLanguage()
    const isTr = language === 'tr'

    const stats = [
        { target: 2012, suffix: '',  tr: 'Yayın Yılı',              en: 'Founded'               },
        { target: 100,  suffix: '%', tr: 'API Testi',                en: 'API Testing'           },
        { target: 3,    suffix: '',  tr: 'Ortam Değişkeni Kapsamı',  en: 'Environment Scopes'    },
        { target: 5,    suffix: '+', tr: 'HTTP Metodu',              en: 'HTTP Methods'          },
    ]

    return (
        <div className="pm-hero-banner-container">
            <div className="pm-hero-banner">
                <PostmanPipeline isTr={isTr} />
                <div className="pm-stats-bar">
                    {stats.map(s => (
                        <div key={s.tr} className="pm-stat-item">
                            <div className="pm-stat-number-wrap">
                                <span className="pm-stat-num" data-target={s.target}>0</span>
                                {s.suffix && <span className="pm-stat-suffix">{s.suffix}</span>}
                            </div>
                            <p className="pm-stat-label">{isTr ? s.tr : s.en}</p>
                        </div>
                    ))}
                </div>
            </div>
            <PostmanConsoleSimulator key={isTr ? 'tr' : 'en'} isTr={isTr} />
        </div>
    )
}
...
function PostmanPage() {
    ...
    useEffect(() => {
        const wrapper = document.querySelector('.postman-page')
        if (!wrapper) return
        ...
        const particles = []
        const pColors = ['#ff6b5b', '#3ddbb8', '#ff9686', '#6fbf73', '#f0b84e']
        for (let i = 0; i < 20; i++) {
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                ...
                if (e.target.classList.contains('pm-reveal')) {
                    requestAnimationFrame(() => requestAnimationFrame(() => e.target.classList.add('pm-visible')))
                }
        ...
        function setupReveal(el) {
            if (el.dataset.pmReveal) return
            el.dataset.pmReveal = '1'
            const inVP = el.getBoundingClientRect().top < window.innerHeight
            if (!inVP && !noMotion) el.classList.add('pm-reveal')
        ...
        function applyBlockClasses() {
            ...
            Array.from(card.children).forEach(child => {
                ...
                if (!child.classList.contains('pm-block')) child.classList.add('pm-block')
                setupReveal(child)
            })
        }
        applyBlockClasses()
        ...
        const statObserver = new IntersectionObserver((entries) => {
            entries.forEach(e => {
                if (!e.isIntersecting) return
                const item = e.target
                item.classList.remove('pm-stat-pending')
                item.classList.add('pm-stat-visible')
                const num = item.querySelector('.pm-stat-num')
        ...
        wrapper.querySelectorAll('.pm-stat-item').forEach((el, i) => {
            if (!noMotion) { el.classList.add('pm-stat-pending'); el.style.transitionDelay = `${i * 0.12}s` }
            statObserver.observe(el)
        })

        const statFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.pm-stat-item.pm-stat-pending').forEach(el => {
                el.classList.remove('pm-stat-pending')
                el.classList.add('pm-stat-visible')
                const num = el.querySelector('.pm-stat-num')
        ...
        if (heroH1) {
            ...
            heroH1.classList.add('pm-glitch')
        }

        let currentMagBtn = null
        function applyMagnetic() {
            wrapper.querySelectorAll(
                '[data-testid="topic-back-btn"]:not(.pm-magnetic-init), ' +
                '[data-testid="dark-mode-toggle"]:not(.pm-magnetic-init)'
            ).forEach(btn => { btn.classList.add('pm-magnetic-init', 'no-hover-scale') })
        }
        applyMagnetic()

        function onWrapperMouseMove(e) {
            if (noMotion) return
            const btn = e.target.closest('button.pm-magnetic-init, a.pm-magnetic-init')
        ...
        function onWrapperPointerDown(e) {
            ...
            btn.classList.remove('pm-squash')
            void btn.offsetWidth
            btn.classList.add('pm-squash')
            btn.addEventListener('animationend', () => btn.classList.remove('pm-squash'), { once: true })
            const r = btn.getBoundingClientRect()
            const size = Math.max(r.width, r.height) * 1.8
            const span = document.createElement('span')
            span.className = 'pm-ripple-span'
            span.style.width = span.style.height = `${size}px`
            span.style.left = `${e.clientX - r.left - size / 2}px`
            span.style.top = `${e.clientY - r.top - size / 2}px`
            span.style.background = 'rgba(233, 241, 250, 0.45)'
        ...
        const revealFallbackTimer = setTimeout(() => {
            wrapper.querySelectorAll('.pm-reveal:not(.pm-visible)').forEach(el => {
                const top = el.getBoundingClientRect().top
                if (top < window.innerHeight * 1.5) el.classList.add('pm-visible')
            })
        }, 1500)
        ...
        function onScroll() {
            ...
            wrapper.style.setProperty('--pm-scroll-y', `${sY * 0.08}px`)
            const totalHeight = document.documentElement.scrollHeight - window.innerHeight
            const progress = totalHeight > 0 ? (sY / totalHeight) * 100 : 0
            wrapper.style.setProperty('--scroll-percent', `${progress}%`)
            const pctEl = wrapper.querySelector('.pm-wave-percent')
            if (pctEl) pctEl.textContent = `${Math.round(progress)}%`
            wrapper.querySelectorAll('.pm-reveal:not(.pm-visible)').forEach(el => {
                if (el.getBoundingClientRect().top < window.innerHeight + 120) {
                    requestAnimationFrame(() => el.classList.add('pm-visible'))
                }
            })
        }
        ...
        return () => {
            ...
            if (heroH1) { heroH1.classList.remove('pm-glitch'); heroH1.removeAttribute('data-text') }
            wrapper.querySelectorAll('.pm-magnetic-init').forEach(btn => {
                btn.classList.remove('pm-magnetic-init', 'no-hover-scale', 'pm-squash')
                btn.style.removeProperty('transform')
                btn.style.removeProperty('transition')
            })
            ...
            wrapper.style.removeProperty('--pm-scroll-y')
    ...
    const soundToggleButton = isLightMode ? (
        <button
            ...
            data-testid="postman-sound-toggle"
        >
            {soundOn ? '🔊' : '🔇'}
        </button>
    ) : null
    ...
    return (
        <div className="postman-page">
            <TopicPage
                ...
                extraBanner={<PostmanStatsBanner />}
                headerExtra={soundToggleButton}
            />
            <div
                className="pm-wave-progress"
                onClick={scrollToTop}
                title={language === 'tr' ? 'Yukarı Git' : 'Scroll to Top'}
                data-testid="ocean-progress"
            >
                <div className="pm-wave-water" />
                <span className="pm-wave-percent">0%</span>
            </div>
        </div>
    )
}
