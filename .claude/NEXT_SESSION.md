...
| **Son commit** | `67b54b9` — Cypress Framework Mimarisi (SOLID + POM) sekmesi eklendi |
| **Çalışma ağacı** | temiz |
...
3. ⏳ **`/rest-assured`** — SIRADA. `restAssuredData` gauge ile AYNI iskelete
   sahip (`// ── N:` + sectionIndex/Feynman deseni) — kaydırma doğrudan
   uygulanabilir olmalı, ama REST Assured'ın kendi gerçek mimarisi (RequestSpec/
   ResponseSpec, Filter'lar, POJO serialization) etrafında yeniden düşünülmeli.
4. ⏳ **`/appium`** — aynı standart, iskelet önce incelenmeli (mobil sürücü
   yönetimi Selenium'a benzer olabilir — DriverManager/ThreadLocal kıyaslaması
   burada GEÇERLİ kalabilir, kör kopya değil gerçek benzerlik).
...
playwrightData 569.69 kB (gzip 162.56 kB) · cypressData ~499 KB —
rest-assured/appium'a eklenince de not edilmeli.
...
Selenium'un testi geçmişti; Playwright ve Cypress'inki (ve sonra rest-assured/
