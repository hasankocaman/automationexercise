...
- **Bu oturumda gözlemlenen, bana ait olmayan eşzamanlı değişiklikler:** `CLAUDE.md` (Bölüm 9.1 öğretme kuralları — kalıcı kural, önceki bir kısımda eklenmiş), `scripts/generate-seo-files.mjs`, `src/data/whatIsTestingData.js` — bunlara 20. kısımda dokunulmadı, başka bir oturum/araca ait olabilir, içerikleri doğrulanmadı.
- **22. kısımda ele alınıp doğrulanan dosyalar (artık "yabancı/doğrulanmamış" değil):** `Documents/GitNotesForProfessionals.md`, `public/documents/GitNotesForProfessionals*.md`, `src/components/GitDocPage.jsx`, `src/components/GitGithubPage.jsx`, `src/locales/en.json`, `src/locales/tr.json` — detay yukarıdaki 22. kısım tablosunda.
