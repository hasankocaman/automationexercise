...
| | |
|---|---|
| **Aktif branch** | `main` — `feature/code-practice-ai-feedback` test edilmeden merge edildi (`6e80054`), ardından main'de tam `npm run build` + `npm run test:e2e` PASS oldu ve `origin/main`'e push edildi (son commit `f39457c`). `feature/code-practice-ai-feedback` hâlâ mevcut, silinmedi. |
| **Plan dosyası** | `Documents/code-practice-ai-feedback-plan.md` — Faz 1 ✅ + Faz 2 ✅ kodlama tarafı bitti, main'de. Manuel test adımları plan dosyasının sonunda. |
| **main'e merge sırasında bulunan/düzeltilen regresyon** | `tests/qa-mentor-progress-tracking.spec.ts` (AC09), bu oturumdaki qa-mentor route-completion fix'inin (bkz. aşağıdaki bölüm — `getCompletedRoutePaths()` artık SADECE `__route_complete__` sentinel satırlarını sayıyor) semantiğine göre GÜNCEL DEĞİLDİ — testin kendi upsert'i rastgele bir `topic_slug` kullanıp eski "herhangi bir completed satır = route tamamlandı" davranışını varsayıyordu, bu da main'e ilk push denemesini pre-push hook'ta kırdı (195/196). Test hem insert hem baseline sorgusunda sentinel'e (`__route_complete__`) göre düzeltildi (commit `f39457c`), tekil koşumda doğrulandı, tam pakette de PASS. |
| **Push denemeleri sırasında öğrenilen altyapı riski** | Art arda `git push`/`npm run test:e2e` denemeleri arasında (özellikle bir Bash komutu 2dk timeout'ta SIGTERM ile kesilince) Playwright'ın chromium alt süreçleri düzgün kapanmayabiliyor — `scripts/kill-stale-test-processes.ps1` SADECE `node.exe`'yi hedefliyor, yetim `chrome.exe` süreçlerine DOKUNMUYOR. Bu oturumda 66 yetim chrome süreci birikip sistemi aşırı yükledi (bir koşum 51 dakikaya çıktı, 12 test alakasız zaman aşımlarıyla başarısız oldu) — manuel temizlik sonrası aynı kod değişmeden 196/196 PASS oldu (17.4 dk). **Olası iyileştirme (henüz yapılmadı):** `kill-stale-test-processes.ps1`'e node.exe'nin child chrome.exe süreçlerini de (veya command-line'ında `--test-type`/playwright'a özgü flag'i olan chrome.exe'leri) temizleyen bir adım eklenebilir. |
| **Test politikası (main push'unda)** | Kullanıcı talimatı (2026-07-23): main'e merge test edilmeden yapıldı, ardından main'de BİR KEZ tam `npm run build` + `npm run test:e2e` çalıştırıldı (pre-push hook üzerinden, zorunlu/engelleyici). İlk deneme yukarıdaki AC09 regresyonuyla başarısız oldu, düzeltme sonrası tekrar denendi; ara denemelerde geçici DNS hatası ve zombi-chrome kaynaklı 1 flaky-timeout turu da yaşandı (kodla ilgisiz) — son temiz koşum 196/196 PASS, push başarılı. |
