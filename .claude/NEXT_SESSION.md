...
- **Aktif branch: `feature/prediction-blocks`.** Kullanıcının öğrenme bilimi değerlendirme yazısına (2026-07-27) karşılık, `Documents/learning-science-upgrade-plan.md` planı uygulandı. **3 yeni blok tipi + 5 dile rollout TAMAMLANDI:**
  ...
  - **Kalan kapsam dışı işler** (backend/mimari gerektirir, kullanıcı onayı bekliyor): kişisel AI Mentor (§5 — Supabase tablo/RPC/edge function), adaptif zorluk (§6), portföy/proje üretimi (§8) — detay plan dosyası Bölüm 5'te.
  - **Açık iş:** `main`'e merge/PR kararı kullanıcıda. SQL/TypeScript'e code-trace/heap-stack eklenmedi (Java/Python/JavaScript'te referans örnekleri var, isteğe bağlı genişletilebilir — SQL için heap/stack modeli zaten kavramsal olarak uymuyor, TS runtime davranışı JS ile aynı olduğundan düşük öncelikli).
...
3. **`javaData.js` (149→86) ve `browserstackData.js` (41→14) temizlendi (2026-07-27, bu oturum, henüz commit edilmedi):** Her iki dosyada da tüm `{TR,EN}` ve `EN-ÇEVİR` etiketli sızıntılar düzeltildi — kalan sayılar TAMAMEN ⚠ OPUS (renderer HAM basıyor, dokunulmadı, kural gereği). Baseline **199 → 109** (`npm run i18n:baseline` ile güncellendi). Kalan borç: `javaData.js` 86 (hepsi OPUS, çoğu `locator-visual`/`playwright-visual` blokları), `browserstackData.js` 14 (hepsi OPUS, `error-dictionary` `codeWrong`/`codeFixed`), `backendData.js`'deki 7 kalıcı OPUS leak.
   - **Önemli mimari bulgu — `javaData.js`'de `sPlaywright.en` "ölü kod" bug'ı:** `sPlaywright` const'ında düzgün çevrilmiş ~1250 satırlık bir `en: {...}` objesi VAR ama satır ~16849'da `sPlaywright.en = { title: '...', blocks: sPlaywright.tr.blocks }` ile EZİLİYOR — yani o güzel İngilizce içerik hiç render edilmiyor, gerçekte kullanılan array TR'nin AYNISI (referans paylaşımı). Muhtemelen sebep: orijinal `en:` objesi TR'nin sonradan eklenen adımlarını (6 `step-animation` + 1 `quiz`) içermiyordu (37 blok vs TR'nin 43'ü), birisi "eksik olacağına aynı içerik göster" diyerek override etmiş. Bu oturumda üstü çizilmedi — sadece paylaşılan TR blokları içindeki sızan alanlar `{tr,en}` yapıldı (leak azaltma kapsamında doğru/pragmatik çözüm). **Asıl mimari borç kalıcı:** ya (a) override silinip orijinal `en:` objesine eksik 6 step-animation + 1 quiz eklenmeli, ya da (b) mevcut durum (paylaşılan blok + alan-bazlı `{tr,en}`) kalıcı kabul edilip ~1250 satırlık ölü `en:` objesi silinmeli. Karar kullanıcıda — küçük bir iş değil, ayrı bir görev olarak ele alınmalı.
   ...
   - **Kalan iş:** `backendData.js`'deki 7 kalıcı OPUS leak zaten kalıcı (renderer işi). Bundan sonraki i18n borcu tamamen OPUS-only (renderer güncellemesi gerektiren `locator-visual`/`playwright-visual`/`error-dictionary` alanları) — düz metin çevirisiyle daha fazla azaltılamaz, ilerlemek için renderer'a (`tx`/`pick` desteği) Opus-seviye bir değişiklik gerekir.
