...
8. **YENİ (32. kısım) — `avatar_emoji` migration'ı kullanıcı tarafından hem `learnqa-test` hem `learnqa-prod`'da çalıştırıldı** (`lesson_comments.avatar_emoji` kolonu eklendi + `handle_verified_user` trigger'ı ilk girişte rastgele varsayılan emoji atayacak şekilde güncellendi + var olan `profiles` satırlarında boş olan `avatar_emoji` geriye dönük dolduruldu). **Doğrulanmadı/eksik kalmış olabilir:** Bu migration sadece `profiles` tablosunu düzeltti; SQL çalıştırılmadan ÖNCE yazılmış var olan `lesson_comments` satırlarının (örn. kullanıcının "deneme" test yorumu) `avatar_emoji`'si hâlâ NULL olabilir. Gerekirse şu backfill'i de her iki projede çalıştır: `update public.lesson_comments lc set avatar_emoji = p.avatar_emoji from public.profiles p where lc.user_id = p.id and lc.avatar_emoji is null;`
9. Commit'ler: `9e82416`, `e0f05fd`, `ee4d74e`, `e7fdaa1`, `d221f2f` + 31. kısım + 32. kısım commit'leri — henüz **push edilmedi**, madde 1-3 tamamlanmadan push edilmemeli.
