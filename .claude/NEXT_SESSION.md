...
1. **i18n scanner'da kritik bug bulundu ve düzeltildi (2026-07-27, commit `71c527b`):** `check-i18n-leaks.mjs` çift-ağaçlı dosyalarda **yanlış ağacı** tarıyordu (`data.tr.sections` yerine EN modunda gerçekten görünen `data.en.sections` taranmalıydı) — eski baseline'daki **8490** sızıntının büyük çoğunluğu hayaletti (TR-ağacı içeriğini yanlışlıkla "leak" sayıyordu). Gerçek borç ölçüldü: **365 → bu oturumda 223'e indirildi**. Ayrıca Windows'ta `import()` ham dosya yolu hatası scanner'ı sessizce no-op yapıyordu (düzeltildi, `pathToFileURL`). Scanner'a iki kalıcı iyileştirme eklendi: `pattern` alanı SAFE_KEYS'e alındı (görünmez regex alanı) ve `why`/`note` için sibling-aware kontrol eklendi (`why_en`/`note_en` doluysa leak sayılmıyor — java-compare bloklarının çoğunda zaten vardı, yanlış-pozitifti).
   ...
   - **`basitBackendData.js` ve `pythonData.js`'de gizli paylaşımlı sabit riski bulundu:** bazı `code`/`python`/`java` alanları TR ve EN ağaçları arasında AYNI JS `const` değişkenini paylaşıyordu (wrapper blok objesi farklı olsa bile) — scanner bunu "paylaşımsız" sanıp yanlış etiketliyordu. Doğru fix uygulandı: yeni İngilizce sabitler oluşturulup `{tr, en}` wrapper ile sarıldı, TR içeriğine dokunulmadı (kanıtlanmış: TR tarafı hâlâ Türkçe gösteriyor).
...
2. **i18n baseline borcunu düşürmeye devam** — kalan **223**, 3 dosyada: `javaData.js` **149**, `browserstackData.js` **41**, `backendData.js` **31**. Üçü de `basitBackendData.js`'deki **aynı paylaşımlı-sabit deseni** taşıyor (ör. `backendData.js`'de `schemaSql` hem TR hem EN'de aynı referans) — fix aynı iki adımlı kalıp: (1) yeni İngilizce sabit oluştur, (2) `code: { tr: eskiSabit, en: yeniSabit }` şeklinde sar, TR'ye dokunma. Bilinçli olarak bu oturumda YAPILMADI çünkü üçünde de çok sayıda BÜYÜK kod bloğu var (SQL şema, Supabase Edge Function, React bileşenleri — muhtemelen 1000+ satır Türkçe yorum), aceleyle çeviri hata riski taşırdı (CLAUDE.md §1.1). `node scripts/check-i18n-leaks.mjs --list <dosya>.js` her sızıntıyı shared/en-only tespiti + fix önerisiyle listeler (EN-ÇEVİR / YERİNDE-ÇEVİR / {TR,EN} / ⚠ OPUS). ⚠ OPUS etiketli leak'lere (backend-practice `example` alanı gibi ham render edilen paylaşımlı alanlar) hiçbir zaman dokunulmamalı — renderer güncellemesi gerektirir.
3. **`/qa-frontend` → `main` merge/PR kararı** kullanıcıda; sayfa içerik olarak bitti, manuel test rehberi (`Documents/qa-frontend-page-plan.md` §F) hazır, isteğe bağlı olarak `npm run test:e2e` ile Playwright koşumu yapılabilir.
4. **`main`'e merge/PR kararı** (her iki açık branch için de) kullanıcıda.
5. **AC08 çoklu tema paleti** — kullanıcı "şimdilik atla" dedi, plan `Documents/acceptancecriterias.md` Madde 11'de hazır bekliyor.
6. **Bilinen ASCII-normalize Türkçe kör noktası** — `bakiyor`, `gunceller` gibi Türkçe-özgü karakter içermeyen sızıntılar hiçbir otomatik kontrolle yakalanamıyor, elle göz gezdirmek gerekiyor.
