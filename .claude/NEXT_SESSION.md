...
4. ⏳ **`/appium`** — SIRADA (SON SAYFA). Aynı standart uygulanacak; iskelet
   önce incelenmeli (mobil sürücü yönetimi Selenium'a benzer olabilir —
   DriverManager/ThreadLocal kıyaslaması burada GEÇERLİ kalabilir, kör kopya
   değil gerçek benzerlik). **UYARI:** eğer restAssuredData gibi tek-ağaç +
   sectionIndex deseni kullanıyorsa, yeni section'ı eklerken SONRAKİ tüm
   sectionIndex referanslarının (varsa Feynman/başka statik dizi) kaymasını
   KONTROL ET — restAssuredData'da bu risk YÖNETİLDİ, appium'da da aynı
   kontrol yapılmalı.
...
restAssuredData ~325 KB — appium'a eklenince de not edilmeli.
...
Selenium'un testi geçmişti; Playwright, Cypress ve REST Assured'ınki (ve
sonra appium'unki) HENÜZ koşulmadı.
