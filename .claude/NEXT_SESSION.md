...
- **Aktif branch: `frontenddevelopment-for-qa`.** Yeni sayfa üzerinde çalışılıyor: **`/qa-frontend` — "QA için Frontend: Developer'la Aynı Dili Konuşmak"**. Plan + görev dağılımı (Opus/Sonnet) + hazır Sonnet promptları: `Documents/qa-frontend-page-plan.md`.
  ...
  - **GRUP J (Mülakat Soruları) TAMAMLANDI ✅** (Sonnet, faz D-S10): `interview-questions` bloğu, **tam 50 soru** (15 Basic / 20 Intermediate / 15 Advanced, doğrulandı — `grep -c "level:"`). Hiçbir "X nedir?" tanım sorusu yok, hepsi senaryo tabanlı (production bug'ı, PR review, deploy sonrası kırılma vb.); her cevap 3-6 cümle + Java analojisi. Sorular sayfanın tüm GRUP'larına (A-I) köprü kuruyor. `relatedTopicId: 'qaf-j-interview'`. Mülakat sekmesi %60 quiz-gating arkasında (beklenen davranış, CLAUDE.md §9.5).
  - **🎉 TÜM İÇERİK FAZLARI (D-S1 → D-S10) TAMAMLANDI.** `/qa-frontend` sayfası artık GRUP A-J'nin tamamında atomik içerik, video-scene, animasyon, sandbox, quiz+retry ve mülakat sorularıyla dolu.
  - **Çalışma düzeni (kullanıcı talimatı, 2026-07-25):** her fazın sonunda (1) bu dosya güncellenir, (2) **test/build çalıştırılmadan** commit atılır (sadece `node --check` ile syntax doğrulanır — bu "test" sayılmıyor, çökme riskini önlemek için), (3) plandaki bir sonraki hazır Sonnet promptuyla devam edilir — bu döngü tüm fazlar bitene kadar tekrarlanır.
  - **Sıradaki (ve SON) faz: D-S11 — Kapanış Denetimi.** Şimdiye kadar hiçbir fazda `npm run build`/`i18n:check`/`check-content-integrity` ÇALIŞTIRILMADI (sadece `node --check` ile syntax doğrulandı) — bu üçünü artık ÇALIŞTIRMANIN ve çıkan TÜM hataları düzeltmenin zamanı. Ayrıca: §9.5 denetimi (her sekmede ≥1 video+animasyon+sandbox — muhtemelen sağlanıyor ama doğrulanmadı), `tests/video-scene.spec.ts` kapsamı gözden geçirilmeli, `scripts/check-i18n-leaks.mjs`'teki `TRIO_COMPLETE_PAGES`'e `qaFrontendData.js` eklenmeli (STRICT_ZERO_FILES'a zaten ekliydi), ve bu dosya (NEXT_SESSION.md) kapanışta güncellenmeli. Prompt plan dosyası §D-S11'de hazır.
